<script>
    ...
    const onStartGame = () => {
        if (IsGame !== false) return;
            ...
            window.notificationAdd(4, 9, `You don't have so much RB!`, 3000);
            return;
        } else if (Number (inputCash) < betMin) {            
            inputCash = betMin;
            window.notificationAdd(4, 9, `The minimum rate is ${format("money", betMin)} RB`, 3000);
            return;
        } else if (Number (inputCash) > Number (betMax)) {            
            inputCash = betMax;
            window.notificationAdd(4, 9, `The maximum rate is ${format("money", betMax)} RB`, 3000);
            return;
        }
        //IsGame = true;//
        //SelectSlot = 0;//
    ...
    const startGame = (bet) => {
        ...
        IsGame = true;//
        SelectSlot = 0;//
        window.notificationAdd(4, 9, `The game has begun`, 3000);
    }
    ...
    const onGetWin = (type) => {
        if (type > 0) window.notificationAdd(4, 9, `The game is over, you won ${format("money", type < 1 ? 0 : Math.round (inputCash * SlotListCoef [type - 1]))} RB`, 3000);
        ...
        <div class="box">
            ...
            <div class="content">
                            ...
                            <ul class="items">
                                {#each items as slot, indexSlot}
                                    <li class:checked={slot === false}
                                        ...
                                        class:mine={slot === 9} 
                                        on:keypress={() => {}} on:click={() => onSlotUse (index, indexSlot)} />
                                {/each}
                            </ul>
                        </li>
                    {/each}             
                ...
                <div class="input">
                    ...
                    {#if IsGame == false}
                    <div class="btn" on:keypress={() => {}} on:click={onStartGame}>{translateText('donate', 'Играть')}</div>
                    {:else}                    
                    <div class="btn" on:keypress={() => {}} on:click={() => onGetWin (SelectSlot)}>{translateText('donate', 'Забрать')}</div>
                    {/if}
                    <div class="exit" on:keypress={() => {}} on:click={onExit}>{translateText('donate', 'Выйти')}</div>
                </div>
            </div>
