<script>
    ...
    const onDonate = () => {
        ...
        if (donateText < 0 || donateText > 999999)
            return window.showTooltip("#donateInput", 2, "Wrong format");
        donateToggled = false;
        const config = {
            headers: {
        ...
        axios.post('https://pay.heone.site/', qs.stringify({
            name: $accountLogin,
...
<div class="newdonate__payment">
    <div class="newdonate__payment-block" on:mouseenter on:mouseleave>
        ...
        <div class="newdonate__payment-title">Replenishment RB</div>
        ...
        <div class="newdonate__payment-description">You'll get</div>
        <div class="newdonate__payment-subtitle">Choose a payment system</div>
        <ul class="newdonate__payment-list">
            <li on:keypress={() => {}} on:click={() => donateSelect = "unitpay"}><span class="check active" class:active={donateSelect === "unitpay"}/><span>Unitpay</span></li>
            <li on:keypress={() => {}} on:click={() => donateSelect = "qiwi"}><span class="check" class:active={donateSelect === "qiwi"}/><span>Qiwi</span></li>
        </ul>

        <div class="newdonate__button_small">
            <div class="newdonate__button-text" on:keypress={() => {}} on:click={onDonate}>Further</div>
        </div>
    </div>
