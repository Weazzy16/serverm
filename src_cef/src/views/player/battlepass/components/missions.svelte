...
<div class="battlepass__missions">
    {#if missionsArray.filter((mission) => isStatusActive (mission.status)).length === missionsArray.length}
        <div class="battlepass__missions_column">
            <div class="battlepass__button battlepass__block_unpremium skip" on:click={onSkip}>{translateText('player', 'ОБНОВИТЬ ЗА')} 50000 RB</div>
        </div>
    {:else if missionsArray.filter((mission) => mission.status === statusData.selected).length > 0}
        <div class="battlepass__missions_column">
            <div class="battlepass__button battlepass__block_unpremium skip" on:click={onSkip}>{translateText('player', 'ВЫПОЛНИТЬ ЗА')} 15000 RB</div>
        </div>
    {/if}
    ...
    <div class="battlepass__missions_bot">
        ...
        <div><span class="red">{translateText('player', 'Награда за прохождение всех')}</span> 100.000.000$ <span class="red">{translateText('player', 'и')}</span> 10 XP</div>
    </div>
</div>