...
export const getPng = (localItem, iconInfo) => {
  try {
    ...
    if (!iconInfo || typeof iconInfo.functionType !== 'number') {
      ...
      return document.cloud + `inventoryItems/items/${id}.png`;
    }
    ...
    if (id !== ItemId.BodyArmor && iconInfo.functionType === ItemType.Clothes) {
      ...
      return document.cloud + `${dir}/${drawable}_${texture}.png`;
    }

    // Купон на машину
    if (id === ItemId.CarCoupon) {
      return document.cloud + `inventoryItems/vehicle/${data.toLowerCase()}.png`;
    }
