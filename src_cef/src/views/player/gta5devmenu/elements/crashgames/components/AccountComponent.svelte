...
<div class="crash-avatar-wrapper">
  {#if show('avatar')}
    <div class="crash-avatar">
      {#if useSvg}
        <img alt="crash-avatar" src={iconPath} />
      {:else}
        <img alt="crash-avatar" src={src} on:error={onError} />
      {/if}
      {#if isYou && show('you')}
        <div class="crash-you"><p>{youText}</p></div>
      {/if}
    </div>
  {/if}
  ...
  {#if show('login') || show('accountId') || show('serverName')}
    <div class="crash-content">
      <div class="crash-main-info">
        {#if show('login')}
          <p>{displayLogin}</p>
        {/if}

        <div class="crash-more-info">
          {#if show('accountId') || (!show('avatar') && isYou && show('you'))}
            <div class="crash-static">
              {#if show('accountId')}
          ...
          {#if show('serverName') && displayServer}
            <div class="crash-serverName"><p>{displayServer}</p></div>
          {/if}
        </div>
      </div>
