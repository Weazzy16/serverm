<script>
  ...
  // ====== Ð ÐµÐ°ÐºÑ‚Ð¸Ð²Ð½Ð°Ñ ÑÑ‚Ñ€ÑƒÑ / Ð²Ð¸Ð´ÐµÐ¾ ======
  ...
  function draw(){
    ...
    // --- Ð¡ÐÐ˜Ð—Ð£: ÑˆÐºÐ°Ð»Ð° Ð²Ñ€ÐµÐ¼ÐµÐ½Ð¸ ---
    ...
    // ÑÐ°Ð¼Ð¾Ð»Ñ‘Ñ‚ Ð¸ ÑÑ‚Ñ€ÑƒÑ â€” Ñ‚Ð¾Ð»ÑŒÐºÐ¾ Ð² Ð¿Ð¾Ð»Ñ‘Ñ‚Ðµ
    if (!holdCrashed){
  ...
  // === Ð”Ð°Ð½Ð½Ñ‹Ðµ Ð¸Ð· ÑÐ½Ð°Ð¿ÑˆÐ¾Ñ‚Ð° ===
  ...
  let pageIndex = 0;            // 0 â€” main, 1 â€” historyList, 2 â€” historyItem
  let historyTargetId = null;   // id Ð¸Ð³Ñ€Ñ‹ Ð´Ð»Ñ ÑÑ‚Ñ€Ð°Ð½Ð¸Ñ†Ñ‹ historyItem
  ...
  // --- Ð¡ÐÐÐŸÐ¨ÐžÐ¢ Ð·Ð°Ð²ÐµÑ€ÑˆÐ¸Ð²ÑˆÐµÐ³Ð¾ÑÑ Ñ€Ð°ÑƒÐ½Ð´Ð° Ð´Ð»Ñ Ð¿Ñ€Ð°Ð²Ð¾Ð¹ ÐºÐ¾Ð»Ð¾Ð½ÐºÐ¸ ---
  let snapshotPlayers = [];
  let snapshotCrashAt = null;
  let snapshotGameId  = null;

  // --- Ð‘ÑƒÑ„ÐµÑ€Ð¸Ð·Ð¸Ñ€ÑƒÐµÐ¼ Â«Ð¿Ð¾ÑÐ»ÐµÐ´Ð½ÐµÐµ Ð°ÐºÑ‚Ð¸Ð²Ð½Ð¾ÐµÂ» ÑÐ¾ÑÑ‚Ð¾ÑÐ½Ð¸Ðµ, Ð¿Ð¾ÐºÐ° Ð¸Ð³Ñ€Ð° Ð¸Ð´Ñ‘Ñ‚ ---
  let lastActiveGameId  = null;
  let roundPlayersBuffer = [];
  $: if (getActiveGame) {
    // Ð’ÑÐµÐ³Ð´Ð° Ð´ÐµÑ€Ð¶Ð¸Ð¼ Ð¿Ð¾ÑÐ»ÐµÐ´Ð½Ð¸Ð¹ Ð°ÐºÑ‚ÑƒÐ°Ð»ÑŒÐ½Ñ‹Ð¹ ÑÐ¿Ð¸ÑÐ¾Ðº Ð¸Ð³Ñ€Ð¾ÐºÐ¾Ð² Ñ‚ÐµÐºÑƒÑ‰ÐµÐ³Ð¾ Ñ€Ð°ÑƒÐ½Ð´Ð°
  ...
  function captureRoundSnapshot(){
    // ÐµÑÐ»Ð¸ ÑÐµÑ€Ð²ÐµÑ€ ÑƒÐ¶Ðµ Ð¾Ð±Ð½ÑƒÐ»Ð¸Ð» currentGame.players â€” Ð±ÐµÑ€Ñ‘Ð¼ Ð¸Ð· Ð±ÑƒÑ„ÐµÑ€Ð°
    const source =
    ...
    // ID Ñ„Ð¸ÐºÑÐ¸Ñ€ÑƒÐµÐ¼ ÐºÐ°Ðº Ð˜Ð” Ð·Ð°Ð²ÐµÑ€ÑˆÐ¸Ð²ÑˆÐµÐ³Ð¾ÑÑ Ñ€Ð°ÑƒÐ½Ð´Ð° (Ð¿Ñ€ÐµÐ´Ð¿Ð¾Ñ‡Ñ‚Ð¸Ñ‚ÐµÐ»ÑŒÐ½Ð¾ lastActiveGameId)
  ...
  function beginCrashHold(){
    ...
    // Ð¡ÐÐÐŸÐ¨ÐžÐ¢ Ñ„Ð¸ÐºÑÐ¸Ñ€ÑƒÐµÐ¼ Ð”Ðž Ð·Ð°Ð¿ÑƒÑÐºÐ° hold (Ñ‡Ñ‚Ð¾Ð±Ñ‹ Ð½Ðµ Ð¿Ð¾Ñ‚ÐµÑ€ÑÑ‚ÑŒ Ð¸Ð³Ñ€Ð¾ÐºÐ¾Ð²)
  ...
  // Ð¿Ñ€Ð¸ ÑÑ‚Ð°Ñ€Ñ‚Ðµ Ð½Ð¾Ð²Ð¾Ð³Ð¾ Ñ€Ð°ÑƒÐ½Ð´Ð° ÑÐ±Ñ€Ð°ÑÑ‹Ð²Ð°ÐµÐ¼ hold Ð¸ ÑÐ½Ð¸Ð¼ÐºÐ¸
  $: if (getActiveGame){
    ...
    roundPlayersBuffer = []; // ÑÐ±Ñ€Ð¾Ñ Ð±ÑƒÑ„ÐµÑ€Ð° Ð½Ð° Ð½Ð¾Ð²Ñ‹Ð¹ Ñ€Ð°ÑƒÐ½Ð´
  }
...
let timerStartedAt = 0; // Ð´Ð¾Ð±Ð°Ð²ÑŒÑ‚Ðµ ÑÑ‚Ñƒ Ð¿ÐµÑ€ÐµÐ¼ÐµÐ½Ð½ÑƒÑŽ
let serverRemainingMs = 0; // Ð´Ð¾Ð±Ð°Ð²ÑŒÑ‚Ðµ ÑÑ‚Ñƒ Ð¿ÐµÑ€ÐµÐ¼ÐµÐ½Ð½ÑƒÑŽ
  ...
  // Ð˜Ð¡ÐŸÐ ÐÐ’Ð›Ð•ÐÐÐÐ¯ Ñ„ÑƒÐ½ÐºÑ†Ð¸Ñ updateCountdown
  function updateCountdown(){
  if (getActiveTimer){
    const now = Date.now();
    
    // Ð•ÑÐ»Ð¸ Ð¿Ð¾Ð»ÑƒÑ‡Ð¸Ð»Ð¸ Ð½Ð¾Ð²Ñ‹Ðµ Ð´Ð°Ð½Ð½Ñ‹Ðµ Ð¾Ñ‚ ÑÐµÑ€Ð²ÐµÑ€Ð°
    ...
    // Ð’Ñ‹Ñ‡Ð¸ÑÐ»ÑÐµÐ¼ Ñ‚ÐµÐºÑƒÑ‰ÐµÐµ Ð²Ñ€ÐµÐ¼Ñ Ð½Ð° Ð¾ÑÐ½Ð¾Ð²Ðµ ÑÐµÑ€Ð²ÐµÑ€Ð½Ñ‹Ñ… Ð´Ð°Ð½Ð½Ñ‹Ñ… + Ð»Ð¾ÐºÐ°Ð»ÑŒÐ½Ñ‹Ð¹ Ð¾Ñ‚ÑÑ‡ÐµÑ‚
    if (timerStartedAt > 0) {
      const elapsed = now - timerStartedAt;
      const remaining = Math.max(0, serverRemainingMs - elapsed);
      startTime = remaining / 1000;
    } else {
      // Fallback Ðº ÑÑ‚Ð°Ñ€Ð¾Ð¼Ñƒ Ð¼ÐµÑ‚Ð¾Ð´Ñƒ ÐµÑÐ»Ð¸ Ð½ÐµÑ‚ Ð´Ð°Ð½Ð½Ñ‹Ñ… Ð¾Ñ‚ ÑÐµÑ€Ð²ÐµÑ€Ð°
    ...
    // Ð¡Ð±Ñ€Ð¾Ñ Ð¿Ñ€Ð¸ Ð¾ÐºÐ¾Ð½Ñ‡Ð°Ð½Ð¸Ð¸ Ñ‚Ð°Ð¹Ð¼ÐµÑ€Ð°
    timerStartedAt = 0;
    serverRemainingMs = 0;
...
$: {
  ...
  if (need && !countdownHandle){ 
    updateCountdown(); 
    countdownHandle=setInterval(updateCountdown, 100); // Ð¸Ð·Ð¼ÐµÐ½Ð¸Ð» Ñ 50 Ð½Ð° 100
  }
  if (!need && countdownHandle){ 
  ...
  // Ð”Ð¾Ð¿Ð¾Ð»Ð½Ð¸Ñ‚ÐµÐ»ÑŒÐ½Ð¾Ðµ Ð¾Ñ‚ÑÐ»ÐµÐ¶Ð¸Ð²Ð°Ð½Ð¸Ðµ Ð¸Ð·Ð¼ÐµÐ½ÐµÐ½Ð¸Ð¹ Ð´Ð°Ð½Ð½Ñ‹Ñ… Ñ‚Ð°Ð¹Ð¼ÐµÑ€Ð°
$: if (data?.timer?.remainingMs !== undefined && getActiveTimer) {
  updateCountdown();
}

// Ð¡Ð±Ñ€Ð¾Ñ Ð¿Ñ€Ð¸ ÑÐ¼ÐµÐ½Ðµ ÑÐ¾ÑÑ‚Ð¾ÑÐ½Ð¸Ñ Ñ‚Ð°Ð¹Ð¼ÐµÑ€Ð°
$: if (!getActiveTimer) {
  timerStartedAt = 0;
  serverRemainingMs = 0;
}
  ...
  // Ð ÐµÐ¸Ð½Ð¸Ñ†Ð¸Ð°Ð»Ð¸Ð·Ð°Ñ†Ð¸Ñ Ð¿Ñ€Ð¸ ÑÐ¼ÐµÐ½Ðµ gameId / serverStartTime
  ...
  function openPage(i, gameId=null){
    ...
    // ÐµÑÐ»Ð¸ Ð±Ð°ÐºÑƒ Ð½ÑƒÐ¶Ð½Ð¾ â€” Ð¿ÑƒÑÑ‚ÑŒ ÑÐ»ÑƒÑˆÐ°ÐµÑ‚ ÑÑ‚Ð¾ ÑÐ¾Ð±Ñ‹Ñ‚Ð¸Ðµ Ð´Ð»Ñ Ð¿Ð¾Ð´Ð³Ñ€ÑƒÐ·ÐºÐ¸ Ð´Ð°Ð½Ð½Ñ‹Ñ…
  ...
  // --- Ð’Ð¸Ð´Ð¸Ð¼Ñ‹Ðµ Ð´Ð°Ð½Ð½Ñ‹Ðµ Ð´Ð»Ñ Ð¿Ñ€Ð°Ð²Ð¾Ð¹ ÐºÐ¾Ð»Ð¾Ð½ÐºÐ¸ ---
  // ÐŸÐ¾ÐºÐ°Ð·Ñ‹Ð²Ð°ÐµÐ¼ Ñ€ÐµÐ·ÑƒÐ»ÑŒÑ‚Ð°Ñ‚Ñ‹ Ð½Ðµ Ñ‚Ð¾Ð»ÑŒÐºÐ¾ Ð² hold, Ð½Ð¾ Ð¸ Ð¿Ð¾ÐºÐ° Ð¿Ð¾ÐºÐ°Ð·Ñ‹Ð²Ð°ÐµÑ‚ÑÑ Ð¾Ð¶Ð¸Ð´Ð°Ð½Ð¸Ðµ ÑÐ»ÐµÐ´ÑƒÑŽÑ‰ÐµÐ³Ð¾ Ñ€Ð°ÑƒÐ½Ð´Ð°.
...
{#if data?.active || getActiveTimer || getActiveTimer === null}
          ...
          <div class="game full-height column-block align-start justify-start">
            <div class="gameInfo row-block align-start justify-center" bind:this={gameInfo} style="position:relative;">
              ...
              {#if getActiveTimer || getActiveTimer === null || waitingVisible}
                ...
                <div class="waitGame column-block align-center justify-center full-width full-height">
                  <span class="title">ÐžÐ¶Ð¸Ð´Ð°Ð½Ð¸Ðµ Ñ€Ð°ÑƒÐ½Ð´Ð°</span>
                  <span class="sub-title">ÐÐ°Ñ‡Ð½Ñ‘Ñ‚ÑÑ Ñ‡ÐµÑ€ÐµÐ·</span>
              ...
              {:else}
                <!-- Ð Ð°ÑƒÐ½Ð´ / 5-ÑÐµÐºÑƒÐ½Ð´Ð½Ñ‹Ð¹ hold -->
                <div class="game-wrapper column-block align-center justify-start full-width full-height">
                  <div class="top column-block align-center justify-start">
                    ...
                    {#if holdCrashed}
                      <span class="text">Ð˜Ð³Ñ€Ð° Ð¾ÐºÐ¾Ð½Ñ‡ÐµÐ½Ð°</span>
                    {:else if betAlreadyExist}
                      <span class="text">Ð’Ð¾Ð·Ð¼Ð¾Ð¶Ð½Ñ‹Ð¹ Ð²Ñ‹Ð¸Ð³Ñ€Ñ‹Ñˆ</span>
                    {/if}
            ...
            <div class="placeBet column-block align-center justify-between">
              <div class="betWrapper full-width row-block align-start justify-between">
                <div class="bet">
                  <div class="title row-block align-center justify-start">Ð¡Ñ‚Ð°Ð²ÐºÐ°</div>
                ...
                <div class="winnig">
                  <div class="title row-block align-center justify-start">Ð’Ñ‹Ð¹Ð³Ñ€Ð°ÐµÑˆÑŒ</div>
              ...
              <div class="multiplier full-width column-block align-start justify-between">
                ...
                {#if getActiveTimer || getActiveTimer === null}
                  <button class="makeBet full-width {(placeBetAmount <= 0 || betAlreadyExist || getCurrentGamePlayers.length >= 999999) ? 'disabled' : ''}"
                    ...
                    on:click={makeBet}>Ð¡Ð´ÐµÐ»Ð°Ñ‚ÑŒ ÑÑ‚Ð°Ð²ÐºÑƒ</button>
                {:else}
                  <button class="makeBet take full-width{canTake ? '' : 'disabled'}" disabled={!canTake} on:click={take}>Ð—ÐÐ‘Ð ÐÐ¢Ð¬</button>
                {/if}
              </div>
            </div>
          </div>

          <!-- ÐŸÑ€Ð°Ð²Ñ‹Ð¹ ÑÐ¿Ð¸ÑÐ¾Ðº ÑÑ‚Ð°Ð²Ð¾Ðº -->
          <div class="bets full-height column-block align-start justify-start">
              ...
              <div class="row-block align-center justify-start">
                <div class="players row-block align-center justify-start">
                  <div class="players-info column-block align-start justify-between">
                    ...
                    <span class="text">Ð˜Ð³Ñ€Ð¾ÐºÐ¸</span>
                  </div>
                </div>
            ...
            <div class="betsWrapper full-height full-width column-block align-center justify-start {visiblePlayers.length <= 0 ? 'empty' : ''}">
              {#if visiblePlayers.length <= 0}
                <div class="emptyBets column-block align-center justify-center"><span class="text">ÐŸÐ¾ÐºÐ° Ð½ÐµÑ‚ ÑÑ‚Ð°Ð²Ð¾Ðº</span></div>
              {/if}
                ...
                <div class="betItem row-block align-center justify-between full-width
                  ...
                  {#if (showingHold && p?.exit != null) || (!showingHold && p?.exit != null && Number(p?.exit) <= crashValue)}
                    ...
                    <div class="winner full-height row-block align-center justify-start">
                      ...
                      <!-- Ñ„Ð¾Ð½ ÐºÑƒÐ±ÐºÐ¾Ð² (Ð¸Ð· Ð¼Ð°ÐºÐµÑ‚Ð°) -->
  ...
  <div class="historyItem full-width full-height column-block align-center justify-start">
    <div class="info row-block align-center justify-between full-width">
      <div class="row-block align-center justify-start">
        <span class="title">Ð˜Ð³Ñ€Ð°</span>
        <div class="gameID"><span>#{currentHistoryGame ? currentHistoryGame.id : (historyTargetId ?? 'â€”')}</span></div>
      </div>
      <div class="row-block align-center justify-start">
        <div class="players row-block align-center justify-start">
          <div class="players-info column-block align-start justify-between">
            <span class="count">{currentHistoryGame ? currentHistoryGame.playersCount : 'â€”'}</span>
            <span class="text">Ð˜Ð³Ñ€Ð¾ÐºÐ¸</span>
          </div>
        </div>
        <div class="bank row-block align-center justify-start">
          <div class="bank-info column-block align-start justify-between">
            <span class="count row-block align-center justify-start">
              {currentHistoryGame ? currentHistoryGame.bank : 'â€”'} <img class="coins" alt="" src={`${cdn}/img/donate/mcoin.svg`} />
            </span>
            <span class="text">Ð‘Ð°Ð½Ðº</span>
          </div>
        </div>
    ...
    <div class="betsWrapper full-height full-width column-block align-center justify-start {( !currentHistoryGame || currentHistoryBets.length === 0 ) ? 'empty' : ''}">
      {#if !currentHistoryGame}
        <!-- Ð´Ð°Ð½Ð½Ñ‹Ñ… ÐµÑ‰Ñ‘ Ð½ÐµÑ‚ â€” Ð½Ð¾ ÑÑ‚Ñ€Ð°Ð½Ð¸Ñ†Ð° Ð½Ðµ Ð·Ð°ÐºÑ€Ñ‹Ð²Ð°ÐµÑ‚ÑÑ -->
        <div class="emptyBets column-block align-center justify-center">
          <span class="text">ÐŸÐ¾ÐºÐ° Ð½ÐµÑ‚ Ð´Ð°Ð½Ð½Ñ‹Ñ…â€¦</span>
        </div>
      {:else if currentHistoryBets.length === 0}
        <div class="emptyBets column-block align-center justify-center">
          <span class="text">ÐŸÐ¾ÐºÐ° Ð½ÐµÑ‚ ÑÑ‚Ð°Ð²Ð¾Ðº</span>
        </div>
      {/if}
      ...
      {#if currentHistoryGame}
        {#each currentHistoryBets as b}
          <div class="betItem row-block align-center justify-between full-width {b?.exitX != null ? 'winnerBack' : ''} {b?.exitX == null ? 'crashed' : ''}">
            ...
            {#if b?.exitX == null}
              <div class="inGame crashed"><span>ÐšÑ€Ð°Ñˆ</span></div>
...
<style>
  /* Ð¤Ð¸ÐºÑ ÑÐ»Ð¾Ñ Ñ„Ð¾Ð½Ð° Ð¿Ð¾Ð±ÐµÐ´Ð¸Ñ‚ÐµÐ»Ñ, Ñ‡Ñ‚Ð¾Ð±Ñ‹ Ð½Ðµ Ð·Ð°ÐºÑ€Ñ‹Ð²Ð°Ð» ÐºÐ¾Ð½Ñ‚ÐµÐ½Ñ‚ */
