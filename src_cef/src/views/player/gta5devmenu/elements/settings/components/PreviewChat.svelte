<script>
  ...
  // ← ДОБАВЬ THROTTLE ДЛЯ ТЯЖЕЛЫХ ВЫЧИСЛЕНИЙ
  let updateTimeout;
  let cachedSettings = {};
  ...
  function throttledUpdate(key, value) {
    ...
    updateTimeout = setTimeout(() => {
      // Применяем изменения пачкой
      Object.assign(cachedSettings, cachedSettings);
    }, 50); // 50мс throttle
  }
  ...
  $: cssVars = {
    ...
    "--1ffd6928": `rgba(26, 26, 26, ${chatFontShadowAlpha / 100})`
  };
