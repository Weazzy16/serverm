    ...
    <div class="popup__newhud_box">
        ...
        <div class="popup__select_elements">
            {#each popupData.list as item, index}
                <div class="popup__select_element" class:active={listId === index} on:keypress={() => {}} on:click={() => onSelected (index)}>
                    {item.name}
                </div>
            {/each}
        </div>
        ...
        <div class="popup__newhud__buttons">
            <div class="popup__newhud_button" in:fly={{ y: 50, duration: 350 }} on:keypress={() => {}} on:click={onSend}>{popupData.button}</div>
