...
{#if selectItem}
    <div class="popup__newhud">
        <div class="popup__newhud_box">
            ...
            <div class="popup__newhud_text">You really want to buy '{selectItem.name}' for {!selectItem.isRb ? `$${format("money", selectItem.price)}` : `${format("money", selectItem.price)} RB`}?</div>

            <div class="popup__newhud__buttons">
                <div class="popup__newhud_button" on:keypress={() => {}} on:click={onBuy}>{translateText('popups', 'Купить')}</div>
                <div class="popup__newhud_button" on:keypress={() => {}} on:click={onClose}>{translateText('popups', 'Отмена')}</div>
            </div>
        </div>
    </div>
{:else}
                ...
                {#each upgrate as item, index}
                    <div class="popup__grid_element">
                        ...
                        <div class="box-between w-100">
                            ...
                            {/if}
                            <div class="popup__grid_button" on:keypress={() => {}} on:click={() => onSelectItem (item)}>{translateText('popups', 'Купить')}</div>
                        </div>
                    </div>
                {/each}
            </div>
            <div class="popup__newhud__buttons">
                <div class="popup__newhud_button" on:keypress={() => {}} on:click={() => setPopup ()}>{translateText('popups', 'Закрыть')}</div>
            </div>
        </div>
    </div>
{/if}