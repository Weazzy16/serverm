...
<div class="newphone__messages_bottom newphone__project_padding16" in:fade>
    ...
    {/if}
    <input type="text" class="newphone__messages_input" placeholder={`Schreiben ${contactData.Name}`} bind:value={inputValue} maxlength={inputMaxLength} bind:this={inputDiv} on:focus={onFocus} on:blur={onBlur} on:input={onInput} />
    {#if !contactData.IsSystem}
