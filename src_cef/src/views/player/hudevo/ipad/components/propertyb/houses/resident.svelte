...
<div class="newtablet__rent_list">
    ...
    <div class="newtablet__rent_none hover" on:click={() => onSelectedName(name)}>
        ...
        <div class="newtablet__rent_noneimage person"></div>
    </div>
    {/each}
...
<div class="newtablet__rent_list">
    ...
    {#if isConfirmed === false}
        <div class="newtablet__project_button" on:click={onSelectResident}>{translateText('player2', 'Выселить')}</div>
    {:else}
        <div class="newtablet__ads_title newtablet__project_padding20">{translateText('player2', 'Вы действительно хотите выселить')} {selectedName.replace(/_/g, ' ')}?</div>
        <div class="newtablet__project_button property mb-6" on:click={onEnter}>{translateText('player2', 'Я уверен')}</div>
        <div class="newtablet__project_button property mb-0" on:click={() => isConfirmed = false}>{translateText('player2', 'Я не уверен')}</div>
    {/if}
    <div class="box-between w-100 mb-10">
        <div>{translateText('player2', 'Доступ к мебели')}</div>
        <div class="newtablet__checkbox" on:click={() => onResidentAccess (selectedName, "isFurniture")}>
    ...
    <div class="box-between w-100 mb-10">
        ...
        <div class="newtablet__checkbox" on:click={() => onResidentAccess (selectedName, "isPark")}>
