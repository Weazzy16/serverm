<script>
    ...
    updateData ();
    addListernEvent ("tabletHouseUpdate", updateData);
...
{#if selectedFurniture === null}
    <div class="newtablet__rent_list">
        {#if houseFurnitures && typeof houseFurnitures === "object" && houseFurnitures.length > 0}
            ...
            <div class="newtablet__rent_none hover" on:click={() => onSelectedFurniture (index)}>
                ...
                <div class="newtablet__rent_noneimage heal" style="background-image: url({document.cloud + `inventoryItems/furniture/${furniture.Model}.png`});"></div>
            </div>
            {/each}
        {:else}
            <div class="newtablet__rent_none">
                <div class="box-column">
                ...
                <div class="newtablet__rent_noneimage"></div>
            </div>
            <div class="newtablet__project_button property" on:click={setPoint}>{translateText('player2', 'Как добраться')}?</div>
            <div class="newtablet__project_button property" on:click={()=> currentPage.set("taxi")}>{translateText('player2', 'Вызвать такси')}</div>
        {/if}
...
{:else}
    <div class="newtablet__rent_list">
        <div class="newtablet__rent_none hover">
            ...
            <div class="newtablet__rent_noneimage heal" style="background-image: url({document.cloud + `inventoryItems/furniture/${houseFurnitures [selectedFurniture].Model}.png`});"></div>
        </div>

        <div class="newtablet__project_button" on:click={() => onFurnitureBuy (1)} class:auction={!houseFurnitures [selectedFurniture].IsSet} class:property={houseFurnitures [selectedFurniture].IsSet}>{!houseFurnitures [selectedFurniture].IsSet ? translateText('player2', 'Установить') : translateText('player2', 'Убрать')}</div>
        <div class="newtablet__project_button" on:click={() => onFurnitureBuy (0)}>{translateText('player2', 'Продать')}</div>
