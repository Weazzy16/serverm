<script>
    ...
    const onDonate = () => {
        ...
        if (!selectType) {
            ...
            window.notificationAdd(4, 9, `Вы начали процесс покупки Battle Pass. Совершите донат через личный кабинет в размере 1999 руб, и премиум-доступ автоматически станет доступен.`, 25000);
    ...
    <div class="donatepopup__payments">
        <div class="box-between">
            <div class="box-column">
                ...
                <div class="donatepopup__small">{!selectType ? "Купить за AstraCoin" : "Купить за рубли"}</div>
                <div class="donatepopup__count">{popupData.data[selectType]}<span class="red"> {!selectType ? "AS" : "руб."}</span></div>
        ...
        <div class="donatepopup__grid">
            <div class="donatepopup__element" class:active={selectType === 0} on:click={() => selectType = 0}>
                <div class="donatepopup__element_img" style="background-image: url('http://cdn.piecerp.ru/cloud/img/roulette/items_5.png')"></div>
            </div>
            <div class="donatepopup__element" class:active={selectType === 1} on:click={() => selectType = 1}>
                ...
                <div class="donatepopup__element_img" style="background-image: url('http://cdn.piecerp.ru/cloud/img/roulette/items_0.png')"></div>
            </div>
        </div>
        ...
        {#if selectType === 1}
            {translateText('popups', 'После нажатия кнопки "Подтвердить" у вас есть 5 минут, чтобы совершить платёж.')}<br/><br/>{translateText('popups', 'Если выйти из игры до завершения транзакции, то на аккаунт будут зачислены AstraCoin по текущему курсу.')}
        {/if}
        </div>
    </div>
