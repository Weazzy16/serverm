    ...
    class Manager : Script
    {
        ...
        public static void onResourceStart()
        {
            try
            {
                ...
                foreach (DataRow Row in result.Rows)
                {
                    ...
                    int houseId = Convert.ToInt32(Row["houseid"]);
        ...
        public static bool UpdateCrimeOptions(ExtPlayer player) 
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "У Вас не хватает Majestic Coins для улучшения семьи.", 3000);
                    return false;
                }
        ...
        public static List<ExtPlayer> GetMembersInOrganization(int id)
        {
            var organizationData = GetOrganizationData(id);
            if (organizationData is null) 
                return new List<ExtPlayer>();

            return Character.Repository.GetPlayers().Where(x => x.OrganizationData != null && x.OrganizationData.Id == organizationData.Id).ToList();
        }

        public static void SendMessageToAllPlayers(int id, string message, bool isNotify = false, NotifyType notifyType = NotifyType.Success, int time = 3000)
        {
            try
            {
                var organizationData = GetOrganizationData(id);
                ...
                foreach (var foreachPlayer in GetMembersInOrganization(organizationData.Id))
                {
                    if (!isNotify)
                    {
                    ...
                    {
                        Notify.Send(foreachPlayer, notifyType, NotifyPosition.BottomCenter, message, time);
                    }
                }
            }
            catch (Exception ex) { Log.Write("SendMessageToAllPlayers: " + ex.ToString()); }
        }
        ...
        public static void CraftBodyArmor(ExtPlayer player)
        {
            try
            {
                var characterData = player.GetCharacterData();
                if (characterData == null) return;
                if (characterData.InsideOrganizationID == -1 && characterData.InsideHouseID == -1) return;
            ...
            try
            {
                ...
                switch (listItem)
                {
                    ...
                    case "createorg":
                        ...
                        Trigger.ClientEvent(player, "openDialog", "CreateOrg", $"Вы уверены, что хотите создать свою семью? Названием семьи служит Ваша фамилия, при создании за сумму {Main.PricesSettings.CreateOrgPrice}$ Вы получаете гараж на 5 т.с и офис начального уровня, а так же семейный чат (/c /cb).");
                        break;
                }
            }
            catch (Exception e)
        ...
        public static void RemovePlayerFromGarage(ExtPlayer player)
        {
            try
            {
                ...
                var orgData = player.GetOrganizationData();
                if (orgData is null) return;

                if (player.HasData("_garage.isHouse"))
                {
                    var house = HouseManager.Houses.Find(x => x.ID == orgData.HouseId);
                    if (house is null) return;

                    var garage = house.GetGarageData();
                    if (garage is null) return;

                    player.Position = garage.Position + new Vector3(0, 0, 1.2);
                    player.Dimension = 0;

                    player.ResetData("_garage.isHouse");
                }
        ...
        public static void organizationChatOOC(ExtPlayer player, string message)
        {
            try
            {
                ...
                GameLog.AddInfo($"(CBChat({organizationData.Id})) player({characterData.UUID}) {message}");
            }
            catch (Exception e)
            {
        ...
        public static OrganizationData GetOrganizationDataByHouseId(int houseId) => Organizations.Values.ToList().Find(x => x.HouseId == houseId);
        public static bool IsMansion(int houseId) => GetOrganizationDataByHouseId(houseId) != null;

        public static void SendOrgMessage(int orgId, string message)
        {
            try
            {

                string msgSender = "!{#50C878}" + $"[ORG] {message}";
                foreach (ExtPlayer foreachPlayer in Character.Repository.GetPlayers())
                {
                    var foreachMemberOrganizationData = foreachPlayer.GetOrganizationMemberData();
                    if (foreachMemberOrganizationData == null)
            ...
            catch (Exception ex) { Log.Write("SendOrgMessage: " + ex.ToString()); }
        }
        ...
        public static void GetVehicleFromGarage(ExtPlayer player, ExtVehicle vehicle, int orgId)
        {
            try
            {
                ...
                var position = OutsideGarage[Step] + new Vector3(0, 0, 0.3);
                if (player.HasData("_garage.isHouse"))
                {
                    var house = Houses.HouseManager.Houses.Find(x => x.ID == organizationData.HouseId);
                    if (house != null)
                    {
                        var garage = house.GetGarageData();
                        if (garage != null)
