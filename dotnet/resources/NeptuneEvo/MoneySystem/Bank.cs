...
namespace NeptuneEvo.MoneySystem
{
    class Bank : Script
    {
        ...
        public static async Task Save(ServerBD db, int bankId)
        {
            try
            {
                ...
                // Убедитесь, что Balance поддерживает большие значения (long/bigint)
                var updateBank = db.Money
                    .Where(m => m.Id == bankId)
                    .Set(m => m.Balance, Convert.ToInt64(bank.Balance)); // Преобразование к long

                // Если есть необходимость сохранить Holder
                ...
                await updateBank.UpdateAsync();
            }
            catch (Exception e)
            {
                // Логируем все исключения для диагностики
    ...
    class ATM : Script
    {
        ...
        public static void OnInteraction(ExtPlayer player, int id)
        {
            try
            {
                var characterData = player.GetCharacterData();
                if (characterData is null || characterData.DemorganTime >= 1)
                    return;
                ...
                Trigger.ClientEvent(player, "client.atm.open", id);
                BattlePass.Repository.UpdateReward(player, 39);
            }
            catch (Exception ex) { Bank.Log.Write("OnInteraction:" + ex.ToString()); }
        }


        public static int GetMaxDays(ExtPlayer player)
        {
            var accountData = player.GetAccountData();
            if (accountData is null) return 7;
        ...
        [RemoteEvent("server.atm.change_page")]
        public static void On_ChangePage(ExtPlayer player, string type)
        {
            try
            {
                var accountData = player.GetAccountData();
                ...
                switch (type) 
                {
                    case "house":
                        ...
                        var balance = Bank.GetBalance(house.BankID);                
                        var maxBalance = Convert.ToInt32(house.Price / 100 * 0.026 * 24 * GetMaxDays(player));

                        Trigger.ClientEvent(player, "client.atm.set_transaction", type, JsonConvert.SerializeObject(new
                        {
                        ...
                        break;
                    case "business":
                        var businessesIds = characterData.BizIDs;
                        if (businessesIds.Count <= 0 || !BusinessManager.BizList.TryGetValue(businessesIds[0], out var business))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"У вас нет Бизнеса", 3000);
                            return;
                        }

                        balance = Bank.GetBalance(business.BankID);
                        maxBalance = Convert.ToInt32(business.SellPrice / 100 * business.Tax * 24 * GetMaxDays(player));
            ...
            catch(Exception ex) { Bank.Log.Write("On_ChangePage: " + ex.ToString()); }
        }

        [RemoteEvent("server.atm.action")]
        public static void On_AtmAction(ExtPlayer player, params object[] objects)
        {
            try
            {
                var characterData = player.GetCharacterData();
                ...
                var type = objects[0].ToString();
                int amount = type == "transfer" ? Convert.ToInt32(objects[2]) : Convert.ToInt32(objects[1]);
                if (amount < 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Сумма не может быть меньше 1$", 3000);
                    return;
                }
                ...
                {
                    case "replenishment":
                        if (!Wallet.Change(player, -amount))
                        {
                        ...
                        if (characterData.AdminLVL <= 7)
                            GameLog.Money($"player({characterData.UUID})", $"bank({characterData.Bank})", amount, $"atmIn");

                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы пополнили банковский счет на {MoneySystem.Wallet.Format(amount)}$", 3000);
                        break;
                    case "withdraw":
                        ...
                        if (characterData.AdminLVL <= 7) 
                            GameLog.Money($"bank({characterData.Bank})", $"player({characterData.UUID})", amount, $"atmOut");

                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы сняли {MoneySystem.Wallet.Format(amount)}$ со своего банковского счета", 3000);
                        break;
                    case "transfer":
                        ...
                        int targetBankId = Convert.ToInt32(objects[1]);
                        if (!Bank.Accounts.TryGetValue(targetBankId, out var targetBank) || targetBank.Type != 1)
                        {
                        ...
                        Bank.Transfer(characterData.Bank, targetBankId, amount);
                    ...
                    case "house":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы не можете пополнить счет на эту сумму", 3000);
                            return;
                        }
                        ...
                        Bank.Change(house.BankID, +amount);
                        GameLog.Money($"player({characterData.UUID})", $"bank({house.BankID})", amount, $"atmHouse");

                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank, LangFunc.GetText(LangType.Ru, DataName.SucTransactH, amount, (balance + amount), maxBalance), DateTime.Now);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы оплатили налог за дом в размере {MoneySystem.Wallet.Format(amount)}$", 3000);
                        break;
                    case "business":
                        var businessesIds = characterData.BizIDs;
                        if (businessesIds.Count <= 0 || !BusinessManager.BizList.TryGetValue(businessesIds[0], out var business))
                        ...
                        Bank.Change(business.BankID, +amount);
                        GameLog.Money($"player({characterData.UUID})", $"bank({business.BankID})", amount, $"atmBiz");

                        Players.Phone.Messages.Repository.AddSystemMessage(player, (int)DefaultNumber.Bank, LangFunc.GetText(LangType.Ru, DataName.SucTransactB, amount, (balance + amount), maxBalance), DateTime.Now);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы оплатили налог за бизнес в размере {MoneySystem.Wallet.Format(amount)}$", 3000);
                        break;
                }

                Trigger.ClientEvent(player, "client.atm.set_page", "main");
            }
            catch(Exception ex) { Bank.Log.Write("On_AtmAction: " + ex.ToString()); }
        }
