...
namespace NeptuneEvo.Houses
{
    ...
    public class HouseType
    {
        ...
        public Vector3 CraftPosition { get; set; }
        public Vector3 StoragePosition { get; set; }

        public HouseType(string name, Vector3 position, Vector3 petpos, float rotation, string ipl = "", Vector3 storage = null, Vector3 craft = null)
        {
            Name = name;
            Position = position;
            IPL = ipl;
    ...
    public class House
    {
        ...
        private ExtColShape _storageColShape { get; set; }
        private Marker _storageMarker { get; set; }
        private TextLabel _storageTextLabel { get; set; }
        private ExtColShape _craftColShape { get; set; }
        private Marker _craftMarker { get; set; }
        private TextLabel _craftTextLabel { get; set; }

        private Blip _organizationBlip { get; set; }

        public void CreateOrganizationPlaces()
        {
            try
            {
                var organizationData = Organizations.Manager.GetOrganizationDataByHouseId(ID);
                if (organizationData is null)
                {
                    DestroyOrganizationPlaces();
                    return;
                }

                _organizationBlip = NAPI.Blip.CreateBlip(181, Position, 1f, 4, organizationData.Name, 255, 0, true, 0, 0);

                var interiorData = HouseManager.HouseTypeList[Type];
                if (interiorData is null || interiorData.CraftPosition is null || interiorData.StoragePosition is null) return;

                _storageColShape = CustomColShape.CreateCylinderColShape(interiorData.StoragePosition, 2, 2, (uint)Dimension, ColShapeEnums.Organizations, 2);
                _storageMarker = NAPI.Marker.CreateMarker(MarkerType.VerticalCylinder, interiorData.StoragePosition, new Vector3(), new Vector3(), .8f, new Color(252, 3, 82, 200), false, (uint)Dimension);
                _storageTextLabel = NAPI.TextLabel.CreateTextLabel("Склад", interiorData.StoragePosition + new Vector3(0, 0, 1), 10f, 5f, 4, new Color(255, 255, 255), false, (uint)Dimension);

                _craftColShape = CustomColShape.CreateCylinderColShape(interiorData.CraftPosition, 2, 2, (uint)Dimension, ColShapeEnums.Organizations, 1);
                _craftMarker = NAPI.Marker.CreateMarker(MarkerType.VerticalCylinder, interiorData.CraftPosition, new Vector3(), new Vector3(), .8f, new Color(252, 3, 82, 200), false, (uint)Dimension);
                _craftTextLabel = NAPI.TextLabel.CreateTextLabel("Крафт", interiorData.CraftPosition + new Vector3(0, 0, 1), 10f, 5f, 4, new Color(255, 255, 255), false, (uint)Dimension);
            }
            catch (Exception ex) { Log.Write("CreateOrganizationPlaces: " + ex.ToString()); }
        }

        public void DestroyOrganizationPlaces()
        {
            try
            {
                if (_storageColShape != null)
                {
                    CustomColShape.DeleteColShape(_storageColShape);
                    _storageColShape = null;
                }

                if (_storageMarker != null)
                {
                    _storageMarker.Delete();
                    _storageMarker = null;
                }

                if (_craftColShape != null)
                {
                    CustomColShape.DeleteColShape(_craftColShape);
                    _craftColShape = null;
                }

                if (_craftMarker != null)
                {
                    _craftMarker.Delete();
                    _craftMarker = null;
                }

                if (_organizationBlip != null)
                {
                    _organizationBlip.Delete();
                    _organizationBlip = null;
                }
            }
            catch (Exception ex) { Log.Write("DestroyOrganizationPlaces: " + ex.ToString()); }
        }
            ...
            try
            {
                if (Type != 7)
                {
                    if (Healkit)
                    {
                        ...
                        //Healkitmarker = (ExtMarker) NAPI.Marker.CreateMarker(1, HouseManager.HouseHealkitPos[Type], new Vector3(0, 0, 0), new Vector3(0, 0, 0), 1, new Color(255, 255, 255, 220), false, (uint)Dimension);
                        ...
                        //Healkitshape = CustomColShape.CreateCylinderColShape(HouseManager.HouseHealkitPos[Type], 1, 2, (uint)Dimension, ColShapeEnums.HealkitHouse);
                    }
                    else
                    {
        ...
        public void UpdateLabel()
        {
            Trigger.SetMainTask(() =>
            {
                try
                {
                    ...
                    {
                        if (!Organizations.Manager.IsMansion(ID))
                            text += $"~p~{Owner}\n";
                        else
                        {
                            var organizationId = Organizations.Manager.GetOrganizationDataByHouseId(ID);
                            if (organizationId != null)
                                text += $"~p~{organizationId.Name}\n";
                        }
        ...
        public void ClearOwner(bool isClearUpgraded = true, bool isSave = true)
        {
            Trigger.SetMainTask(() =>
            {
                try
                {
                    ...
                    {
                        organizationData.HouseId = -1;
                        Organizations.Manager.SendOrgMessage(organizationData.Id, $"У вашей организации теперь нет особняка!");
                    }
    ...
    class HouseManager : Script
    {
        ...
        public static List<HouseType> HouseTypeList = new List<HouseType>
        {
            ...
            new HouseType("Особняк", new Vector3(373.53, 423.47, 145.05), new Vector3(373.5995, 404.09, 145.407), 234.0f, "",
                storage: new Vector3(378.79465, 429.9608, 137.18016), craft: new Vector3(373.27136, 431.06036, 137.18016)),
        };
        ...
        public static List<int> MaxRoommates = new List<int>() { 1, 2, 3, 4, 5, 6, 7, 0, 15, 30, 30, 10 };
        ...
        public static void OnEnterHouse(ExtPlayer player, int index)
        {
            ...
            if (house == null) return;
            if (house.Type != 7)
                if (Events.Christmas.ChristmasManager.CanTakeCandy(player, house.ID)) return;
            ...
            else
            {
                if (house.Type != 7)
                {
                    if (house.Locked)
                    {
                        ...
                        var playerOrgData = player.GetOrganizationData();
                        var houseOrganizationData = Organizations.Manager.GetOrganizationDataByHouseId(house.ID);

                        var playerHouse = GetHouse(player);
                        if (playerHouse != null && playerHouse.ID == house.ID) house.SendPlayer(player);
                        else if (houseOrganizationData != null && playerOrgData == houseOrganizationData) house.SendPlayer(player);
            ...
            try
            {
                ...
                switch (action)
                {
                    case "buy":
                        ...
                        if (house.Price > Convert.ToInt64(characterData.Money))
                        {
                       ...
                       // Notify.Send(player, NotifyType.Success, NotifyPosition.Center, $"НЕ ЗАБУДЬТЕ ВНЕСТИ НАЛОГИ В БЛИЖАЙШЕМ БАНКОМАТЕ!", 8000);
                        ...
                        var interiorData = HouseManager.HouseTypeList[house.Type];
                        if (interiorData != null && interiorData.CraftPosition != null && interiorData.StoragePosition != null)
                        {
                            var playerOrgData = player.GetOrganizationData();
                            if (playerOrgData is null) return;

                            if (playerOrgData.IsLeader(player.GetUUID()))
                            {
                                playerOrgData.HouseId = house.ID;
                                Organizations.Manager.SendOrgMessage(playerOrgData.Id, $"Ваш лидер приобрел новый особняк!");
                            }
                        }
            ...
            try
            {
                ...
                if (garage != null && garage.Type != 6)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "У Вас недостаточно Majestic Coins.", 3000);
                            return;
                        }
            ...
            try
            {
                ...
                switch (action)
                {
                    ...
                    case "spawn1":
                        ...
                        int nearestServiceIndex = AleSystems.VehicleSpawn.GetNearestAutoService(player.Position);

                        if (player.Position.DistanceTo2D(AleSystems.VehicleSpawn.AutoServicePositions[nearestServiceIndex - 1]) > 5f)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы можете починить авто рядом с NPC", 3000);
                            return;
                        }
                        ...
                        if (playerData.Money < spawnCost)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Недостаточно средств для спавна транспорта (не хватает {spawnCost - playerData.Money}$)", 3000);
                            return;
                        }

                        // Списываем деньги
                        MoneySystem.Wallet.Change(player, -spawnCost);
                        GameLog.Money($"player({playerData.UUID})", "server", spawnCost, "spawn_vehicle");

                        var vehiclesNumber = VehicleManager.GetVehiclesAirNumberToPlayer(player.Name);
                        var positionData = AleSystems.VehicleSpawn.GetSpawnPosition(nearestServiceIndex);
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Ваш транспорт ожидает вас. Списано {spawnCost}$ за спавн.", 3000);
                        return;
                    ...
                    case "evac1":
                        ...
                        if (isAir)
                        {
                            int serviceIndex = AleSystems.VehicleSpawn.GetNearestAutoService(player.Position);

                            if (player.Position.DistanceTo2D(AleSystems.VehicleSpawn.AutoServicePositions[serviceIndex - 1]) > 5f)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы можете починить авто рядом с NPC", 3000);
                                return;
                            }
                        ...
                        if (isAir)
                        {
                            ...
                            vehiclePlayer = VehicleStreaming.CreateVehicle(NAPI.Util.GetHashKey(vehicleData.Model), positionData.Item1, positionData.Item2.Z, 0, 0, number, petrol: vehicleData.Fuel, acc: VehicleAccess.Personal, locked: true, dirt: vehicleData.Dirt);
                            VehicleManager.ApplyCustomization(vehiclePlayer);
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Ваш транспорт эвакуирован", 3000);
                        }
                        else
                        {
                    ...
                    case "evac":
                        ...
                        if (Convert.ToInt64(characterData.Money) < Main.EvacCar)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Недостаточно средств (не хватает {Main.EvacCar - Convert.ToInt64(characterData.Money)}$)", 3000);
                            return;
                        }
