...
namespace NeptuneEvo.Houses
{
    ...
    class GarageManager : Script
    {
        ...
        public static void OnEnterGarage(ExtPlayer player, int index)
        {
            ...
            var house = HouseManager.Houses.Find(x => x.GarageID == index);
            if (house is null) return;

            var playerHouse = HouseManager.GetHouse(player);

            var playerOrgData = player.GetOrganizationData();
            var orgData = Organizations.Manager.GetOrganizationDataByHouseId(house.ID);

            bool canEnter = false;
            if (orgData != null)
            {
                if (playerOrgData?.Id == orgData.Id) canEnter = true;
            }

            if (!canEnter)
                if (playerHouse == house) canEnter = true;

            if (!canEnter) return;

            var garage = house?.GetGarageData();
            if (garage is null || house.GarageID != garage.Id || house.GarageID != index)
                return;
            ...
            {
                player.SetData("_garage.id", index);

                var frameList = new FrameListData();
                frameList.Header = "Гараж";
                frameList.Callback = CallbackGarage;

                if (house == playerHouse)
                    frameList.List.Add(new ListData("Гараж дома", "hGarage"));

                if (orgData != null && playerOrgData?.Id == orgData.Id)
                    frameList.List.Add(new ListData("Гараж организациия", "oGarage"));
            ...
            if (player.IsInVehicle)
            {
                ...
                switch (vehicleLocalData.Access)
                {
                    case VehicleAccess.Organization:
                        if (vehicleLocalData.Fraction != orgData.Id) return;

                        Trigger.ClientEvent(player, "garageload");
                        player.Position = new Vector3(-1396.274, -480.7186, 57.100);
                        Trigger.Dimension(player, playerOrgData.GetDimension());
                        characterData.InsideOrganizationID = playerOrgData.Id;
                        ...
                        var organizationVehicles = playerOrgData.Vehicles[vehicleLocalData.NumberPlate];
                        var veh = VehicleStreaming.CreateVehicle(NAPI.Util.GetHashKey(organizationVehicles.model), Organizations.Manager.GaragePositions[organizationVehicles.garageId], Organizations.Manager.GarageRotations[organizationVehicles.garageId], 0, 0, vehicleLocalData.NumberPlate, dimension: playerOrgData.GetDimension(), locked: true, acc: VehicleAccess.OrganizationGarage, fr: playerOrgData.Id, minrank: organizationVehicles.rank, petrol: petrol, dirt: dirt);
                    ...
                    case VehicleAccess.Personal:
                        ...
                        vehicleStateData = vehicle.GetVehicleLocalStateData();
                        if (vehicleStateData != null)
                            vehicleData.Dirt = vehicleStateData.Dirt;
        ...
        private static void CallbackGarage(ExtPlayer player, object _item)
        {
            try
            {
                var sessionData = player.GetSessionData();
                ...
                if (!player.HasData("_garage.id")) return;

                int garageId = player.GetData<int>("_garage.id");

                var house = HouseManager.Houses.Find(x => x.GarageID == garageId);
                if (house is null) return;

                var garage = house?.GetGarageData();
                if (garage is null || house.GarageID != garage.Id || house.GarageID != garageId)
                    return;

                string callback = Convert.ToString(_item);
                ...
                switch (callback)
                {
                    case "hGarage":
                        ...
                        return;
                    case "oGarage":
                        player.SetData("_garage.isHouse", true);
                        Organizations.Manager.SendPlayerToGarage(player);
                        return;
                }
            }
            catch (Exception ex) { Log.Write("CallbackGarage: " + ex.ToString()); }
        }
