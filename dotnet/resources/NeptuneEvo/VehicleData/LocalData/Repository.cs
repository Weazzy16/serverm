...
namespace NeptuneEvo.VehicleData.LocalData
{
    public static class Repository
    {
        ...
        public static VehicleLocalData GetVehicleLocalData(this ExtVehicle vehicle)
        {
            ...
            if (vehicle.VehicleLocalData == null)
            {
                // подтягиваем in-memory объект, загруженный из БД
                var data = VehicleManager.GetVehicleToNumber(vehicle.NumberPlate);

                vehicle.VehicleLocalData = new VehicleLocalData
                {
                    Position = $"{vehicle.Position.X};{vehicle.Position.Y};{vehicle.Position.Z}",
                    EngineOn = false,
                    Centimeters = 0,
                    Mileage = data?.Mileage ?? 0        // ← теперь стартуем с прошлым пробегом
                };
            }
            return vehicle.VehicleLocalData;
        }
