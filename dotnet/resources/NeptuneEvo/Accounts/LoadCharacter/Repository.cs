                ...
                foreach (var character in characters)
                {
                    try
                    {
                        if (accountData.Chars.Contains(character.Uuid))
                        {
                            ...
                            string typeDataName = character.Adminlvl > 0 ? "Администратор" : "Игрок";
                            playerData.Add("TypeData", typeDataName);
                            ...
                            var lastPosition = JsonConvert.DeserializeObject<Vector3>(character.Pos);
                            ...
                            if (memberFractionData != null && memberFractionData.Id > 0)
                            {
                                spawnData.Add("organization", new
                                {
                                    Position = Fractions.Manager.FractionSpawns[memberFractionData.Id],
                                    Name = "Фракция"
                                });
                            }

                            if (memberOrganizationData != null && memberOrganizationData.Id > 0)
                            {
                                var organizationData = Organizations.Manager.GetOrganizationData(memberOrganizationData.Id);
                                if (organizationData != null)
                                {
                                    var pos = new Vector3(-774.045, 311.2569, 85.70606);
                                    if (organizationData.BlipId != -1)
                                        pos = organizationData.BlipPosition;

                                    spawnData.Add("family", new
                                    {
                                        Position = pos,
                                        Name = "Семья"
                                    });
                            ...
                            playerData.Add("Spawn", spawnData);

                            charsData.Add(character.Uuid, playerData);

                            if (isReg)
                            {
                                player.TriggerEvent("startWelcomeCutscene", character.Gender, $"{character.Firstname} {character.Lastname}");
                            }
                        }
