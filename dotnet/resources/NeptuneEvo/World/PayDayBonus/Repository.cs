             ...
             public static void Bonus()
             {
                 ...
                 if (players.Count > 0)
                 {
                     ...
                     foreach (var foreachPlayer in winnersPlayer)
                     {
                         ...
                         Trigger.ClientEvent(foreachPlayer, "hud.info", "Удача на твоей стороне!", $"Поздравляем с победой в розыгрыше! {Main.DonateSettings.HappyHoursRB} AstraCoins уже зачислены на твой аккаунт.", "СЧАСТЛИВЫЕ ЧАСЫ", "http://cdn.piecerp.ru/cloud/img/time.png");

                         winersName += $" {foreachSessionData.Name}";
                     }

                     NAPI.Chat.SendChatMessageToAll($"~o~[СЧАСТЛИВЫЕ ЧАСЫ] Розыгрыш {Main.DonateSettings.HappyHoursRB} AstraCoins проводится каждый час только среди активных игроков.");
