...
namespace NeptuneEvo.World.War
{
    public class Repository
    {
        ...
        /// старт капта
        /// </summary>
        /// <param name="warData"></param>
        /// <param name="typeBattle"></param>
        /// <param name="composition"></param>
        /// <param name="weaponsCategory"></param>
        /// <param name="day"></param>
        /// <param name="hour"></param>
        /// <param name="min"></param>
        /// <returns></returns>
        public static ushort WarGang(Players.Models.WarData warData, sbyte typeBattle, sbyte composition, sbyte weaponsCategory, sbyte day = 0, sbyte hour = 0, sbyte min = 0)
        {
            ushort id = _getId();
        ...
        public static ushort War(ExtPlayer player, Players.Models.WarData warData, sbyte typeBattle, sbyte composition, sbyte weaponsCategory, sbyte day = 0, sbyte hour = 0, sbyte min = 0, bool isCheckTime = true, bool isWarInterface = true)
        {
            ...
            try
            {
                ...
                if (warData.Type == WarType.OrgWarZone)
                    Organizations.FamilyZones.Repository.Open(player);
            }
            catch (Exception e)
            {
                Debugs.Repository.Exception(e);
        ...
        private static void UpdateWar(ushort warId)
        {
            try
            {
                ...
                {
                    //если время войны меньше ТЕКУЩЕГО ВРЕМЕНИ
                    if (war.Time <= time)
                    {
                        //удаляет войну
                        war.Delete();
                        ...
                        else
                        {
                            if (war.GripType == WarGripType.RetentionTerritory)
                            {
                                ...
                                Notification(war.Id, "Команда противника может начать атаку.");
                            }
                            else
                            {
                    ...
                    else
                    {
                        ...
                        if (getTime == 15)
                            ...
                            Notification(war.Id, minute: getTime, wayPoint: war.Position);
                        else if (getTime == (1 * 60))
                            Notification(war.Id, minute: getTime, wayPoint: war.Position);
                        else if (getTime == (2 * 60))
                            Notification(war.Id, wayPoint: war.Position);
                        else if (getTime == (3 * 60))
                            Notification(war.Id, wayPoint: war.Position);
                        else if (getTime == (4 * 60))
                            Notification(war.Id, wayPoint: war.Position);
                        else if (getTime == (5 * 60))
                            Notification(war.Id, wayPoint: war.Position);
                        else if (getTime == (10 * 60))
                            Notification(war.Id, wayPoint: war.Position);
                        else if (getTime == (15 * 60))
                            Notification(war.Id, wayPoint: war.Position);
                        else if (getTime == (30 * 60))
                            Notification(war.Id);
                        else if (getTime == (60 * 60))
                            Notification(war.Id);
                ...
                else if (war.Status == WarStatus.Protection)
                {
                    ...
                    war.Status = WarStatus.War;
                    war.Counting = WarCounting;
                    Notification(war.Id, "Команда атаки может начать атаку.");
        ...
        public static void EndWar(ushort warId)
        {
            try
            {
                if (!Wars.ContainsKey(warId))
                ...
                switch (war.Type)
                {
                    case WarType.OrgWarZone:
                        ...
                        break;
                    case WarType.Gangs:
                        Fractions.GangsCapture.UpdateZone((byte)war.ObjectId, victoryId, war.AttackingId, war.ProtectingId);
                        break;
                    case WarType.Mafia:
