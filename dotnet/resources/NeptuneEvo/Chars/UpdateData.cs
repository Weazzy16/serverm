    ...
    class UpdateData : Script
    {
        ...
        public static void RedBucks(ExtPlayer player, int value, string msg)
        {
            try
            {
                ...
                if (msg != String.Empty)
                    GameLog.AccountLog(accountData.Login, accountData.HWID, accountData.IP, accountData.SocialClub, $"{msg} ({value} Majestic Coins)");
            }
            catch (Exception e)
            {
        ...
        public static byte CanIChange(ExtPlayer player, int value, bool errortext = false)
        {
            try
            {
                ...
                if (Convert.ToInt64(characterData.Money) < value)
                {
                    if (errortext) Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.Ru, DataName.NoMoneyForIt, MoneySystem.Wallet.Format(value - Convert.ToInt64(characterData.Money))), 5000);
                    return 3;
                }
