...
namespace NeptuneEvo.Core
{
    class BusinessManager : Script
    {
            ...
            try
            {
                ...
                switch (data.Type)
                {
                    case 0:
                        ...
                        break;
                    case 1:
                        var gasStationProducts = GetGasStationsProducts();

                        foreach (var product in gasStationProducts)
                        {
                            if (data.Products.Find(x => x.Name == product.Name) is null)
                            {
                        ...
                        foreach (var product in data.Products.ToList())
                        {
                            if (gasStationProducts.Find(x => x.Name == product.Name) is null)
                            {
                                data.Products.Remove(product);
                                changed = true;
                            }
        ...
        public static void Init()
        {
            try
            {
                Main.CreateBlip(new Main.BlipData(73, "Premium Clothes Shop", new Vector3(-721.36536, -385.6738, 34.827595), 35, true, 1f));
                PedSystem.Repository.CreateQuest("a_m_y_hipster_01", new Vector3(-722.36084, -386.92456, 34.827896), 160.39421f, title: "~y~NPC~w~ Продавец элитной одежды", colShapeEnums: ColShapeEnums.PremiumShop);
        ...
        public static string[] BusinessTypeNames = new string[13]
         {
            "Магазин 24/7", // 0
            "АЗС", // 1
            "Автосалон", // 2
        ...
        public static int[] BlipByType = new int[13]
        {
            52, // 24/7
            361, // petrol station
            381, // premium
       ...
       //    0, //463, // masks shop
        ...
        public static int[] BlipColorByType = new int[13]
        {
            4, // 24/7
            76, //76, // petrol station
            4, // showroom
            4, // showroom
            4, // showroom
            4, // showroom
        ...
        public static List<Product> GetGasStationsProducts()
        {
            var products = new List<Product>();

            foreach (var petrol in EternalDev.Businesses.GasStation.Controller.Config.PetrolTypes)
                products.Add(new Product(petrol.Value.Price, 0, 0, petrol.Value.Name, false));

            foreach (var product in EternalDev.Businesses.GasStation.Controller.Config.Products)
                products.Add(new Product(product.Price, 0, 0, product.Name, false));
            ...
            try
            {
                ...
                switch (type)
                {
                    case 0:
                        ...
                        break;
                    case 1:
                        _ProductsList.AddRange(GetGasStationsProducts());
                        break;
                    case 2:
                        foreach (string name in CarsNames[0])
                        {
                    ...
                    case 11:
                        ...
                        break;
                    case 12:
                        _ProductsList.Add(new Product(100, 100000, 0, "Запчасти", false));
                        break;
                    case 13:
        ...
        public static void OnBusinessAction(ExtPlayer player, int ID)
        {
            try
            {
                ...
                switch (biz.Type)
                {
                    ...
                    //    return;
                    case 1:
                        EternalDev.Businesses.GasStation.Controller.Interaction(player, biz);
                        return;
                    case 2:
                    case 3:
                    case 4:
        ...
        public static void RemoteEvent_petshopBuy(ExtPlayer player, string petName)
        {
            try
            {
                ...
                if (Convert.ToInt64(characterData.Money) < price)
                {
        ...
        public static void StartTuning(ExtPlayer player)
        {
            try
            {
                ...
                if (vehicleLocalData.Access == VehicleAccess.Personal)
                {
                    ...
                    int koef = 10;
                    if (BusProductsData[model].Price / 10 >= 90000000) koef = 100;
                    int modelPrice = BusProductsData[model].Price / koef;
                ...
                else if (vehicleLocalData.Access == VehicleAccess.Organization && player.IsOrganizationAccess(RankToAccess.OrgTuning))
                {
                    ...
                    int koef1 = 10;
                    if (BusProductsData[model].Price / 10 >= 90000000) koef1 = 100;
                    int modelPrice = BusProductsData[model].Price / koef1;
                    modelPrice = CarsNames[2].Contains(model) ? modelPrice * 10 : modelPrice;
        ...
        public static void RemoteEvent_buyTuning(ExtPlayer player, string category, int index, int otherIndex, int r, int g, int b)
        {
            try
            {
                ...
                int koef1 = 10;
                if (BusProductsData[vehModel].Price / 10 >= 90000000) koef1 = 100;
                int modelPrice = BusProductsData[vehModel].Price / koef1;
        ...
        private static void TryUpdateArmor(ExtPlayer player)
        {
            ...
            {
                item.Data = player.Armor.ToString() + "_" + item.Data.Split("_")[1];
        ...
        public static void buyBusinessCommand(ExtPlayer player)
        {
            try
            {
                ...
                if (id == (int)ColShapeData.Error)
                    id = CustomColShape.GetDataToEnum(player, ColShapeEnums.BusinessInfo);
                if (id == (int)ColShapeData.Error)
                {
                ...
                if (biz.Type == 5 || biz.Type == 2 || biz.Type == 4 || biz.Type == 15)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы не можете купить данный бизнес!", 3000);
                    return;
                }
        ...
        public static void sellBusinessCommand(ExtPlayer player, ExtPlayer target, int price)
        {
            try
            {
                ...
                if (Convert.ToInt64(targetCharacterData.Money) < price)
                {
