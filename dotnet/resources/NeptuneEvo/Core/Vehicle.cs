    ...
    class VehicleManager : Script
    {
        ...
        public static void Init()
        {
            try
            {
              ...
              //  Timers.Start("mileage", 1000, () => MileageControl(), true);
                ...
                foreach (var vehicle in vehiclesData)
                {
                    var vehicleData = new VehicleData.Models.VehicleData
                    {
                        ...
                        MileageCentimeters = (uint)vehicle.MileageCentimeters
                    };
        ...
        private static void MileageControl()
        {
            try
            {
                Log.Write($"📊 [MileageControl] Started, vehicles count: {RAGE.Entities.Vehicles.All.Count}");

                // ✅ УБРАЛИ ПРОВЕРКУ EngineOn
                var allVehicles = RAGE.Entities.Vehicles.All
                    .Cast<ExtVehicle>()
                    .Where(v => v.GetVehicleLocalData() != null)
                    .ToList();

                Log.Write($"📊 [MileageControl] Vehicles with LocalData: {allVehicles.Count}");

                foreach (var veh in allVehicles)
                {
                    var local = veh.GetVehicleLocalData();
                    var data = GetVehicleToNumber(local.NumberPlate);
                    if (local == null || data == null) continue;

                    var parts = (local.Position ?? "").Split(';');
                    float.TryParse(parts.ElementAtOrDefault(0), out var lx);
                    float.TryParse(parts.ElementAtOrDefault(1), out var ly);
                    float.TryParse(parts.ElementAtOrDefault(2), out var lz);

                    var cx = veh.Position.X;
                    var cy = veh.Position.Y;
                    var cz = veh.Position.Z;
                    ...
                    // Сохраняем новую позицию ВСЕГДА (для следующего тика)
                    local.Position = $"{cx};{cy};{cz}";

                    // ✅ Пропускаем, если машина не двигалась
                    if (meters < 1f) continue;

                    var m = (int)Math.Floor(meters);

                    // ✅ ЛОГ ПЕРЕД ИЗМЕНЕНИЯМИ
                    Log.Write($"📊 [Before] Number={local.NumberPlate}, DB={data.Mileage}, Cents={local.Centimeters}, Meters={m}");

                    local.Centimeters += (uint)(m * 100);

                    var totalKm = data.Mileage + (local.Centimeters / 100000f);
                    var mileageToSend = (int)(totalKm * 100);

                    // ✅ ЛОГ ПОСЛЕ РАСЧЁТОВ
                    Log.Write($"📊 [After] Number={local.NumberPlate}, TotalKm={totalKm:F2}, Sending={mileageToSend}");

                    veh.SetSharedData("MILEAGE", mileageToSend);

                    // ✅ Раз в 1 км сохраняем в БД
                    if (local.Centimeters >= 100000)
                    {
                        data.Mileage += (int)(local.Centimeters / 100000);
                        local.Centimeters %= 100000;

                        Log.Write($"📊 [Saved] Number={local.NumberPlate}, NewMileage={data.Mileage}, RemainingCents={local.Centimeters}");
                    }
                }
            }
            catch (Exception e)
            {
                Log.Write($"MileageControl Exception: {e}");
            }
        }
        ...
        public static void Create(ExtPlayer player, string Model, Color Color1, Color Color2, int Health = 1000, int Fuel = 100, int Mileage = 0, string Text = "", string Logs = "")
        {
            if (!player.IsCharacterData()) return;
            
            Trigger.SetTask(() =>
            {
                CreateThread(player, Model, Color1, Color2, Health, Fuel, Mileage, Text, Logs);
            });
        }
        /// <summary>
        /// Создание авто
        ...
        /// <returns></returns>
        public static async void CreateThread(ExtPlayer player, string Model, Color Color1, Color Color2, int Health = 1000, int Fuel = 100, int Mileage = 0,  string Text = "", string Logs = "")
        {
            try
            {
                var sessionData = player.GetSessionData();
        ...
        public static async Task<int> SaveSql(ServerBD db, string number)
        {
            try
            {
                // 1) Берём in‐memory данные
                var vehicleData = GetVehicleToNumber(number);
                if (vehicleData == null)
                {
                ...
                // 2) Пытаемся обновить по строковому полю number
                var affected = await db.Vehicles
                    .Where(v => v.AutoId == vehicleData.SqlId)          // обязательно small‐letter field name!
                    ...
                    .Set(v => v.Position, vehicleData.Position ?? "")
                    .Set(v => v.Rotation, vehicleData.Rotation ?? "")
                    .Set(v => v.Keynum, vehicleData.KeyNum)
                    .Set(v => v.Dirt, vehicleData.Dirt)
                    .Set(v => v.Tag, vehicleData.Tag)
                    .Set(v => v.Mileage, vehicleData.Mileage)
                        .Set(v => v.MileageCentimeters, (int)vehicleData.MileageCentimeters) // ← НОВОЕ
        ...
        public static List<string> GetVehiclesCarModelToPlayer(string name)
        {
            if (name == string.Empty)
                return new List<string>();
        ...
        private static void callback_sellcar(ExtPlayer player , object listItem) /// Никитос Чини  
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.Ru, DataName.ProdazhaVeh), LangFunc.GetText(LangType.Ru, DataName.VvediteCenu), 12, "sellcar");
            }
            catch (Exception e)
            {
            ...
            try
            {
                if (veh != null)
                {
                    ...
                    if (!VehicleModel.AirAutoRoom.isAirCar(veh.Model))
                    {
                        ...
                        if (vehicleData != null)
                            EternalDev.ChipTuning.Controller.ApplyVehicleData(veh, vehicleData.SqlId);
                    ...
                    switch (vehicleLocalData.Access)
                    {
                        case VehicleAccess.Hotel:
                            ...
                            if (VehicleStreaming.GetLockState(vehicle))
                            {
                                ...
                                VehicleNotify.Send(player, NotifyType.Success, LangFunc.GetText(LangType.Ru, DataName.VehOpenDoor));

                            }
                            else
                            {
                                VehicleStreaming.SetLockStatus(vehicle, true);
                                VehicleNotify.Send(player, NotifyType.Success, LangFunc.GetText(LangType.Ru, DataName.VehCloseDoor));
                        ...
                        default:
                            if (characterData.AdminLVL <= 3)
                            {
                                VehicleNotify.Send(player, NotifyType.Success, LangFunc.GetText(LangType.Ru, DataName.VehNoLock));
                                return;
                            }
                            if (VehicleStreaming.GetLockState(vehicle))
                            {
                ...
                switch (eventName)
                {
                    case "engineCarPressed":
                        ...
                        if (vehicleLocalData != null)
                        {
                            ...
                            if (vehicleLocalData.Petrol <= 0)
                            {
                                VehicleNotify.Send(sender, NotifyType.Error, LangFunc.GetText(LangType.Ru, DataName.NoGasVeh)); // ✅
                                return;
                            }
                            switch (vehicleLocalData.Access)
                            {
                                case VehicleAccess.Hotel:
                                    ...
                                    {
                                        VehicleNotify.Send(sender, NotifyType.Error, LangFunc.GetText(LangType.Ru, DataName.NoKeysFromVeh)); // ✅
                                        return;
                                    }
                                    break;
                                ...
                                case VehicleAccess.Fraction:
                                    ...
                                    {
                                        VehicleNotify.Send(sender, NotifyType.Error, LangFunc.GetText(LangType.Ru, DataName.WorkDayNotStarted)); // ✅
                                        return;
                                    }
                                    break;
                                case VehicleAccess.Organization:
                                case VehicleAccess.OrganizationGarage:
                                    ...
                                    {
                                        VehicleNotify.Send(sender, NotifyType.Error, LangFunc.GetText(LangType.Ru, DataName.NoAccessToVeh)); // ✅
                                        return;
                                    }
                                ...
                                case VehicleAccess.Personal:
                                    ...
                                    {
                                        VehicleNotify.Send(sender, NotifyType.Error, LangFunc.GetText(LangType.Ru, DataName.CarDestroyed)); // ✅
                                        return;
                                    }
                                    break;
                                case VehicleAccess.Garage:
                            ...
                            {
                                VehicleStreaming.SetEngineState(vehicle, false);
                                VehicleNotify.Send(sender, NotifyType.Success, LangFunc.GetText(LangType.Ru, DataName.VehEngineON)); // ✅
                            ...
                            else
                            {
                                ...
                                VehicleNotify.Send(sender, NotifyType.Success, LangFunc.GetText(LangType.Ru, DataName.VehEngineOFF)); // ✅
