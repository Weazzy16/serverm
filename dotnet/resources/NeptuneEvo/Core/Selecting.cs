    ...
    class Selecting : Script
    {
        ...
        public static void BeltSelected(ExtPlayer player, bool toggle)
        {
            try
            {
                ...
                if (!toggle)
                {
                    // ✅ Ремень пристёгнут (зелёное уведомление)
                    VehicleNotify.Send(player, NotifyType.Success, "Ремень безопасности пристёгнут");
                    Commands.RPChat("sme", player, $"пристегнул" + (characterData.Gender ? "" : "а") + " ремень безопасности");
                }
                else
                {
                    // ✅ Ремень отстёгнут (красное уведомление)
                    VehicleNotify.Send(player, NotifyType.Error, "Ремень безопасности отстёгнут");
                    Commands.RPChat("sme", player, $"отстегнул" + (characterData.Gender ? "" : "а") + " ремень безопасности");
                }
        ...
        public static void playerSelected(ExtPlayer player, ExtPlayer target, string action)
        {
            try
            {
                ...
                {
                    case "ETERNAL_DICE":
                        Trigger.ClientEvent(player, "openInput", "Введите сумму в долларах", "Сумма...", 7, "ETERNAL_DICE");
                        return;
                    case "ETERNAL_EXCHANGE-PROPS":
                        if (target.HasData("exchange.owner") || target.HasData("exchange.target"))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Игрок уже обменивается с кем-то!", 3000);
                            return;
                        }

                        player.SetData("exchange.target", target);
                        target.SetData("exchange.owner", player);

                        Trigger.ClientEvent(target, "openDialog", "EXCHANGE_ACCEPT", $"Игрок ({player.Value}) предлагает вам обменяться имуществом");
                        return;
                    case "handshake":
                    ...
                    case "givemoney":
                        ...
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.Ru, DataName.GiveMoney), LangFunc.GetText(LangType.Ru, DataName.SumBucks), 9, "player_givemoney");
                        return;
                    case "handsup":
                        if (player.IsInVehicle) return;
                    ...
                    case "sellkit":
                        ...
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.Ru, DataName.OfferHealth), LangFunc.GetText(LangType.Ru, DataName.SumBucks), 12, "player_medkit");
                        return;
                    case "offerheal":
                        ...
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.Ru, DataName.OfferHeal), LangFunc.GetText(LangType.Ru, DataName.SumBucks), 12, "player_heal");
                        return;
                    case "heal":
                    ...
                    case "sellhouse":
                        ...
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.Ru, DataName.OfferHouse), LangFunc.GetText(LangType.Ru, DataName.SumBucks), 12, "player_offerhousesell");
                        return;
                    case "roommate":
                    ...
                    case "ticket":
                        ...
                        Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.Ru, DataName.SendTicket), LangFunc.GetText(LangType.Ru, DataName.Sum0to1000), 12, "player_ticketsum");
                ...
                if (targetSessionData.DeathData.IsDying && !targetSessionData.DeathData.IsReviving)
                {
                    ...
                    NAPI.Task.Run(() =>
                    {
                        try
                        {
                            ...
                            targetSessionData.DeathData.IsReviving = false;
                            int IsPlayerHaveDefibrillator = Chars.Repository.getCountToLacationItem($"char_{player.CharacterData.UUID}", "inventory", ItemId.Defibrillator);
                            if (IsPlayerHaveDefibrillator > 1)
                            {
        ...
        //[RemoteEvent("WhistleAction")]
        //public static void WhistleAction(ExtPlayer player, bool state)
        //{
        //    try
        //    {
        //        if (!player.IsCharacterData()) return;
        ...
        //            player.PlayAnimation("gestures@m@standing@casual", "gesture_what_soft", 0);
        //        }
        ...
        //    catch (Exception e)
        //    {
        //        Log.Write($"WhistleAction Exception: {e.ToString()}");
        //    }
        //}
