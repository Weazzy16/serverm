...
namespace NeptuneEvo.Core
{
    public class GameLog
    {
        ...
        public static void Stock(int Frac, int Uuid, string Name, string Type, int Amount, string direction)
        {
            var cmd = new List<string>();
            cmd.Add("CALL `addLogsData`(@var0, @var1, @var2)");
            cmd.Add("`stocklog`");
            cmd.Add("`time`,`frac`,`uuid`,`name`,`type`,`amount`,`direction`");
            cmd.Add($"'{DateTime.Now:s}',{Frac},{Uuid},'{Name}','{Type}',{Amount},'{direction}'");
