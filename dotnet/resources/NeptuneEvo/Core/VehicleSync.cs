...
namespace NeptuneEvo.Core
{
    public class VehicleStreaming : Script
    {
        ...
        public static ExtVehicle CreateVehicle(
            uint model, Vector3 pos, float rot,
            int color1, int color2, string numberPlate = "",
            byte alpha = 255, bool locked = false,
            bool engine = false, uint dimension = 0,
            int petrol = 0, int workdriv = -1,
            bool cm = false, bool cd = false, bool cmk = false,
            string by = "NONE", VehicleAccess acc = VehicleAccess.None,
            int fr = -1, int numb = -1, JobsId work = JobsId.None,
            float dirt = 0.0f, int minrank = 100)
        {
            try
            {
                var veh = (ExtVehicle)NAPI.Vehicle.CreateVehicle(
                    model, pos, rot, color1, color2,
                    numberPlate, alpha, locked, engine, dimension
                );
                ...
                var serverData = VehicleManager.GetVehicleToNumber(numberPlate);
                if (serverData == null)
                {
                    serverData = new VehicleData.Models.VehicleData
                    {
                        // базовый пробег — 0
                        Mileage = 0,
                        // базовые «сырые» уровни тюнинга от –1
                ...
                else if (serverData.Components == null)
                {
                    // на всякий случай, если Components вдруг null
                    serverData.Components = new VehicleCustomization
                    {
                        Engine = -1,
                        Transmission = -1,
                        Turbo = -1
                ...
                var initMileage = serverData.Mileage;
                var initCents = serverData.MileageCentimeters; // ← НОВОЕ

                // ─────────────────────────────────────────────────

                // локальные данные машины
                veh.SetVehicleLocalData(new VehicleLocalData
                {
                    ...
                    Centimeters = initCents,

                    Mileage = initMileage,
                    Position = $"{veh.Position.X};{veh.Position.Y};{veh.Position.Z}",
                    EngineOn = engine
                });
                Log.Write($"📊 [CreateVehicle] Setting MILEAGE SharedData: {initMileage} for {numberPlate}");

                var totalKm = initMileage + (initCents / 100000f);
                ...
                // ───────────── ИНИЦИАЛИЗАЦИЯ ТЮНИНГА ─────────────
                // "сырые" из БД: –1…3 для двигателя, –1…2 для коробки, –1…0 для турбо
                var rawEngine = serverData.Components.Engine;
                var rawTrans = serverData.Components.Transmission;
                var rawTurbo = serverData.Components.Turbo;

                // приводим к 0…4 / 0…3 / 0…1
                var engineLvl = Math.Max(rawEngine + 1, 0);
                var transLvl = Math.Max(rawTrans + 1, 0);
                var turboLvl = Math.Max(rawTurbo + 1, 0);

                veh.SetSharedData("engineLevel", engineLvl);
                veh.SetSharedData("transmissionLevel", transLvl);
                veh.SetSharedData("turboLevel", turboLvl);
                // ────────────────────────────────────────────────
        ...
        public async Task UpdateMileage(ExtPlayer player, int vehicleId, int centimeters)
        {
            var veh = (ExtVehicle)player.Vehicle;
            var local = veh.GetVehicleLocalData();
            if (local == null || veh.Value != vehicleId) return;

            // ✅ ЗАЩИТА ОТ ЧИТОВ: макс 3000 см (30 метров) за раз
            if (centimeters > 15000)
            {
                Log.Write($"⚠️ [UpdateMileage CHEAT] Player={player.Name}, Cents={centimeters} (too high!)");
                return;
            }

            local.Centimeters += (uint)centimeters;

            var addKm = (int)(local.Centimeters / 100000u);
            if (addKm > 0)
            {
                local.Mileage += addKm;
            ...
            var serverData = VehicleManager.GetVehicleToNumber(local.NumberPlate);
            if (serverData != null)
            {
                serverData.Mileage = local.Mileage;
                serverData.MileageCentimeters = local.Centimeters; // ← НОВОЕ

            }

            var totalKm = local.Mileage + (local.Centimeters / 100000f);
            var mileageToSend = (int)(totalKm * 100);

            Log.Write($"📊 [UpdateMileage] Number={local.NumberPlate}, +{centimeters}cm, Total={totalKm:F2}, Sending={mileageToSend}");

            veh.SetSharedData("MILEAGE", mileageToSend);

            // ✅ Сохраняем в БД только при накоплении целого км
            if (addKm > 0)
            {
                await using var db = new ServerBD("MainDB");
                await VehicleManager.SaveSql(db, local.NumberPlate);
            }
        }
        ...
        public void SyncLights(ExtPlayer player, int lightsState)
        {
            try
            {
                ...
                {
                    Log.Write($"🔦 Player not in vehicle");
                    return;
                }
