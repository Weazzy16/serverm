    ...
    class Commands : Script
    {
        ...
        public static void CMD_giveredsall(ExtPlayer player, int amount)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Выдать можно минимум 1 AC и максимум 150000 AC.", 3000);
                    return;
                }
                ...
                foreach (ExtPlayer foreachPlayer in NeptuneEvo.Character.Repository.GetPlayers())
                {
                    ...
                    UpdateData.RedBucks(foreachPlayer, correctValue, msg: "Отправка AC");
                ...
                NAPI.Chat.SendChatMessageToAll($"{CommandsAccess.AdminPrefixChat}{player.Name.Replace('_', ' ')} выдал всем игрокам {amount} Majestic Coin.");
        ...
        */
        ...
        [Command("tpayday", GreedyArg = false)]
        public static void tPayDay(ExtPlayer player, int seconds = 60)
        {
            try
            {
                // Та же проверка прав, что и для обычного payday
                if (!CommandsAccess.CanUseCmd(player, AdminCommands.Payday)) return;

                if (seconds < 0)
                {
                    player.SendChatMessage("~r~Ошибка: время должно быть неотрицательным числом секунд.");
                    return;
                }

                GameLog.Admin($"{player.Name}", $"tpayday", $"Через {seconds} сек.");

                // Запускаем фоновую задачу, которая ждёт указанное число секунд
                _ = Task.Run(async () =>
                {
                    try
                    {
                        ...
                        Main.payDayTrigger(false);
                        player.SendChatMessage($"~g~Отложенная выплата payday ({seconds} сек) выполнена.");
                    }
                    catch (Exception ex)
                    {
                        Log.Write($"tpayday Trigger Exception: {ex}");
                    }
                });

                player.SendChatMessage($"~g~Выплата payday будет через {seconds} сек.");
            }
            catch (Exception e)
            {
                Log.Write($"tpayday Exception: {e}");
                player.SendChatMessage("~r~Произошла ошибка при планировании отложенной выплаты.");
            }
        }
        ...
        [Command(AdminCommands.setCaseTimer)]
        public void CMD_SetCaseTimer(ExtPlayer admin, int caseId, string start, string end)
        {

            DateTime st = DateTime.ParseExact(start, "yyyy-MM-dd_HH:mm", CultureInfo.InvariantCulture);
            DateTime en = DateTime.ParseExact(end, "yyyy-MM-dd_HH:mm", CultureInfo.InvariantCulture);
            using var db = new ConfigBD("ConfigDB");
            db.InsertOrReplace(new CaseSchedule { CaseId = caseId, StartCase = st, EndCase = en });
            Notify.Send(admin, NotifyType.Success, NotifyPosition.BottomCenter,
               $"Таймер кейса {caseId} установлен: с {st:dd.MM HH:mm} до {en:dd.MM HH:mm}.", 3000);
        }
        ...
        [Command(AdminCommands.Setneeds)] // Команда: /setneeds [id] [еда] [вода]
        public static void CMD_adminSetNeeds(ExtPlayer player, int id, int food, int water)
        {
            try
            {
                var targetPlayer = Main.GetPlayerByUUID(id);
                if (targetPlayer == null || !targetPlayer.IsCharacterData())
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Игрок не найден.", 3000);
                    return;
                }

                var targetCharacterData = targetPlayer.GetCharacterData();

                targetCharacterData.Golod = Math.Clamp(food, 0, 100);
                targetCharacterData.Water = Math.Clamp(water, 0, 100);

                GolodSystem.GolodChanger(targetPlayer);
                GolodSystem.WaterChanger(targetPlayer);

                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы успешно установили еду и воду игроку с ID {id}.", 3000);
                Notify.Send(targetPlayer, NotifyType.Info, NotifyPosition.BottomCenter, $"Администратор {player.Name} пополнил ваши показатели еды и воды.", 3000);

            }
            catch (Exception e)
            {
                Log.Write($"CMD_adminSetNeeds Exception: {e}");
            }
        }
        ...
        public static void CMD_setWeather(ExtPlayer player, int weather)
        {
            try
            {
                ...
                var currentWeatherData = World.Weather.Repository.GetCurrentWeather();
                int temp = currentWeatherData != null ? currentWeatherData.Temp : 20;

                Trigger.ClientEventForAll("client.phone.updWeather", weather, temp);

                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({characterData.UUID}) изменил погоду({weather})");
        ...
        // Альтернативный подход - создание ботов как Ped с системой nametag
        public static Dictionary<int, BotData> ActiveBots = new Dictionary<int, BotData>();

        public class BotData
        {
            public Ped PedEntity { get; set; }
            ...
            public ExtPlayer Creator { get; set; }
            public DateTime CreateTime { get; set; }
        }

        [Command(AdminCommands.Createbot)]
        public static void CMD_createBot(ExtPlayer player, string name, int adminlvl = 0, bool isMasked = false, bool isFriend = false)
        {
            try
            {
                if (!CommandsAccess.CanUseCmd(player, AdminCommands.Createbot)) return;
                var characterData = player.GetCharacterData();
                if (characterData == null) return;
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Неверный формат имени. Используйте: Имя_Фамилия", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Имя и фамилия должны contain минимум 3 символа", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Игрок с таким именем уже существует", 3000);
                    return;
                }

                // Проверка уровня админки
                if (adminlvl < 0 || adminlvl > 10)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Уровень админки должен быть от 0 до 10", 3000);
                    return;
                }

                // Проверка лимита ботов
                if (ActiveBots.Count >= 50)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Достигнут лимит ботов (50)", 3000);
                    return;
                }

                // Создание бота
                var bot = CreateBotPed(name, player.Position, player.Rotation.Z, UpdateData.GetPlayerDimension(player), adminlvl, isMasked, isFriend, player);
                if (bot == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Ошибка создания бота", 3000);
                    return;
                }

                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) создал бота {name} (Admin: {adminlvl}, Mask: {isMasked}, Friend: {isFriend})");
                GameLog.Admin($"{player.Name}", $"createBot({name},{adminlvl},{isMasked},{isFriend})", "");

                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Бот {name} успешно создан! ID: {bot.PedEntity.Value}", 3000);
            }
            catch (Exception e)
            {
                Log.Write($"CMD_createBot Exception: {e.ToString()}");
            }
        }

        private static BotData CreateBotPed(string name, Vector3 position, float rotation, uint dimension, int adminlvl, bool isMasked, bool isFriend, ExtPlayer creator)
        {
            try
            {
                // Создаем Ped
                var botPed = NAPI.Ped.CreatePed(PedHash.Business01AMM, position, rotation, dimension);
                if (botPed == null) return null;

                // Генерируем уникальные данные
                int botUUID = GenerateBotUUID();
                ...
                ActiveBots[botPed.Value] = botData;

                // Добавляем в глобальные списки
                Main.PlayerNames[botUUID] = name;
                Main.PlayerUUIDs[name] = botUUID;

                // Настройка Shared Data для nametag.js
                if (adminlvl > 0)
                {
                    botPed.SetSharedData("REDNAME", true);
                    botPed.SetSharedData("ALVL", adminlvl);
                }

                if (isMasked)
                {
                    botPed.SetSharedData("IS_MASK", true);
                }

                botPed.SetSharedData("UUID", botUUID);
                botPed.SetSharedData("hmn", false); // Показывать имя
                botPed.SetSharedData("tempName", (string)null);

                // Создаем текстовую метку над головой (статичная позиция)
                var displayName = GenerateBotDisplayName(name, botUUID, adminlvl, isMasked, isFriend, creator);
                var nameLabel = NAPI.TextLabel.CreateTextLabel(displayName, new Vector3(position.X, position.Y, position.Z + 1.0f), 30f, 0.5f, 0, new Color(255, 255, 255), true, dimension);

                // Сохраняем ссылку на метку для последующего удаления
                botData.NameLabel = nameLabel;

                Log.Write($"Bot {name} created successfully with UUID {botUUID}");
                return botData;
            }
            catch (Exception e)
            {
                Log.Write($"CreateBotPed Exception: {e.ToString()}");
                return null;
            }
        }

        private static string GenerateBotDisplayName(string name, int uuid, int adminlvl, bool isMasked, bool isFriend, ExtPlayer creator)
        {
            try
            {
                var creatorCharacterData = creator.GetCharacterData();
                if (creatorCharacterData == null) return $"{name}\n#{uuid}";

                // Генерируем отображаемое имя как в nametag.js
                string displayName = "";

                if (creatorCharacterData.AdminLVL > 0) // Админ видит всё
                {
                    displayName = $"{name.Replace('_', ' ')} [BOT]\n#{uuid}";
                }
                else if (adminlvl > 0) // Бот-админ
                {
                    displayName = $"{name.Replace('_', ' ')} [BOT]\n#{uuid}";
                }
                else if (isMasked) // В маске
                {
                    displayName = $"[BOT]\n#{uuid}";
                }
                else if (isFriend) // Друг
                {
                    displayName = $"{name.Replace('_', ' ')} [BOT]\n#{uuid}";
                }
                else // Обычный бот
            ...
            {
                Log.Write($"GenerateBotDisplayName Exception: {e.ToString()}");
                return $"{name}\n#{uuid}";
            }
        }

        private static int GenerateBotUUID()
        {
            // Генерируем уникальный UUID для бота
            int uuid;
            do
            {
                uuid = Main.rnd.Next(1000000, 9999999);
            } while (Main.PlayerNames.ContainsKey(uuid));

            return uuid;
        }

        [Command(AdminCommands.Deletebot)]
        public static void CMD_deleteBot(ExtPlayer player, int id)
        {
            try
            {
                if (!CommandsAccess.CanUseCmd(player, AdminCommands.Deletebot)) return;
                var characterData = player.GetCharacterData();
                if (characterData == null) return;

                if (!ActiveBots.ContainsKey(id))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Бот с таким ID не найден", 3000);
                    return;
                }

                var botData = ActiveBots[id];

                // Очистка данных
                Main.PlayerNames.TryRemove(botData.UUID, out _);
                Main.PlayerUUIDs.TryRemove(botData.Name, out _);

                // Удаление Ped и метки
                if (botData.PedEntity != null && botData.PedEntity.Exists)
                {
                    botData.PedEntity.Delete();
                }

                if (botData.NameLabel != null && botData.NameLabel.Exists)
                {
                    botData.NameLabel.Delete();
                }

                // Удаление из списка
                ActiveBots.Remove(id);

                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) удалил бота {botData.Name} ({id})");
                GameLog.Admin($"{player.Name}", $"deleteBot({id})", botData.Name);

                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Бот {botData.Name} удален", 3000);
            }
            catch (Exception e)
            {
                Log.Write($"CMD_deleteBot Exception: {e.ToString()}");
            }
        }

        [Command(AdminCommands.Botlist)]
        public static void CMD_botList(ExtPlayer player)
        {
            try
            {
                if (!CommandsAccess.CanUseCmd(player, AdminCommands.Botlist)) return;

                Trigger.SendChatMessage(player, "=== BOTS ONLINE ===");

                if (ActiveBots.Count == 0)
                {
                    Trigger.SendChatMessage(player, "Нет активных ботов");
                }
                else
                {
                    foreach (var botEntry in ActiveBots)
                    {
                        var botData = botEntry.Value;
                        string maskStatus = botData.IsMasked ? "[В МАСКЕ]" : "";
                        string adminStatus = botData.AdminLevel > 0 ? $"[A{botData.AdminLevel}]" : "";
                        string friendStatus = botData.IsFriend ? "[ДРУГ]" : "";

                        Trigger.SendChatMessage(player, $"[{botEntry.Key}] {botData.Name} {adminStatus} {maskStatus} {friendStatus}");
                    }

                    Trigger.SendChatMessage(player, $"Всего ботов: {ActiveBots.Count}");
                }
            ...
            {
                Log.Write($"CMD_botList Exception: {e.ToString()}");
            }
        }

        [Command(AdminCommands.Deleteallbots)]
        public static void CMD_deleteAllBots(ExtPlayer player)
        {
            try
            {
                if (!CommandsAccess.CanUseCmd(player, AdminCommands.Deleteallbots)) return;
                var characterData = player.GetCharacterData();
                if (characterData == null) return;

                if (ActiveBots.Count == 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Нет активных ботов", 3000);
                    return;
                }

                int deletedCount = ActiveBots.Count;

                foreach (var botEntry in ActiveBots.Values.ToList())
                {
                    // Очистка данных
                    Main.PlayerNames.TryRemove(botEntry.UUID, out _);
                    Main.PlayerUUIDs.TryRemove(botEntry.Name, out _);

                    // Удаление Ped и метки
                    if (botEntry.PedEntity != null && botEntry.PedEntity.Exists)
                    {
                        botEntry.PedEntity.Delete();
                    }

                    if (botEntry.NameLabel != null && botEntry.NameLabel.Exists)
                    {
                        botEntry.NameLabel.Delete();
                    }
                }
                ...
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) удалил всех ботов ({deletedCount} шт.)");
                GameLog.Admin($"{player.Name}", $"deleteAllBots({deletedCount})", "");

                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Удалено {deletedCount} ботов", 3000);
            }
            catch (Exception e)
            {
                Log.Write($"CMD_deleteAllBots Exception: {e.ToString()}");
            }
        }
        ...
        public static void CMD_report(ExtPlayer player, string message)
        {
            try
            {
                ...
                var opened = ReportNew.Tickets.Values.FirstOrDefault(p => p.Author == player.Name && !p.Closed);
                if (opened != null)
                {
                    ReportNew.ReportAddMessage(player, message);
                    return;
                }
                ReportNew.AddReport(player, message, player.Name);
            }
            catch (Exception e)
            {
      ...
      /*  [Command("buybiz")]
            ...
            try
            {
                ...
                switch (result)
                {
                    ...
                    case 2:
                        ...
                        {
                            Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "try", "!{#ffffff}{name} " + message + "!{#61de2f}" + " удачно", new int[] { sender.Value });
                    ...
                    default:
                        foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(sender.Position, 10f,
                                     UpdateData.GetPlayerDimension(sender)))
                        {
                            Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "try", "!{#ffffff}{name} " + message + "!{#de2f2f}" + " неудачно", new int[] { sender.Value });
                ...
                switch (cmd)
                {
                    case "me":
                        foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(sender.Position, 10f,
                                     UpdateData.GetPlayerDimension(sender)))
                        {
                            Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "me", "!{#c2a2da}{name} " + message, names);
                    ...
                    case "do":
                        foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(sender.Position, 10f,
                                     UpdateData.GetPlayerDimension(sender)))
                        {
                            Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "do", "!{#00c9c3}" + message+"." + " ({name})", names);
                    ...
                    case "b":
                        foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(sender.Position, 10f, UpdateData.GetPlayerDimension(sender)))
                            Trigger.ClientEvent(foreachPlayer, "sendRPMessage", "b", "!{#dbdbdb}(( {name}: " + message + " ))", names);
                        return;
                    case "sb":
                        foreach (ExtPlayer foreachPlayer in Main.GetPlayersInRadiusOfPosition(sender.Position, 10f,
                                     UpdateData.GetPlayerDimension(sender)))
        ...
        [Command("buychips")]
        public static void CMD_BuyChips(ExtPlayer player, int amount)
        {
            try
            {
                if (amount <= 0) return;
                if (!MoneySystem.CasinoChips.ExchangeMoneyToChips(player, amount))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.Ru, DataName.NoMoney), 3000);
                    return;
                }
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы обменяли {amount}$ на {amount} фишек", 3000);
            }
            catch (Exception e)
            {
                Log.Write($"CMD_BuyChips Exception: {e.ToString()}");
            }
        }

        [Command("sellchips")]
        public static void CMD_SellChips(ExtPlayer player, int amount)
        {
            try
            {
                if (amount <= 0) return;
                if (!MoneySystem.CasinoChips.ExchangeChipsToMoney(player, amount))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Недостаточно фишек", 3000);
                    return;
                }
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы обменяли {amount} фишек на {amount}$", 3000);
            }
            catch (Exception e)
            {
                Log.Write($"CMD_SellChips Exception: {e.ToString()}");
            }
        }
        #endregion RP Chat
    }
