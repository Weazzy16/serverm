...
using static NeptuneEvo.Core.EventSys;
    ...
    class Admin : Script
    {
        ...
        public static void sendRedbucks(ExtPlayer player, ExtPlayer target, int amount)
        {
            try
            {
                ...
                UpdateData.RedBucks(target, amount, msg: "Отправка RC");
            ...
            try
            {
                ...
                Trigger.SetTask(async () =>
                {
                    try 
                    {
                        ...
                        await db.Characters
                            .Where(c => c.Uuid == character.Uuid)
                            .Set(c => Convert.ToInt64(c.Money), money)
                            .UpdateAsync();
                    }
                    catch (Exception e)
                    {
        ...
        public static void getRb(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"У игрока {target.Name} ({target.Value}) - {targetRb} Majestic Coins", 5000);
                AdminLog(characterData.AdminLVL, $"{player.Name} ({player.Value}) узнал количество AC у {target.Name} ({target.Value}) - {targetRb}");
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                else
                {
...

                    {
                        ...
                        if (isSBan == true) AdminsLog(characterData.AdminLVL, $"{player.Name} {characterData.UUID} выдал бан {target.Name}({target.Value}) Причина: {reason}", 1, "#FFB833", false);
...

                    {
                        ...
                        if (isSBan == true) AdminsLog(characterData.AdminLVL, $"{player.Name} {characterData.UUID} выдал бан {target.Name}({target.Value}) без лишнего шума на {time} дней. Причина: {reason}", 1, "#FFB833", false);
        ...
        public void SetPlayerNeeds(ExtPlayer player, int targetId, int food, int water)
        {
            try
            {
                if (!CommandsAccess.CanUseCmd(player, AdminCommands.Setneeds)) return;

                var targetPlayer = Main.GetPlayerByID(targetId);
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы успешно установили еду и воду игроку {targetPlayer.Name} ({targetId}).", 3000);
            ...
            {
                Log.Write($"SetPlayerNeeds Exception: {e}");
            }
        }
        ...
        public static void adminSMS(ExtPlayer player, ExtPlayer target, string message)
        {
            try
            {
                ...
                Trigger.SendToAdmins(characterData.AdminLVL, $"{ChatColors.Report}[ADMIN] {player.Name} ({player.Value}) ответил {target.Name} ({target.Value}): {message}");
                ...
                Trigger.ClientEvent(target, "StartDangerButtonSound_client", "sounds/admin_notification.ogg");
            }
            catch (Exception e)
            {
        ...
        public static void sendPlayerToDemorgan(ExtPlayer admin, ExtPlayer target, int time, string reason)
        {
            try
            {
                ...
                //55
                target.Position = DemorganPositions[Main.rnd.Next(3)] + new Vector3(0, 0, 1.5);
        ...
        [RemoteEvent("requestBanCount")]
        public static void OnRequestBanCount(ExtPlayer player, int uuid)
        {
            // 1) Формируем команду с COUNT(*)
            var cmd = new MySqlConnector.MySqlCommand(
                @"SELECT COUNT(*) AS cnt
          FROM `mainlogs`.`banlog`
          WHERE `player` = @uuid;"
            );
            ...
            int count = 0;
            if (dt != null && dt.Rows.Count > 0 && dt.Rows[0]["cnt"] != DBNull.Value)
            {
                count = Convert.ToInt32(dt.Rows[0]["cnt"]);
            }

            // 3) Шлем клиенту
            Trigger.ClientEvent(player, "showBanCount", count);
        }

        [RemoteEvent("requestBanHistory")]
        public static async Task OnRequestBanHistory(ExtPlayer player, int targetUuid)
        {
            NAPI.Util.ConsoleOutput($"[BanHist] запрос истории банов для UUID={targetUuid}");
            try
            {
                // Инициализируем соединение (если ещё не инициализировано)
                MySQL.Init();

                // Формируем команду
                const string sql = @"
                    SELECT `time`, `admin`, `reason`, `until`, `ishard`
                    FROM `mainlogs`.`banlog`
                    WHERE `player` = @uuid
                    ORDER BY `time` DESC;";
                var cmd = new MySqlConnector.MySqlCommand(sql);
                cmd.Parameters.AddWithValue("@uuid", targetUuid);

                // Выполняем синхронный запрос и получаем DataTable
                ...
                if (dt != null)
                {
                    ...
                    {
                        // Получаем время
                        var time = row.Field<DateTime>("time").ToString("yyyy-MM-dd HH:mm:ss");
                        // Получаем окончание
                        var until = row.Field<DateTime>("until").ToString("yyyy-MM-dd HH:mm:ss");

                        // Для admin и reason -- любое содержимое приводим к строке
                        var adminValue = row["admin"];
                        var reasonValue = row["reason"];
                        var rawIsHard = row["ishard"];
                        bool isHard = false;
                        if (rawIsHard != DBNull.Value)
                            isHard = Convert.ToBoolean(rawIsHard);

                        list.Add(new BanEntry

                        {
                            Time = time,
                            Admin = adminValue != null ? adminValue.ToString() : string.Empty,
                            Reason = reasonValue != null ? reasonValue.ToString() : string.Empty,
                            Until = until,
                            IsHard = isHard
                        });
                    }
                ...
                Trigger.ClientEvent(player, "showBanHistory", json);
            }
            catch (Exception e)
            {
                NAPI.Util.ConsoleOutput($"[OnRequestBanHistory] Exception: {e}");
            }
        }
        [RemoteEvent("requestPaydayTimer")]
        public static void OnRequestPaydayTimer(ExtPlayer player)
        {
            var data = player.GetCharacterData();
            if (data == null) return;
            Trigger.ClientEvent(player, "updatePaydayTimer", data.SecondsSincePayday);
        }
        ...
        public static Vector3[] DemorganPositions = new Vector3[3]
        {
            new Vector3(1774.4744, 2568.2441, 50.550163),
            new Vector3(1769.7117, 2573.667, 50.550167),
            new Vector3(1769.1252, 2581.2195, 45.798588)
        };
