...
namespace NeptuneEvo.Fractions
{
    class Manager : Script
    {
        ...
        public static bool EnableFractionSM = false;
        public static void OpenFractionSM(ExtPlayer player, string type)
        {
            try
            {
                if (!EnableFractionSM) {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "В данный момент этот бот не работает.", 3000);
                    return;
                }
            ...
            try
            {
                ...
                switch (index)
                {
                    case 33:
                        ...
                        if (Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", ItemId.BodyArmor, 1, "100_True") == -1)
                        {
                    ...
                    case 26:
                        ...
                        else armorp = Convert.ToInt32(ItemStruct.Item.Data.Split("_")[0]);
                        int matstoadd;
