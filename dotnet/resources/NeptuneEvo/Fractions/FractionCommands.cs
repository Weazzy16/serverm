    ...
    class FractionCommands : Script
    {
        ...
        public static void ticketToTarget(ExtPlayer player, ExtPlayer target, int sum, string reason)
        {
            try
            {
                ...
                if (Convert.ToInt64(targetCharacterData.Money) < sum && MoneySystem.Bank.GetBalance(targetCharacterData.Bank) < sum)
                {
        ...
        public static void unFollow(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                target.SetSharedData("im.followed", -1);
                targetSessionData.Following.SetSharedData("follow.suspect", -1);

                target.Position = targetSessionData.Following.Position;
        ...
        public static void OnDeath(ExtPlayer player, ExtPlayer killer, uint weapon)
        {
            try
            {
                var sessionData = player.GetSessionData();
                ...
                if (sessionData.Follower != null)       // cop
                {
                    sessionData.Follower.SetSharedData("im.followed", -1);
                    player.SetSharedData("follow.suspect", -1);

                    var suspect = sessionData.Follower;
                    var suspectSessionData = suspect.GetSessionData();
                    if (suspectSessionData is null) return;

                    suspectSessionData.Following = null;
                    sessionData.Follower = null;
                }
                else if (sessionData.Following != null)  // suspect
                {
                    player.SetSharedData("im.followed", -1);
                    sessionData.Following.SetSharedData("follow.suspect", -1);

                    var cop = sessionData.Following;
                    var copSessionData = cop.GetSessionData();
                    if (copSessionData is null) return;

                    copSessionData.Follower = null;
                    sessionData.Following = null;
                }
            }
            catch (Exception ex) { Log.Write("OnDeath: " + ex.ToString()); }
        }
        ...
        public static void targetFollowPlayer(ExtPlayer player, ExtPlayer target, bool force = false)
        {
            try
            {
                ...
                player.SetSharedData("follow.suspect", target.Value);
                target.SetSharedData("im.followed", player.Value);
        ...
        public static void robberyTarget(ExtPlayer player, ExtPlayer target)
        {
            try
            {
                ...
                int max = (targetCharacterData.Money >= Convert.ToInt64(Main.PricesSettings.MaxOgrableine)) ? Main.PricesSettings.MaxOgrableine : Convert.ToInt32(targetCharacterData.Money) - 20;
        ...
        public static void sellMedKitToTarget(ExtPlayer player, ExtPlayer target, int price)
        {
            try
            {
                ...
                if (player.IsFractionAccess(RankToAccess.Medkit))
                {
                    ...
                    if (price < 500000 || price > 1500000)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы должны установить цену от 500000$ до 1500000$", 3000);
                        return;
                    }
                    if (player.Position.DistanceTo(target.Position) > 2)
                    {
