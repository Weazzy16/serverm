    ...
    class AlcoFabrication : Script
    {
        ...
        public static void ResistTimer(ExtPlayer player)
        {
            try
            {
                ...
                var time = resistData.Time;
                Trigger.ClientEvent(player, "update:AlcoSystem::client", time);
                if (resistData.Time-- <= 0)
                {
                    ...
                    NAPI.Task.Run(() =>
                    {
                        AlcoholBoost.Reset(player);
                        var time = resistData.Time;
                        Trigger.ClientEvent(player, "stop:AlcoSystem::client");
                    }, 1000);
                }
            }
            catch (Exception e)
