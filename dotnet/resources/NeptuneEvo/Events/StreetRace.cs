    ...
    class StreetRace : Script
    {
        ...
        private static void callback_selectChar(ExtPlayer player, object listItem)
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "openInput", LangFunc.GetText(LangType.Ru, DataName.StreetRace), LangFunc.GetText(LangType.Ru, DataName.EnterStavka0to100k), 12, "streetrace");
            }
            catch (Exception e)
            {
        ...
        public static void SelectRate(ExtPlayer player, int rate)
        {
            try
            {
                ...
                if (createdCharacterData == null || Convert.ToInt64(createdCharacterData.Money) < rate)
                {
                    ClearData(streetRaceData.created, 0);
                    return;
                }
                var joinedCharacterData = streetRaceData.joined.GetCharacterData();
                if (joinedCharacterData == null || Convert.ToInt64(joinedCharacterData.Money) < rate)
                {
