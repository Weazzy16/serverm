...
using NeptuneEvo.EternalDev.VehiclePetrol;  // чтобы найти Controller
            ...
            try
            {
                ...
                foreach (var number in vehiclesNumber)
                {
                    ...
                    if (vehicleData.Holder == sessionData.Name) //sell
                    {
                        ...
                        // индекс 10 — RecommendedFuel
                        var fuelType = NeptuneEvo.EternalDev.VehiclePetrol.Controller
                                           .GetValidPetrol(vehicleData.Model);
                        carData.Add(fuelType.ToString());
                        carData.Add(vehicleData.Components.Engine);       // idx 11
                        carData.Add(vehicleData.Components.Brakes);       // idx 12
                        carData.Add(vehicleData.Components.Transmission); // idx 13
                        carData.Add(vehicleData.Components.Turbo);        // idx 14
                        carData.Add(vehicleData.Mileage);

                    }
                    // теперь item[15] — пробег в километрах
