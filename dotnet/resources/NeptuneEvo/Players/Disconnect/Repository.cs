        ...
        public static void OnPlayerDisconnect(ExtPlayer player, DisconnectionType type, string reason, bool isSave = true)
        {
            try
            {
                    ...
                    if (type != DisconnectionType.Kicked && characterData.AdminLVL == 0)
                    {
                        ...
                        if (cufdata.Cuffed && (cufdata.CuffedByMafia || cufdata.CuffedByCop) && characterData.DemorganTime <= 0)
                        {
                            if (cufdata.CuffedByCop)
                            {
                                ...
                                GameLog.Admin($"server", $"demorgan(30мин,выход из игры в наручниках)", $"{player.Name}");
                            }
                            else if (cufdata.CuffedByMafia)
                            {
                                ...
                                GameLog.Admin($"server", $"demorgan(30мин,выход из игры в стяжках)", $"{player.Name}");
                            }
                            ...
                            characterData.DemorganTime = 1800;
                ...
                if (sessionData != null)
                {
                    ...
                    if (sessionData.TimersData.SkillBoostTimer != null)
                    {
                        Timers.Stop(sessionData.TimersData.SkillBoostTimer);
                        sessionData.TimersData.SkillBoostTimer = null;
                    }
                    if (sessionData.TimersData.HairStylingTimer != null)
                    {
                        Timers.Stop(sessionData.TimersData.HairStylingTimer);
                        sessionData.TimersData.HairStylingTimer = null;
                    }
                    if (sessionData.TimersData.SalaryBonusTimer != null)
                    {
                        Timers.Stop(sessionData.TimersData.SalaryBonusTimer);
                        sessionData.TimersData.SalaryBonusTimer = null;
                    }
                    if (sessionData.TimersData.RecoveryTimer != null)
                    {
                        Timers.Stop(sessionData.TimersData.RecoveryTimer);
                ...
                if (characterData != null && Chars.Repository.ItemsData.ContainsKey($"char_{characterData.UUID}") && Chars.Repository.ItemsData[$"char_{characterData.UUID}"].ContainsKey("accessories") && Chars.Repository.ItemsData[$"char_{characterData.UUID}"]["accessories"].ContainsKey(7))
                {
                    ...
                    aItem.Data = $"{characterData.Armor}_{aItem.Data.Split("_")[1]}";
