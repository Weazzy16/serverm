...
namespace NeptuneEvo.Players.Connect
{
    class Events : Script
    {
        ...
        private void OnPlayerConnected(ExtPlayer player)
        {
            ...
            Trigger.ClientEvent(player, "client.init.cef",
        ...
        [RemoteEvent("server.startGame")]
        private void StartGame(ExtPlayer player)
        {
            try
            {
                if (player.IsSessionData())
            ...
            catch (Exception ex) { Log.Write(nameof(StartGame) + ": " + ex.ToString()); }
        }
    }
}
