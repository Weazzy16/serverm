...
namespace NeptuneEvo
{
    ...
    {
        public static string Codename { get; } = "Majestic RolePlay";
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                NAPI.Server.SetDefaultSpawnLocation(new Vector3(385.223755, -752.47345, 26.4760361), 0);
                            ...
                            AppDomain.CurrentDomain.ProcessExit += (o, e) =>
                            {
                                try
                                {
                                    ...
                                    NAPI.Task.Run(() =>
                                    {
                                        ...
                                        catch (Exception ex) { Log.Write("Task.OnProcessExit:" + ex.ToString()); }
                                    });
                                ...
                                catch (Exception ex) { Log.Write("OnProcessExit: " + ex.ToString()); }
                            };
                        }
                        catch (Exception e)
                        {
        ...
        // Исправление плохих UUID во всех БД | by @merumond ======================== //
        public static void NormalizeUUIDs(int badUUID, int normalUUID)
        {
            var pool = new Dictionary<int, int>();
            var difference = badUUID - normalUUID;

            // Get pool ----------------------------- //
            var characters = MySQL.QueryRead("SELECT * FROM `characters`");
            foreach (DataRow row in characters.Rows)
            {
                ...
                pool.TryAdd(uuid, uuid - difference);
            }
            ...
            // Accounts db -------------------------- //
            void FixAccounts(int slot)
            {
                foreach (var item in pool)
                {
                    MySQL.Query($"UPDATE `accounts` SET `character{slot}`={item.Value} WHERE `character{slot}`={item.Key};");
                }
            }
            ...
            // Other db ----------------------------- //
            void FixOtherDb(string dbName, string columnName = "uuid")
            {
                foreach (var item in pool)
                {
                    MySQL.Query($"UPDATE `{dbName}` SET `{columnName}`={item.Value} WHERE `{columnName}`={item.Key};");
                }
            }

            FixOtherDb("characters");
            FixOtherDb("customization");
            FixOtherDb("chatcfg");
            FixOtherDb("bindcfg");
            FixOtherDb("businesshistory");
            FixOtherDb("admins");
            FixOtherDb("banned");
            FixOtherDb("fracranks");
            FixOtherDb("fractionlogs");
            FixOtherDb("orgranks");
            FixOtherDb("orglogs");
            FixOtherDb("player_armors");
            FixOtherDb("phoneinfo");
            FixOtherDb("phonemessage", "fromUuid");
            FixOtherDb("phonemessage", "toUuid");
            FixOtherDb("phonetinder");
            FixOtherDb("auctions", "createUUID");
            FixOtherDb("auctions", "lastBetUUID");
            FixOtherDb("lottery_players", "player");
            FixOtherDb("reports", "characterId");
            // -------------------------------------- //

            // InventoryItems db -------------------- //
            foreach (var item in pool)
            {
                MySQL.Query($"UPDATE `items_data` SET `data_id`='char_{item.Value}' WHERE `data_id`='char_{item.Key}';");
            }
            // -------------------------------------- //

            // Set new AUTO_INCREMENT --------------- //
            MySQL.Query($"ALTER TABLE `characters` AUTO_INCREMENT = {pool.Values.Max() + 1};");
            // -------------------------------------- //

            foreach (var uuid in pool)
            {
                Console.WriteLine($"[ETERNAL] Normalize UUID | Changed #{uuid.Key} => #{uuid.Value}");
            }
        }
        // ========================================================================== //
        ...
        public static void ClientEvent_Spawn(ExtPlayer player, string id)
        {
            try
            {
                var sessionData = player.GetSessionData();
                ...
                Vector3 playerPosition = characterData.SpawnPos;
                ...
                if (characterData.ArrestTime > 0)
                {
                    if (sessionData.TimersData.ArrestTimer == null)
                    {
                        ...
                        if (characterData.ArrestType == 1) playerPosition = Sheriff.FirstPrisonPosition;
                        else if (characterData.ArrestType == 2) playerPosition = Sheriff.SecondPrisonPosition;
                        else playerPosition = Police.PrisonPosition;
                    }
                }
                else if (characterData.DemorganTime > 0)
                {
                    if (sessionData.TimersData.ArrestTimer == null)
                    {
                        ...
                        playerPosition = Admin.DemorganPositions[rnd.Next(3)] + new Vector3(0, 0, 1.5);
                ...
                else
                {
                    ...
                    {
                        case "last":
                            playerPosition = characterData.SpawnPos;
                            break;
                        case "organization":
                            var fractionId = player.GetFractionId();
                            if (fractionId != (int)Fractions.Models.Fractions.None)
                            {
                                playerPosition = Manager.FractionSpawns[fractionId];

                                Chars.Repository.RemoveAllIllegalStuff(player);
                            }
                            else
                                playerPosition = characterData.SpawnPos + new Vector3(0, 0, 1.5); ;
                            break;
                        case "family":
                            ...
                            if (organizationData != null)
                            {
                                if (organizationData.BlipId != -1)
                                    playerPosition = organizationData.BlipPosition;
                                else
                                    playerPosition = new Vector3(-774.045, 311.2569, 85.70606);
                        ...
                        case "house":
                            if (house != null)
                            {
                                playerPosition = house.Position + new Vector3(0, 0, 1.5);
                                //Chars.Repository.RemoveAllWeapons(player, true, true, armour: true);
                                Chars.Repository.RemoveAllIllegalStuff(player);
                            }
                            else if (characterData.HotelID != -1) playerPosition = Hotel.HotelEnters[characterData.HotelID] + new Vector3(0, 0, 1.12);
                            else
                            {
                                playerPosition = characterData.SpawnPos;
                            }
        ...
        public void ClientEvent_inputCallback(ExtPlayer player, params object[] arguments)
        {
            try
            {
                ...
                {
                    case "ETERNAL_DICE":
                        if (!int.TryParse(text, out int diceBet))
                        {
                        ...
                        var dicePlayer = sessionData.SelectData.SelectedPlayer;
                        if (dicePlayer is null || player.Position.DistanceTo(dicePlayer.Position) > 10)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Игрок слишком далеко", 3000);
                            return;
                        }

                        EternalDev.Dice.Controller.SendRequest(player, dicePlayer, diceBet);
                        return;
                    ...
                    case "streetrace":
                        ...
                        {
                            Trigger.ClientEvent(player, "openInput", "Уличная гонка", LangFunc.GetText(LangType.Ru, DataName.InputBet0100k), 12, "streetrace");
                            return;
                        }
                    ...
                    case "sellcar":
                        ...
                        if (Convert.ToInt64(targetCharacterData.Money) < limit)
                        {
                            sessionData.SellItemData = new SellItemData();
                    ...
                    case "sell_tent":
                        ...
                        if (limit <= 0 || limit > 1_999_999_999)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.Ru, DataName.AmountIncorrect), 3000);
                            return;
                        }
        ...
        public void RemoteEvent_DialogCallback(ExtPlayer player, string callback, bool yes)
        {
            try
            {
                ...
                if (yes)
                {
                    ...
                    {
                        case "ETERNAL_DICE":
                            EternalDev.Dice.Controller.Accept(player);
                            return;
                        case "EXCHANGE_ACCEPT":
                            EternalDev.ExChangeProps.Controller.ConfirmRequest(player);
                            return;
                        case "RepairMyVeh":
                            if (!player.IsInVehicle) return;
                        ...
                        case "DICE_PLAY":
                            {
                                if (sessionData.DiceData.Target != null)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (Convert.ToInt64(characterData.Money) < dicemon)
                                        {
                                        ...
                                        if (Convert.ToInt64(diceplayerCharacterData.Money) < dicemon)
                                        {
                                            diceplayerSessionData.DiceData = new DiceData();
                                            sessionData.DiceData = new DiceData();
                        ...
                        case "CONFIRM_BUY_BODYARMOUR":
                            ...
                            if (Chars.Repository.AddNewItem(player, $"char_{characterData.UUID}", "inventory", ItemId.BodyArmor, 1, "50_False") == -1)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.Ru, DataName.NoSpaceInventory), 3000);
                                return;
                            }
                ...
                else
                {
                    ...
                    {
                        case "ETERNAL_DICE":
                            EternalDev.Dice.Controller.Cancel(player);
                            return;
                        case "EXCHANGE_ACCEPT":
                            EternalDev.ExChangeProps.Controller.RefusalRequest(player);
                            return;
                        case "ORGCAR_SELL_TOGOV":
                        case "CAR_SELL_TOGOV":
                            sessionData.CarSellGov = null;
    ...
    /*    private static void OnlineReward(ExtPlayer player, byte hours, int redbuckses)
        ...
        private void playedMinutesTrigger()
        {
            try
            {
                Events.Christmas.BonusItems.BonusItemsManager.OnEveryMinute();
                EternalDev.ActivityWar.Manager.EveryMinute();
            ...
            try
            {
                ...
                if (accountData.RefferalId != 0 && !accountData.RefPresentGet)
                {
                    if (characterData.LVL >= 3)
                    {
                        ...
                        MoneySystem.Wallet.Change(player, 50000000);
                        ...
                        UpdateData.RedBucks(player, 45000, msg: "Подарок за ввод реф-кода");
                        ...
                        Trigger.SetTask(async () =>
                        {
                            try
                            {
                                ...
                                if (targetAccountData != null)
                                {
                                    ...
                                    {
                                        successText += $" Majestic Coins (+{MoneySystem.Wallet.Format(refferalData.RB)}MC);";
                                ...
                                else
                                {
                                    ...
                                    if (character != null)
                                    {
                                        await db.Characters
                                            ...
                                            .Set(c => Convert.ToInt64(c.Money), Convert.ToInt64(character.Money) + Convert.ToInt64(refferalData.Money))
                                            .UpdateAsync();
                                    }
                                }
                            }
                ...
                foreach (ExtPlayer foreachPlayer in Character.Repository.GetPlayers())
                {
                    try
                    {
                        ...
                        if (DateTime.Now.Hour == 0)
                        {
                            ...
                            using (StreamWriter CoinsLogs = new StreamWriter("Logs/Coins.txt", true, Encoding.UTF8))
                            {
                                DateTime Time2 = DateTime.Now;
                                CoinsLogs.Write(Time2 + $" [System] У всех игроков обнулён таймер коинов!\r\n");
                                CoinsLogs.Close();
                            }
                            NAPI.Task.Run(() =>
                            {
                                AleCoins.AleCoinsStart(foreachPlayer);
                            }, 1100);
                        }
                      ...
                      //  NAPI.Chat.SendChatMessageToPlayer(foreachPlayer, $"Вы получили {payment}"); // todo поменять на свой текст, payment -  кол-во денег
                        ...
                        CalendarTasks.Repository.AddProgress(foreachPlayer, taskId: 1);

                       
                        Achievements.Repository.AddProgress(foreachPlayer, 1);
                        if (DateTime.Now.Hour >= 5 && DateTime.Now.Hour < 8)
                            Achievements.Repository.AddProgress(foreachPlayer, 2);
                        // Пособие по безработице/зарплата:
                        Achievements.Repository.AddProgress(foreachPlayer, 21);
                                ...
                                Trigger.SetTask(async () =>
                                {
                                    try
                                    {
                                        ...
                                        if (character != null)
                                        {
                                            ...
                                            await db.Characters
                                                .Where(c => c.Uuid == character.Uuid)
                                                .Set(c => Convert.ToInt64(c.Money), Convert.ToInt64(character.Money) + Convert.ToInt32(biz.SellPrice * 0.8))
        ...
        private static void callback_mayormenu(ExtPlayer player, object listItem) /// Никитос Чини 
        {
            try
            {
                ...
                {
                    case "take":
                        Trigger.ClientEvent(player, "openInput", "Получить деньги из казны", "Количество", 12, "mayor_take");
                        return;
                    case "put":
                        Trigger.ClientEvent(player, "openInput", "Положить деньги в казну", "Количество", 12, "mayor_put");
                        return;
                    case "fuelcontrol":
        ...
        internal static void ClientEvent_Spawn(ExtPlayer player, int spawnid)
        {
        ...
        public static void PlayAnimation(ExtPlayer player, string animDict, string animName, int flag, bool share = true, string attachmentName = null)
        {
            try
            {
                ...
                if (Thread.CurrentThread.Name == "Main")
                {
                    ...
                    Sounds.PlayPlayer3d(player, $"cloud/sound/animations/{animName}.ogg", new SoundData
                    {
                        looped = true,
                        loop = true,
                        fade = 1000,
