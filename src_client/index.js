/*mp.events._callRemote = callRemote;

callRemote = (eventName, ...handler) => {
...
global.playerPersonalId = global.localplayer.getVariable("Personal_ID");
...
require('./AleSystems/fractionCraft.js');

require('./MontanaDEV/index.js');
...
require('./player/crashgames.js');
require('./player/wheelgame.js');
require('./player/jackpotgame.js');


require('./player/menus.js');
require('./player/petshop.js');
require('./player/render.js');
require("./player/armorManager.js");
...
require('./vehicle/speedLimits.js');
require('./vehicle/vehicleTitles.js');
...
require('./player/arrestSuspect.js');
require('./main.js');

require('./casino');
require ('./christmas.js');
...
require('./gta5devmenu/index.js');



require('./jobs/builder.js');
...
gm.events.add('setFriendList', function (clear, friendlist) 
{
	...
	{
		callRemote("client_trycatch", "index", "setFriendList", e.toString());
	}
})
...
gm.events.add('setFriend', function (friend, fullname) 
{
	...
	{
		callRemote("client_trycatch", "index", "setFriend", e.toString());
	}
})
...
gm.events.add('petinhouse', (petName, petX, petY, petZ, petC, Dimension) => {
	...
	{
		callRemote("client_trycatch", "index", "petinhouse", e.toString());
	}
});
...
global.loadModel = (requiredModel) => new Promise(async (resolve, reject) => {
    ...
    {
        callRemote("client_trycatch", "events/airdrop", "loadModel", e.toString());
        resolve(false);
    }
});
...
global.isAttached = (entity) => new Promise(async (resolve, reject) => {
    ...
    {
        callRemote("client_trycatch", "events/airdrop", "isAttached", e.toString());
        resolve(false);
    }
});
...
gm.events.add("hud.info", (title, desc, header, image) => {
	...
	call("sounds.playInterface", "cloud/sound/missionComplite.ogg", 0.005);
});
...
delete mp.events.call;
delete mp.events.callRemote;
delete mp.events.callRemoteUnreliable;
delete mp.browsers;
delete mp._events.callRemote;
delete mp._events.callRemoteUnreliable;
delete mp._events.call;
