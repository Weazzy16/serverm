...
gm.events.add("render", function () {
	...
	catch (e) 
	{
		...
		callRemote("client_trycatch", "inventory/objectEditor", "render", e.toString());
	}
});
...
gm.events.add('click', (x, y, upOrDown, leftOrRight, relativeX, relativeY, worldPosition, hitEntity) => {
	try
	{
		...
		if (leftOrRight == 'left' && global.localplayer.isInWater()) return call('notify', 1, 9, translateText("Нельзя устанавливать объекты здесь"), 3000);
		else if (objectEditor.type === editorType.exact) {
			if (upOrDown == 'up' && leftOrRight === 'right') {
				call('client.dropinfo.mod');
			}
			return;
		}
		//if (global.isInSafeZone) return call('notify', 1, 9, translateText("Нельзя ставить в зеленой зоне"), 3000);
		if (leftOrRight == 'left' && mp.objects.exists(objectEditor.entity)) {
			call('client.dropinfo.enter');
			return;
		}

		call('client.dropinfo.close');

	}
	catch (e) 
	{
		callRemote("client_trycatch", "inventory/objectEditor", "click", e.toString());
	}
});
...
gm.events.add('client.dropinfo.enter', () => {
	...
	call('client.dropinfo.close', true);
});
...
gm.events.add('client.dropinfo.close', (isEnter = false) => {
	...
	// call('notify', 1, 9, translateText("Нельзя ставить в зеленой зоне"), 3000);
...
mp.keys.bind(global.Keys.VK_DOWN, true, () => {
	...
	{
		callRemote("client_trycatch", "inventory/objectEditor", "VK_DOWN", e.toString());
	}
});
...
mp.keys.bind(global.Keys.VK_UP, true, () => {
	...
	{
		callRemote("client_trycatch", "inventory/objectEditor", "VK_UP", e.toString());
	}
});
...
mp.keys.bind(global.Keys.VK_RIGHT, true, () => {
	...
	{
		callRemote("client_trycatch", "inventory/objectEditor", "VK_RIGHT", e.toString());
	}
});
...
mp.keys.bind(global.Keys.VK_LEFT, true, () => {
	...
	{
		callRemote("client_trycatch", "inventory/objectEditor", "VK_LEFT", e.toString());
	}
});
...
global.OnPetEditor = (model, arrayObject = [], call = null, callEsq = null) => {  
	...
	{
		callRemote("client_trycatch", "inventory/objectEditor", "global.OnObjectEditor", e.toString());
	}  
}
...
mp.keys.bind(global.Keys.VK_8, true, () => {
    ...
    global.OnObjectEditor (mp.game.joaat ("ind_prop_firework_01"), data, (pos, rot, index) => {
        callRemote('server.object.finish', data [index], pos.x, pos.y, pos.z, rot, 25000);
    })
});*/
...
gm.events.add('client.editor.start', (objid, save, isDell = false) => {
	try
	{
		...
		global.OnObjectEditor (mp.game.joaat(objid), null, (pos, rot, _) => {
			if (Editor.objid) {
				...
				callRemote('server.editor.save', Editor.objid, Editor.save, pos.x, pos.y, pos.z, rot);
				setTimeout(() => {
					call('client.editor.start', Editor.objid, Editor.save, true);
				}, 250)
			}
		})
	}
	...
	{
		callRemote("client_trycatch", "inventory/dropEditor", "client.editor.start", e.toString());
	}
})