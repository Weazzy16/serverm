...
class Casino {
    ...
    constructor() {
        ...
        mp.keys.bind(global.Keys.VK_LEFT, true, () => {
			try 
			{
				...
				const minBet = rouletteTables[this.selectTable][0] == "vw_prop_casino_roulette_01" ? 500000 : 5000000;
				if (minBet > this.bet) this.bet = minBet;
				mp.gui.emmit(`window.events.callEvent("cef.roullete.bet", ${this.bet})`);
			}
			catch (e) 
			{
				callRemote("client_trycatch", "casino/roulette", "VK_LEFT", e.toString());
			}
        });
        ...
        mp.keys.bind(global.Keys.VK_RIGHT, true, () => {
			try 
			{
				...
				this.bet *= 2;
				const maxBet = rouletteTables[this.selectTable][0] == "vw_prop_casino_roulette_01" ? 5000000 : 15000000;
				if (maxBet < this.bet) this.bet = maxBet;
				mp.gui.emmit(`window.events.callEvent("cef.roullete.bet", ${this.bet})`);
			}
			catch (e) 
			{
				callRemote("client_trycatch", "casino/roulette", "VK_RIGHT", e.toString());
			}
        });
        ...
        gm.events.add('client.roullete.TOGGLE_BET', (value) => {
			...
			{
				callRemote("client_trycatch", "casino/roulette", "client.roullete.TOGGLE_BET", e.toString());
			}
        });
        ...
        gm.events.add('client.roullete.START_GAME', (text) => {
			...
			{
				callRemote("client_trycatch", "casino/roulette", "client.roullete.START_GAME", e.toString());
			}
        });
        ...
        gm.events.add('client.roullete.CHARACTER_OCCUPY_PLACE', async (table, seat, state) => {
			...
			{
				callRemote("client_trycatch", "casino/roulette", "client.roullete.CHARACTER_OCCUPY_PLACE", e.toString());
			}
        });
        ...
        gm.events.add('client.roullete.CHARACTER_LEAVE_PLACE', () => {
			...
			{
				callRemote("client_trycatch", "casino/roulette", "client.roullete.CHARACTER_LEAVE_PLACE", e.toString());
			}
        });
        ...
        gm.events.add('client.roullete.timer', (value) => {
			...
			{
				callRemote("client_trycatch", "casino/roulette", "client.roullete.timer", e.toString());
			}
        });
        ...
        gm.events.add('playerEnterColshape', (shape) => {
			...
			{
				callRemote("client_trycatch", "casino/roulette", "playerEnterColshape", e.toString());
			}
        });
        ...
        gm.events.add('playerExitColshape', (shape) => {
			...
			{
				callRemote("client_trycatch", "casino/roulette", "playerExitColshape", e.toString());
			}
        });
        ...
        gm.events.add('playerDeath', (player) => {
			...
			{
				callRemote("client_trycatch", "casino/roulette", "playerDeath", e.toString());
			}
        });
        ...
        global.binderFunctions.rouletteExit = () => {
			try 
			{
				if (this.selectTable != null && this.isSeat) {
					callRemote("server.roullete.CHARACTER_LEAVE_PLACE");
				}
			}
			catch (e) 
			{
				callRemote("client_trycatch", "casino/roulette", "global.binderFunctions.rouletteExit", e.toString());
			}
        }
    }
    ...
    async startRouletteWheel(text) {
		try 
		{
			...
			await global.wait(1000 * 10);
			callRemote('server.roullete.CLEAR_TABLE');
			if (!this.isCreateCroupierAndTable()) return;
			this.clearMyBetsTableMarkers();
		...
		{
			callRemote("client_trycatch", "casino/roulette", "startRouletteWheel", e.toString());
		}
    }
    DealerClothes (randomNumber, dealerPed) {
		...
		{
			callRemote("client_trycatch", "casino/roulette", "DealerClothes", e.toString());
		}
    }
    ...
    DealerPedVoiceGroup (randomNumber, dealerPed) {
		...
		{
			callRemote("client_trycatch", "casino/roulette", "DealerPedVoiceGroup", e.toString());
		}
    ...
    init() {
		...
		{
			callRemote("client_trycatch", "casino/roulette", "init", e.toString());
		}
    }
    ...
    pressKeyE() {
		try 
		{
			...
			if (this.nearestSeat != null && this.selectTable == null) { // Сесть за стол 
				callRemote("server.roullete.CHARACTER_OCCUPY_PLACE", this.nearestTable, this.nearestSeat);
				return;
			}
		...
		{
			callRemote("client_trycatch", "casino/roulette", "pressKeyE", e.toString());
		}
    }
    ...
    pressKeyH() {
		...
		{
			callRemote("client_trycatch", "casino/roulette", "pressKeyH", e.toString());
		}
    }
    ...
    renderRoulette() {
				...
				if(this.rouletteBetObject != null) {
					...
					if(mp.game.controls.isDisabledControlJustReleased(0, 24) && !mp.gui.cursor.visible) {
						if(this.closestChipSpot != null && this.betState) {
							callRemote("server.roullete.CREATE_BET", this.bet, this.closestChipSpot, 
					...
					if(mp.game.controls.isDisabledControlJustReleased(0, 25) && !mp.gui.cursor.visible && this.betState)  {
						if (this.closestChipSpot != null) 
						{
							...
							callRemote("server.roullete.DESTROY_LAST_BET");
						}
					}
		...
		catch (e) 
		{
			...
			callRemote("client_trycatch", "casino/roulette", "render", e.toString());
		}
    }
    ...
    onPlayerEnterColshape(shape) {
		...
		{
			callRemote("client_trycatch", "casino/roulette", "onPlayerEnterColshape", e.toString());
		}
    }
    ...
    onEntityStreamIn(entity) {
		...
		{
			//callRemote("client_trycatch", "casino/roulette", "onEntityStreamIn", e.toString());
		}
    }
    ...
    onPlayerExitColshape(shape) {
		...
		{
			callRemote("client_trycatch", "casino/roulette", "onPlayerExitColshape", e.toString());
		}
    }
    ...
    onPlayerDeath(player) {
		...
		{
			callRemote("client_trycatch", "casino/roulette", "onPlayerDeath", e.toString());
		}
    }
    ...
    clearBetObject() {
		...
		{
			callRemote("client_trycatch", "casino/roulette", "clearBetObject", e.toString());
		}
    }
    ...
    clearCamera() {
		...
		{
			callRemote("client_trycatch", "casino/roulette", "clearCamera", e.toString());
		}
    }
    ...
    getClosestChipSpot(vector) {
		...
		{
			callRemote("client_trycatch", "casino/roulette", "getClosestChipSpot", e.toString());
		}
    }
	...
	async getChipsSpots() {
		...
		{
			callRemote("client_trycatch", "casino/roulette", "getChipsSpots", e.toString());
		}	
    }
    ...
    clearTableMarkers() {
		...
		{
			callRemote("client_trycatch", "casino/roulette", "clearTableMarkers", e.toString());
		}
    }
    ...
    clearMyBetsTableMarkers() {
		...
		{
			callRemote("client_trycatch", "casino/roulette", "clearMyBetsTableMarkers", e.toString());
		}
    }
    ...
    getCameraHitCoord() {
		...
		{
			callRemote("client_trycatch", "casino/roulette", "getCameraHitCoord", e.toString());
			return null;
		}
    }
