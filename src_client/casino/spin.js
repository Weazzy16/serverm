...
class Spin {
    ...
    constructor() {
        ...
        global.binderFunctions.spinExit = () => {
			try 
			{
				...
				callRemote('server.spin.LEAVE_SLOT');
			...
			{
				callRemote("client_trycatch", "casino/spin", "global.binderFunctions.spinExit", e.toString());
			}
        }
        ...
        gm.events.add('client.spin.OCCUPY_SLOT', async (nearest_spin) => {
			...
			{
				callRemote("client_trycatch", "casino/spin", "client.spin.OCCUPY_SLOT", e.toString());
			}
        });
        ...
        gm.events.add('client.spin.exit', () => {
			...
			{
				callRemote("client_trycatch", "casino/spin", "client.spin.exit", e.toString());
			}
        });
        ...
        gm.events.add('client.spin.LEAVE_SLOT', (nearest_spin) => {
			...
			{
				callRemote("client_trycatch", "casino/spin", "client.spin.LEAVE_SLOT", e.toString());
			}
        });
        ...
        gm.events.add('client.spin.SPIN', (r1, r2, r3) => {
			...
			{
				callRemote("client_trycatch", "casino/spin", "client.spin.SPIN", e.toString());
			}
        });
        ...
        gm.events.add('client.spin.CLEAR_SPIN', () => {
			...
			{
				callRemote("client_trycatch", "casino/spin", "client.spin.CLEAR_SPIN", e.toString());
			}
        });
        ...
        gm.events.add('client.spin.setBet', (value) => {
			try 
			{
				...
				this.bet = value;
				callRemote('server.spin.bet', this.bet);
			}
			catch (e) 
			{
				callRemote("client_trycatch", "casino/spin", "client.spin.setBet", e.toString());
			}
        });
    }
    ...
    getSounds (type) {
		...
		{
			callRemote("client_trycatch", "casino/spin", "getSounds", e.toString());
			return '';
		}
    }
    async spin(r1, r2, r3) {
		...
		{
			callRemote("client_trycatch", "casino/spin", "spin", e.toString());
		}
    }
    ...
    render() {
		...
		catch (e) 
		{
			...
			callRemote("client_trycatch", "casino/spin", "render", e.toString());
		}
    }
    ...
    press_key_e() {
		try 
		{
			...
			if(this.select_spin == null && this.nearest_spin != null) { // Если игрок собирается сесть за автомат
				...
				callRemote('server.spin.OCCUPY_SLOT', this.nearest_spin, position.x, position.y, position.z, slots_positions[this.nearest_spin].rz);
				return;
			}
		}
		catch (e) 
		{
			callRemote("client_trycatch", "casino/spin", "press_key_e", e.toString());
		}
    }
    ...
    on_enter_colshape(shape) {
		...
		{
			callRemote("client_trycatch", "casino/spin", "on_enter_colshape", e.toString());
		}
    }
    ...
    on_leave_colshape(shape) {
		...
		{
			callRemote("client_trycatch", "casino/spin", "on_leave_colshape", e.toString());
		}
    }
    ...
    init() {        
		...
		{
			callRemote("client_trycatch", "casino/spin", "init", e.toString());
		}
    }
}
