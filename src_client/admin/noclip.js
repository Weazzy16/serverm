...
mp.events.addDataHandler("INVISIBLE", (entity, value, oldValue) => {
	...
	{
		callRemote("client_trycatch", "synchronization/state", "INVISIBLE", e.toString());
	}
});
...
global.binderFunctions.noclip = () => {
	try 
	{
		...
		callRemote('invisible', global.fly.flying);
	}
	catch (e) 
	{
		callRemote("client_trycatch", "admin/noclip", "binderFunctions.noclip", e.toString());
	}
};
