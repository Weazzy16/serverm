...
function DestroySPCam() 
{
	...
	{
		callRemote("client_trycatch", "admin/spectate", "DestroySPCam", e.toString());
	}
}
...
gm.events.add("spmode", (target, toggle) => {
	...
	{
		callRemote("client_trycatch", "admin/spectate", "spmode", e.toString());
	}
});
	...
	try 
	{
		if (item instanceof global.UIMenuListItem) {
			if(item.Text == "Spectate") {
				if(item.Index == 0) callRemote("SpectateSelect", false);
				else callRemote("SpectateSelect", true);
			}
		} else if (item instanceof global.UIMenuItem) {
			if(item.Text == "Refresh") call("spmode", sptarget, true);
			else if(item.Text == "Unspectate") callRemote("UnSpectate");
		}
	}
	catch (e) 
	{
		callRemote("client_trycatch", "admin/spectate", "spmenu.ItemSelect", e.toString());
	}
});
...
spmenu.MenuClose.on(() => {
	...
	{
		callRemote("client_trycatch", "admin/spectate", "spmenu.MenuClose", e.toString());
	}
});
