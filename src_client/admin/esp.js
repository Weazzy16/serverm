...
global.binderFunctions.esp = () => {
	try 
	{
		...
		callRemote('saveEspState', global.esptoggle);
	}
	catch (e) 
	{
		callRemote("client_trycatch", "admin/esp", "binderFunctions", e.toString());
	}
...
gm.events.add('setEspState', function (state) {
	...
	{
		callRemote("client_trycatch", "admin/esp", "setEspState", e.toString());
	}
...
gm.events.add('CheckMyVList', function (argument) {
	...
	{
		callRemote("client_trycatch", "admin/esp", "CheckMyVList", e.toString());
	}
