...
const
    clientName = "client.ipad.auction.",
    rpcName = "rpc.ipad.auction.",
    serverName = "server.ipad.auction.";
...
gm.events.add(clientName + "getItem", (index) => {
    callRemote(serverName + "getItem", index);
});
...
gm.events.add(clientName + "add", (text, cameraLink, price) => {
    callRemote(serverName + "add", selectCategory, selectId, text, cameraLink, price);
    mp.gui.emmit(`window.listernEvent ('auction.view', 'Main');`);
    selectId = -1;
});
...
gm.events.add(clientName + "bet", (id, price) => {
    callRemote(serverName + "bet", id, price);
});
