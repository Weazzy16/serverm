...
const updateInterfaceState = () => {
    ...
    let popupData = {
        ...
        isIpadThisFrame: isIpadThisFrame,
...
global.ipadCameraOpen = false;

gm.events.add('camera.open', (_isDofEnabled = true) => {
    isIpadThisFrame = false;
    isFreeMode = false;
    isMode = false;
...
const onScreen = async () => {
    ...
    screenshotsBrowser = browsers.new(`http://screenshots/${nameImg}`);

    onDestroy (true);

    mp.game.audio.playSoundFrontend(-1, "Camera_Shoot", "Ipad_SoundSet_Michael", true);
    return;
}
...
const CreateMobile = () => {
    ...
    nativeInvoke ("_CELL_CAM_DISABLE_THIS_FRAME", isIpadThisFrame);
    nativeInvoke ("0xA2CCBE62CD4C91A4", nativeInvoke ("_SET_MOBILE_PHONE_UNK", dofEnabled));
    callRemote("server.ipad.anim", 1);
}
...
const changeView = async () => {
    //if (mp.keys.isUp(40)/* && global.actionAntiFlood("__smartipadScaleVisibleChange", 500)*/) {
    if (!isMode && mp.game.controls.isControlJustPressed(3, 172)) {
        ...
        //Natives.CellFrontCamActivate (isIpadThisFrame)
        nativeInvoke ("_CELL_CAM_DISABLE_THIS_FRAME", isIpadThisFrame);
