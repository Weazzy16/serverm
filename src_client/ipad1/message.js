...
const onSelectNumber = (number) => {
    ...
    if (!number)
        callRemote(serverName + "sNumber", -1);
    else {
        updateStatus (number);
        callRemote(serverName + "sNumber", number);
    }
}
...
rpc.register(rpcName + "getMessages", () => {
    ...
    messagesData.list.forEach(message => {
        ...
        const contactData = ipadData.contacts [newMessage.Number];
...
const updateStatus = (number) => {
    ...
    if (typeof messagesData.list [index] === "object" && !messagesData.list [index].Status) {
        ...
        callRemote(serverName + "updStatus", number);
    }
}
...
rpc.register(rpcName + "getMessage", (number) => {
    ...
    if (messageData !== -1) {
        ...
        callRemote(serverName + "getMsg", number);
    }
...
gm.events.add(clientName + "sendMsg", (key, text, type) => {
    if (typeof selectNumber === "number") {
        ...
        if (contact && contact.NoSend) {
            call(clientName + "updMsgStatus", selectNumber, key, JSON.stringify(global.DateTime), messageStatus.error);
            return;
        }

        if (contact && contact.IsSystem)
            callRemote(serverName + "sendSystemMsg", selectNumber, key, text, type);
        else
            callRemote(serverName + "sendMsg", selectNumber, key, text, type);
...
gm.events.add(clientName + "updMsgStatus", (number, key, date, status) => {
    ...
    if (typeof message === "object") {
        ...
        if (status === messageStatus.received)
            global.ipadSound ("msgReceived", "messagesent.ogg");
...
gm.events.add(clientName + "msgAdd", (number, text, date, type) => {
    ...
    if (isStatus)
        ...
        call('ipad.notify', number, notifyText, 5);
    }

    updateListData (number, JSON.parse(date), text, type, false, isStatus);


    //global.ipadSound ("msgReceived", "newmessage.ogg");
});
...
const setChatStatus = (number) => {
    if (messagesData.ChatStatus [number]) {
        ...
        if (chatStatus.IsWrite)
            mp.gui.emmit(`window.listernEvent ('ipadChatUpdStatus', 3);`);
        else
            mp.gui.emmit(`window.listernEvent ('ipadChatUpdStatus', ${chatStatus.status});`);
    } else
        mp.gui.emmit(`window.listernEvent ('ipadChatUpdStatus', 0);`);
}


let statusAntiFlood = 0;

gm.events.add(clientName + "getIpadChatStatus", (number) => {
    const contact = global.getContact(number);

    if (contact && contact.IsSystem) {
        mp.gui.emmit(`window.listernEvent ('ipadChatUpdStatus', 1);`);
        return;
    }
    ...
    callRemoteUnreliable(serverName + "getChatStatus", number, status);
});

gm.events.add(clientName + "setIpadChatStatus", (number, status) => {
...
gm.events.add(clientName + "setIpadChatWrite", (number, toggled) => {
    if (typeof messagesData.ChatStatus [number] !== "object")
        messagesData.ChatStatus [number] = {};
...
gm.events.add(clientName + "startWrite", () => {
    if (typeof selectNumber === "number") {
        if (ipadData.blackList.includes (selectNumber))
            return;

        callRemoteUnreliable(serverName + "write", selectNumber, true);
    }
});
...
gm.events.add(clientName + "endWrite", () => {
    ...
    if (typeof selectNumber === "number") {
        ...
        callRemoteUnreliable(serverName + "write", selectNumber, false);
    }
});
...
gm.events.add(clientName + "sendPopupMsg", (number, key, text, type) => {
    ...
    if (contact && contact.NoSend) {
        call(clientName + "updMsgStatus", number, key, JSON.stringify(global.DateTime), messageStatus.error);
        return;
    }

    if (contact && contact.IsSystem)
        callRemote(serverName + "sendSystemMsg", number, key, text, type);
    else
        callRemote(serverName + "sendMsg", number, key, text, type);
});
