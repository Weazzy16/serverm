...
gm.events.add('showMafiaGameMenu', (data, mute_status, vote_status, timesday, action_info, action_time_info) => {
    ...
    {
        callRemote("client_trycatch", "events/mafia_game", "showMafiaGameMenu", e.toString());
    }
});
...
gm.events.add('mafia_closeGameMenu', () => {
    ...
    {
        callRemote("client_trycatch", "events/mafia_game", "mafiaLobbyFunctions_client", e.toString());
    }
});
...
gm.events.add('voteMafiaGame_client', (value) => {
    callRemote('voteMafiaGame', value);
});
...
gm.events.add('openMafiaHelpStartPage', (seconds) => {
    try
    {
        if (!global.mafiaMenuActive) {
            ...
            call("sounds.playInterface", "mafia/start", 0.03);
    ...
    {
        callRemote("client_trycatch", "events/mafia_game", "openMafiaHelpStartPage", e.toString());
    }
});

gm.events.add('mafia_startSoundSpeech', (speech) => {
    call("sounds.stop", mafiaGameInfo.currentSoundId);
    call("sounds.playInterface", `mafia/${speech}`, 0.07);
...
gm.events.add('mafia_createLobby_client', (lobby_name, lobby_price, lobby_password, lobby_map) => {
    callRemote('mafia_createLobby_server', lobby_name, lobby_price, lobby_password, lobby_map);
});

gm.events.add('mafia_joinLobby_client', (lobby_index) => {
    callRemote('mafia_joinLobby_server', lobby_index, 0);
});
...
gm.events.add('mafia_joinPrivateLobby_client', (state, value) => {
    try
    {
        if (state == 1) {
            ...
            callRemote('mafia_joinLobby_server', mafiaGameInfo.joinLobbyIndex, value);
        }
    }
    catch (e) 
    {
        callRemote("client_trycatch", "events/mafia_game", "mafia_joinPrivateLobby_client", e.toString());
    }
});
