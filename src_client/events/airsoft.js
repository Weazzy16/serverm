...
gm.events.add('playerEnterColshape', (shape) => {
    ...
    {
        callRemote("client_trycatch", "events/airsoft", "playerEnterColshape", e.toString());
    }
});
...
gm.events.add('playerExitColshape', (shape) => {
        ...
        if (shape === airsoft.zone_shape && global.inAirsoftLobby !== undefined && global.inAirsoftLobby >= 0) {
            ...
            airsoft.zone_leave_interval = setInterval(() => {
                ...
                if (airsoft.zone_return_seconds >= 1) {
                    ...
                    airsoft.zone_return_show_message = undefined;
                    callRemote('airsoft_respawnPlayer');
    ...
    {
        callRemote("client_trycatch", "events/airsoft", "playerExitColshape", e.toString());
    }
});
...
gm.events.add('airsoft_lobbyMenuHandler', (state, data, gameType = 'airsoft', startSeconds = 120) => {
    try
    {
        if (state == 1 && !global.lobbyMenuActive) { // Подгрузка списка лобби с сервера
            ...
            if (gameType == 'airsoft') callRemote('UpdateServerPlayersInLobbyMenuList', 1);
            else if (gameType == 'mafia') callRemote('MafiaUpdateServerPlayersInLobbyMenuList', 1);
            else if (gameType == 'tanks') callRemote('TanksUpdateServerPlayersInLobbyMenuList', 1);
        }
            ...
            callRemote('UpdateServerPlayersInLobbyMenuList', 2);
            callRemote('MafiaUpdateServerPlayersInLobbyMenuList', 2);
            callRemote('TanksUpdateServerPlayersInLobbyMenuList', 2);
        }
    ...
    {
        callRemote("client_trycatch", "events/airsoft", "airsoft_lobbyMenuHandler", e.toString());
    }
});

gm.events.add('airsoft_createLobby_client', (lobby_name, lobby_price, lobby_password, lobby_mode, lobby_weapon, lobby_map) => {
    callRemote('airsoft_createLobby_server', lobby_name, lobby_price, lobby_password, lobby_mode, lobby_weapon, lobby_map);
});

gm.events.add('airsoft_joinLobby_client', (lobby_index) => {
    callRemote('airsoft_joinLobby_server', lobby_index, 0);
});
...
gm.events.add('airsoft_joinPrivateLobby_client', (state, value) => {
    try
    {
        if (state == 1) {
            ...
            callRemote('airsoft_joinLobby_server', airsoft.joinLobbyIndex, value);
        }
    }
    catch (e) 
    {
        callRemote("client_trycatch", "events/airsoft", "airsoft_joinPrivateLobby_client", e.toString());
    }
});
...
gm.events.add('airsoft_updateLobbyList_client', (data, gameType = 'airsoft') => {
    ...
    {
        callRemote("client_trycatch", "events/airsoft", "airsoft_updateLobbyList_client", e.toString());
    }
});
...
gm.events.add('airsoft_updateStats_client', async (state, value1, value2) => {
    ...
    {
        callRemote("client_trycatch", "events/airsoft", "airsoft_updateStats_client", e.toString());
    }
});
...
gm.events.add('airsoft_updateAreaLimit', (state, coords, radius) => {
    ...
    {
        callRemote("client_trycatch", "events/airsoft", "airsoft_updateAreaLimit", e.toString());
    }
});
...
gm.events.add('airsoft_updateAirsoftLobbyValue', (value) => {
    ...
    {
        callRemote("client_trycatch", "events/airsoft", "airsoft_updateAirsoftLobbyValue", e.toString());
    }
});
