...
gm.events.add('client.events.open', (json) => {
    ...
    {
        callRemote("client_trycatch", "events/festive", "client.events.open", e.toString());
    }
})
...
gm.events.add('createMPWaypoint', (adminValue, posX, posY, adminDimension) => {
    ...
    {
        callRemote("client_trycatch", "events/festive", "createMPWaypoint", e.toString());
    }
});
...
gm.events.add('playerEnterColshape', (shape) => {
	try
	{
		...
		if (shape && shape.festive !== undefined && shape.fId !== undefined && global.selectFestive == -1) {
			...
			if (getRandomInt(1, 10) > 8) call("client.particleEffect", "scr_indep_fireworks", "scr_indep_firework_shotburst", eventsData [shape.eventMusicID].Position, 5000);
			call("sounds.playInterface", "cloud/sound/festive/put2.ogg", 50);
			selectMusic = shape.eventMusicID;
		}
	}
	catch (e) 
    {
        callRemote("client_trycatch", "events/festive", "playerEnterColshape", e.toString());
    }
});
...
gm.events.add('playerExitColshape', (shape) => {
    ...
    {
        callRemote("client_trycatch", "events/festive", "playerExitColshape", e.toString());
    }
});
...
gm.events.add('client.events.confirming', async (count, countItems, countMaxFestive) => {
	try
	{
        ...
        call('hud.event.cool', "Мероприятие", "Ого!", translateText("Вы собрали секретный подарок и получили {0} коин{1}!", count, prefix), "png", 10000);
        if (countItems !== countMaxFestive) call('hud.event.cool', "Мероприятие", "Ого!", translateText("Отлично! Вам осталось собрать {0} подарков. Продолжайте поиски!", countMaxFestive - countItems), "png", 10000);
        else call('hud.event.cool', "Мероприятие", "Ого!", translateText("Поздравляем, вы собрали абсолютно все существующие подарков на карте. Вы действительно чемпион с сильными нервами!"), "png", 10000);
        global.StopSound ("cloud/sound/festive/put.ogg");
        call("sounds.playInterface", "cloud/sound/festive/bring2.ogg", 50);
        if (global.selectFestive !== -1) {
            const eData = eventsData [global.selectFestive.fIndex];
            if (eData) {
                call("client.particleEffect", "scr_indep_fireworks", "scr_indep_firework_fountain", eData.Position, 7500);
                if (eData.shape)
                    eData.shape.destroy();
    ...
    {
        callRemote("client_trycatch", "events/festive", "client.events.confirming", e.toString());
    }
})
...
gm.events.add('client.events.buyItem', (index) => {
	try
	{
		...
		callRemote('sever.events.buyItem', index)
	}
	catch (e) 
    {
        callRemote("client_trycatch", "events/festive", "client.events.buyItem", e.toString());
    }
})
