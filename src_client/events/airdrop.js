    ...
    try
    {
        ...
        setTimeout(() => {
            ...
            if (_fightData) {
                ...
                position.z -= 0.3;
                call("client.particleEffect", "core", "exp_grd_flare", position, 30000);
            }
        }, 1000 * (8.33 * time));
    }
    catch (e) 
    {
        callRemote("client_trycatch", "events/airdrop", "client.fight.create", e.toString());
    }
});
...
gm.events.add("client.fight.dell", (id) => {
    ...
    {
        callRemote("client_trycatch", "events/airdrop", "client.fight.dell", e.toString());
    }
});
...
gm.events.add("airdrop.updateTeamsInfo", (stats) => {
    ...
    {
        callRemote("client_trycatch", "events/airdrop", "airdrop.updateTeamsInfo", e.toString());
    }
});
...
gm.events.add('playerEnterColshape', (shape) => {
    if (shape && shape.FightId !== undefined && global.selectFightId === -1) {
        ...
        callRemote("CheckAirdropLockStatus", global.selectFightId);
    }
});
    ...
    if (state == 1) {
        ...
        airdropLock.interval = setInterval(() => {
            ...
            if (airdropLock.health <= 0) {
                ...
                callRemote('AirdropChangeLockStatus', global.selectFightId, airdropLock.health);
            }
        }, 1000);
    }
    else if (state == 2) {
...
gm.events.add("render", () => {
    if (global.selectFightId !== -1 && global.canHackAirdrop === true && mp.game.controls.isControlJustPressed(0, 38)) {
        callRemote('server.fight.player.start.hack', global.selectFightId);
    }

    if (global.canHackAirdrop === true && mp.game.controls.isControlJustReleased(0, 38)) {
        call('airdrop_hackStatus', 2);
    }
});
...
gm.events.add("client.airdropZone.create", (position, radius, color, airdrop_minutes) => {
    try
    {
        ...
        call("hud.event", "AirDrop", "Внимание!", `AirDrop будет сброшен через ${airdrop_minutes} мин`, "png", 30 * 1000);

        //_subTitle, _title, _desc, _image, timeWait
    }
    catch (e) 
    {
        callRemote("client_trycatch", "events/airdrop", "client.airdropZone.create", e.toString());
    }
});
...
gm.events.add("client.blipZone.remove", () => {
    ...
    {
        callRemote("client_trycatch", "events/airdrop", "client.blipZone.remove", e.toString());
    }
});
...
gm.events.add('playerEnterColshape', (shape) => {
    if (shape === airdrop.zone_shape) {
        ...
        callRemote('AirdropChangePlayerDimension', 1);
    }
});
...
gm.events.add('playerExitColshape', (shape) => {
    if (shape === airdrop.zone_shape) {
        ...
        callRemote('AirdropChangePlayerDimension', 2);
    }
});
