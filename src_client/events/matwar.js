...
gm.events.add("client.matwar.fight.create", async (id, propId, position, time) => {
    try
    {
        call("client.matwar.fight.dell", id);
    ...
    {
        callRemote("client_trycatch", "events/matwar", "client.matwar.fight.create", e.toString());
    }
});
...
gm.events.add("client.matwar.fight.dell", (id) => {
    ...
    {
        callRemote("client_trycatch", "events/matwar", "client.matwar.fight.dell", e.toString());
    }
});
...
gm.events.add('playerEnterColshape', (shape) => {
	try
	{
		if (shape && shape.MatWarFightId !== undefined && global.selectMatwarFightId === -1) {
            ...
            callRemote("CheckMatWarDropLockStatus", global.selectMatwarFightId);
            gm.discord(translateText("Играет MatWar"));
		}
	}
	catch (e) 
    {
        callRemote("client_trycatch", "events/matwar", "playerEnterColshape", e.toString());
    }
});
...
gm.events.add('playerExitColshape', (shape) => {
    ...
    {
        callRemote("client_trycatch", "events/matwar", "playerExitColshape", e.toString());
    }
});
...
gm.events.add("client.updateMatWarHackStatus", (status, health) => {
    ...
    {
        callRemote("client_trycatch", "events/matwar", "client.updateMatWarHackStatus", e.toString());
    }
});
    ...
    if (state == 1) {
        ...
        matWarDropLock.interval = setInterval(() => {
            ...
            if (matWarDropLock.health <= 0) {
                ...
                callRemote('MatWarDropChangeLockStatus', global.selectMatwarFightId, matWarDropLock.health);
            }
        }, 1000);
    }
    else if (state == 2) {
...
gm.events.add("render", () =>  {
    if (global.selectMatwarFightId !== -1 && global.canHackMatWarDrop === true && mp.game.controls.isControlJustPressed(0, 38)) {
        callRemote('server.matWar.fight.player.start.hack', global.selectMatwarFightId);
    }

    if (global.canHackMatWarDrop === true && mp.game.controls.isControlJustReleased(0, 38)) {
        call('matWarDrop_hackStatus', 2);
    }
});