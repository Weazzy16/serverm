...
gm.events.add('client.streetrace.position', async () => {
	try
	{
		...
		call('notify', 3, 9, translateText("Поставьте метку на карте!"), 3000);
		getBlipPosition((position) => {
			callRemote('server.streetrace.position', position.x, position.y, position.z);
		});
		
	}
	catch (e) 
	{
		callRemote("client_trycatch", "vehicle/race_system", "sendRaceOffer_client", e.toString());
	}
});
...
const getBlipPosition = (callback) => {
		...
		let intervalId = setInterval(() => 
		{
			...
			if(returnPosition !== null) {
				...
				if (dist > 50.0) {	
					...
					call('notify', 3, 9, translateText("Вы не молжете поставить так близко к себе!"), 3000);
					clearInterval (intervalId);
				}
			}
		}, 250);
	...
	{
		callRemote("client_trycatch", "vehicle/race_system", "getBlipPosition", e.toString());
		return undefined;
	}
}
...
gm.events.add('client.streetrace.start', (sr_X, sr_Y, sr_Z, isCreated) => {
	...
	{
		callRemote("client_trycatch", "vehicle/race_system", "acceptRaceOffer_client", e.toString());
	}
});
...
gm.events.add('playerEnterColshape', (shape) => {
	try
	{
		if (shape === race_system.colshape && race_system.started) {
			callRemote('server.streetrace.finished');
			call('client.streetrace.clear');
		}
	}
	catch (e) 
	{
		callRemote("client_trycatch", "vehicle/race_system", "playerEnterColshape", e.toString());
	}
});
...
gm.events.add('client.streetrace.clear', () => {
	...
	{
		callRemote("client_trycatch", "vehicle/race_system", "clearRaceInfo", e.toString());
	}
});
...
function clearPlayerInfo() {
	...
	{
		callRemote("client_trycatch", "vehicle/race_system", "clearPlayerInfo", e.toString());
	}
}