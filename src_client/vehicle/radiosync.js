...
function vehradio(entity) {
		...
		if (entity && mp.vehicles.exists(entity)) {
			if (global.localplayer.vehicle == entity) {
				...
				if (entity.getPedInSeat(-1) == global.localplayer.handle) {
					if (vehrad != nowplaying) callRemote('VehStream_RadioChange', nowplaying);
				} else {
	...
	catch (e) 
	{
		...
		callRemote("client_trycatch", "vehicle/radiosync", "vehradio", e.toString());
	}
};
...
gm.events.add("playerEnterVehicle", (entity, seat) => {
	...
	{
		callRemote("client_trycatch", "vehicle/radiosync", "playerEnterVehicle", e.toString());
	}
});
...
gm.events.add("playerLeaveVehicle", (entity) => {
	...
	{
		callRemote("client_trycatch", "vehicle/radiosync", "playerLeaveVehicle", e.toString());
	}
});