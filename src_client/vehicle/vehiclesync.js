...
var lastdirt;
//var imadriver = false;
// в начале vehiclesync.js
require('./speedLimits.js');

let currentLimitMs = 0;

// Пример корректной обработки данных о уровнях тюнинга на клиенте
mp.events.add('playerEnterVehicle', (veh, seat) => {
  if (seat !== -1) return;

  const baseKmh = global.speedLimits[veh.model] || 0;

  const engineLvl = veh.getVariable('engineLevel') || 0;
  const gearLvl   = veh.getVariable('transmissionLevel') || 0;
  const turboLvl  = veh.getVariable('turboLevel') || 0;

  const maxKmh = baseKmh + engineLvl * 5 + gearLvl * 4 + turboLvl * 2;
  const currentLimitMs = maxKmh / 3.6;

  mp.gui.chat.push(`[MAX] base=${baseKmh} kmh, final=${maxKmh} kmh`);

  veh.setMaxSpeed(currentLimitMs);
  setTimeout(() => {
    if (veh && mp.vehicles.exists(veh)) {
      const mileage = veh.getVariable('MILEAGE') || 0;
      mp.gui.execute(`
        if (window.vehicleState && window.vehicleState.mileage) {
          window.vehicleState.mileage(${mileage});
          console.log('🚗 Initial mileage:', ${mileage});
        }
      `);
    }
  }, 500); // задержка, чтобы CEF успел инициализироваться
});



// Обработчик начальной загрузки пробега
gm.events.add("vehicles.mileage.init", (initialMileage) => {
    console.log('🚗 Initial mileage from server:', initialMileage);
    
    // Отправляем в CEF
    mp.gui.execute(`
        if (window.vehicleState && window.vehicleState.mileage) {
            window.vehicleState.mileage(${initialMileage});
            console.log('📊 Mileage sent to CEF:', ${initialMileage});
        }
    `);
});

// ← ДОБАВЬ: Обработка обновления пробега
gm.events.add("vehicles.mileage.update", (newMileage) => {
    console.log('🚗 Mileage updated from server:', newMileage);
    
    // Отправляем в CEF
    mp.gui.execute(`
        if (window.vehicleState && window.vehicleState.mileage) {
            window.vehicleState.mileage(${newMileage});
            console.log('📊 Mileage updated in CEF:', ${newMileage});
        }
    `);
});

// ← ДОБАВЬ: Следим за изменением SharedData пробега
mp.events.addDataHandler("MILEAGE", (entity, value, oldValue) => {
    try {
        if (entity && mp.vehicles.exists(entity) && entity.type === 'vehicle' && entity.getPedInSeat(-1) === global.localplayer.handle) {
            ...
            console.log('📊 Mileage from SharedData:', mileage);
        ...
        callRemote("client_trycatch", "vehicle/vehiclesync", "MILEAGE", e.toString());
    }
});
...
mp.events.add('render', () => {
  ...
  if (v && currentLimitMs > 0) {
    v.setMaxSpeed(currentLimitMs);
  }
});
...
gm.events.add("VehStream_SetSirenSound", (veh, status) => {
	...
	{
		callRemote("client_trycatch", "vehicle/vehiclesync", "VehStream_SetSirenSound", e.toString());
	}
});
...
gm.events.add("playerEnterVehicle", (vehicle, seat) => {
	...
	{
		callRemote("client_trycatch", "vehicle/vehiclesync", "playerEnterVehicle", e.toString());
	}
});
...
gm.events.add("playerLeaveVehicle", async () => {
	...
	{
		callRemote("client_trycatch", "vehicle/vehiclesync", "playerLeaveVehicle", e.toString());
	}
});
...
function dirtlevel(entity) {
    try {
        if (entity && mp.vehicles.exists(entity) && global.localplayer.vehicle == entity && entity.getPedInSeat(-1) == global.localplayer.handle) {
            call("VehStream_GetVehicleDirtLevel", entity);
        } else if (dirtt != null) {
			clearInterval(dirtt);
			dirtt = null;
        }
	...
	{
		callRemote("client_trycatch", "vehicle/vehiclesync", "dirtlevel", e.toString());
	}
};
    ...
    try {
        if (entity && mp.vehicles.exists(entity) && entity.getPedInSeat(-1) == global.localplayer.handle) {
			...
			if (raznica >= 0.01) {
				...
				lastdirt = newdirt;
				callRemote("VehStream_SetVehicleDirt", entity, newdirt);
            }
        }
    } catch (e) 
	{
		callRemote("client_trycatch", "vehicle/vehiclesync", "VehStream_GetVehicleDirtLevel", e.toString());
	}
});
...
const setVehicleExtraColours = (entity, data) => {
	try {
		...
		callRemote("client_trycatch", "vehicle/vehiclesync", "setVehicleExtraColours", e.toString());
	}
}
...
const setVehicleHeadlights = (entity, data) => {
	try {
		...
		//callRemote("client_trycatch", "vehicle/vehiclesync", "setVehicleHeadlights", e.toString());
	}
}
...
const setVehicleCover = (entity, data) => {
	try {
		...
		callRemote("client_trycatch", "vehicle/vehiclesync", "setVehicleCover", e.toString());
	}
}
...
const setVehicleNeon = (entity, data) => {
	try {
		...
		callRemote("client_trycatch", "vehicle/vehiclesync", "setVehicleNeon", e.toString());
	}
}
...
const setVehicleDirt = (entity, data) => {
	try {
		...
		callRemote("client_trycatch", "vehicle/vehiclesync", "setVehicleDirt", e.toString());
	}
}
...
const setVehicleEngine = (entity, data) => {
	try {
		...
		callRemote("client_trycatch", "vehicle/vehiclesync", "setVehicleEngine", e.toString());
	}
}
...
const setVehicleIL = (entity, data) => {
	try {
		...
		callRemote("client_trycatch", "vehicle/vehiclesync", "setVehicleIL", e.toString());
	}
}
...
const setVehicleLock = (entity, data) => {
	try {
		...
		callRemote("client_trycatch", "vehicle/vehiclesync", "setVehicleLock", e.toString());
	}
}
...
gm.events.add("client.vehicle.door", (vehicle, doorId, status) => {
	...
	{
		callRemote("client_trycatch", "vehicle/vehiclesync", "VehStream_SetLockStatus", e.toString());
	}
});
...
const setVehicleDoor = (entity, data) => {
	try {
		...
		callRemote("client_trycatch", "vehicle/vehiclesync", "setVehicleDoor", e.toString());
	}
}
...
const setVehicleSyncData = (entity) => {
	try {
		...
		callRemote("client_trycatch", "vehicle/vehiclesync", "setVehicleSyncData", e.toString());
	}
}
...
global.getVehicleIndicatorsInfo = (vehicle) => {
	...
	try {
		...
		callRemote("client_trycatch", "vehicle/vehiclesync", "getVehicleIndicatorsInfo", e.toString());
	}
...
const UpdateVehicleEngine = async (vehicle, toggle) => {
	try {
		...
		callRemote("client_trycatch", "vehicle/vehiclesync", "UpdateVehicleEngine", e.toString());
	}
}
...
mp.events.addDataHandler("PETROL", (entity, value, oldValue) => {
	...
	{
		callRemote("client_trycatch", "vehicle/vehiclesync", "PETROL", e.toString());
	}
});
mp.events.addDataHandler("MILEAGE", (entity, value, oldValue) => {
    try {
        if (entity && mp.vehicles.exists(entity) && entity.type === 'vehicle' && entity.getPedInSeat(-1) == global.localplayer.handle) {
            const mileage = Number(value) || 0;
            mp.gui.emmit(`window.vehicleState.mileage(${mileage})`);
        }
    }
    catch (e) {
        callRemote("client_trycatch", "vehicle/vehiclesync", "MILEAGE", e.toString());
...
setInterval(() => {
    try {
		if (global.localplayer.vehicle && global.localplayer.vehicle.getPedInSeat(-1) == global.localplayer.handle && global.localplayer.vehicle.isSirenOn() != global.localplayer.vehicle.getVariable('isSirenOn')) {
			...
			callRemote("VehStream_updateSirenStatus", global.localplayer.vehicle, global.localplayer.vehicle.isSirenOn());
		}
    }
	catch (e) 
	{
		...
		callRemote("client_trycatch", "vehicle/vehiclesync", "sirenInterval", e.toString());
	}
}, 1000);*/
