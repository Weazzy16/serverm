...
gm.events.add('auto', async (act, index) => {
	...
	{
		callRemote("client_trycatch", "vehicle/autoshop", "auto", e.toString());
	}
});
...
gm.events.add('buyAuto', (id) => {
	try
	{
		...
		callRemote(buyMetodName, auto.model, auto.color, id);
		destroyAutoShop ();
	}
	catch (e) 
	{
		callRemote("client_trycatch", "vehicle/autoshop", "buyAuto", e.toString());
	}
});
...
gm.events.add('closeAuto', () => {
	try
	{
		if(new Date().getTime() - global.lastCheck < 50) return; 
		global.lastCheck = new Date().getTime();    
		callRemote('carroomCancel');
		destroyAutoShop ();
	}
	catch (e) 
	{
		callRemote("client_trycatch", "vehicle/autoshop", "closeAuto", e.toString());
	}
});
...
gm.events.add('testDrive', (type) => {
	try
	{
		if(new Date().getTime() - global.lastCheck < 50) return; 
		global.lastCheck = new Date().getTime(); 
		callRemote('testDrive', auto.model, auto.color, type);
		destroyAutoShop ();
	}
	catch (e) 
	{
		callRemote("client_trycatch", "vehicle/autoshop", "testDrive", e.toString());
	}
});
...
gm.events.add('startTestDrive', async (vehicle) => {
	try
	{
		...
		call('notify', 1, 9, translateText("Вы получили транспортное средство на тест-драйв. Тест-драйв будет окончен через 2 минуты или при выходе из транспортного средства."), 10000);
	...
	{
		callRemote("client_trycatch", "vehicle/autoshop", "startTestDrive", e.toString());
	}
});
...
gm.events.add('setIntoVehicle', async (vehicle, seat, callRemote = false) => {
	...
	{
		callRemote("client_trycatch", "vehicle/autoshop", "setIntoVehicle", e.toString());
	}
});
...
const destroyAutoShop = () => {
	...
	{
		callRemote("client_trycatch", "vehicle/autoshop", "destroyAutoShop", e.toString());
	}
};
...
gm.events.add('openAuto', async (models, prices, gosPrices, bagageSlots, _buyMetodName, isDonate = false) => {
        ...
        models.forEach((value, index) => {
            ...
            autoModels = [
                ...autoModels,
                {
                    ...
                    modelName:  value,
                    price:      price,
                    // ↓ добавили сюда scrapPrice
                    scrapPrice: Math.round(price * 0.7),
	...
	{
		callRemote("client_trycatch", "vehicle/autoshop", "openAuto", e.toString());
	}
});