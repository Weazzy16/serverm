...
gm.events.add('client.rentcar.buy', (carId, colorId, hour) => {
    ...
    callRemote('server.rentcar.buy', carId, colorId, hour);
    call('client.rentcar.exit');
});
...
gm.events.add('client.rentcar.func', (func) => {
    callRemote('server.rentcar.func', func);
});
...
gm.events.add("vehicleStreamIn", (entity) => {
    if (!selectRentData.vehicle && entity && entity.remoteId === selectRentData.vehicleId) {
        ...
        call("createBlip", "rentcar", translateText("Аренда"), 225, entity.position, 1.25, 4);
        //call("client.showWithPicture", translateText("Арендодатель"), "Mors Mutual Insurance", translateText("Спасибо за использование наших услуг. Транспортное средство можно найти на парковке. Хорошего дня!"), "CHAR_MP_MORS_MUTUAL");
