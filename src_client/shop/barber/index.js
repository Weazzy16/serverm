...
const onCreateBarberScene = async () => {
	...
	{
		callRemote("client_trycatch", "shop/barber/index", "onCreateBarberScene", e.toString());
	}
}
...
function onBarberFinished() {
	...
	{
		callRemote("client_trycatch", "shop/barber/index", "onBarberFinished", e.toString());
	}
}
...
function destroyEntities() {
	...
	{
		callRemote("client_trycatch", "shop/barber/index", "destroyEntities", e.toString());
	}
}
...
function playVoice(speechName) {
	...
	{
		callRemote("client_trycatch", "shop/barber/index", "playVoice", e.toString());
	}
}
...
function createScene(looped = false) {
	...
	{
		callRemote("client_trycatch", "shop/barber/index", "createScene", e.toString());
		return -1;
	}
}
...
function playPedBarberAnim(keeperAnim, scissorsAnim = undefined, looped = false) {
	...
	{
		callRemote("client_trycatch", "shop/barber/index", "playPedBarberAnim", e.toString());
	}
}
		...
		if (stage === 2) {
			...
			if (sceneId !== -1) {
				...
				if (phase >= 0.3 && phase <= 0.4 && !cutSoundStarted) {
					...
					call("client.barber.updateData", selectToBuyComponent, JSON.stringify (barberData[selectToBuyComponent]));
...
function playBaseAnims() {
	...
	{
		callRemote("client_trycatch", "shop/barber/index", "playBaseAnims", e.toString());
	}
}
...
function playCutAnim(withScissors = true) {
	try
	{
		...
		//call("selectMenu.hide", "barbershop_concrete");
		//camera.setFov(47);
	...
	{
		callRemote("client_trycatch", "shop/barber/index", "playCutAnim", e.toString());
	}
}
...
gm.events.add('client.barber.open', async (price, json) => {
	...
	{
		callRemote("client_trycatch", "shop/barber/index", "client.barber.open", e.toString());
	}
});
...
gm.events.add('client.barber.startAnim', () => {
	...
	{
		callRemote("client_trycatch", "shop/barber/index", "client.barber.startAnim", e.toString());
	}
});
...
gm.events.add('client.barber.update', (act, id, val) => {
	try
	{
		...
		switch (act) {
			case "buy":
				...
				callRemote("server.barber.buy", id, barberData[id][0], barberData[id][1] ? barberData[id][1] : 0);
				break;
			case "style":
	...
	{
		callRemote("client_trycatch", "shop/barber/index", "client.barber.update", e.toString());
	}
});
...
const setDafaultAppearance = (id, data) => {
	...
	{
		callRemote("client_trycatch", "shop/barber/index", "setDafaultAppearance", e.toString());
	}
}
...
gm.events.add('client.barber.close', () => {
	...
	{
		callRemote("client_trycatch", "shop/barber/index", "client.barber.close", e.toString());
	}
});
