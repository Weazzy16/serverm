...
gm.events.add('client.custom.open', async (pPercent, pVehicle, components, _eventId = 0) => {
    ...
    {
        callRemote("client_trycatch", "shop/custom/index", "client.custom.open", e.toString());
    }
});
...
gm.events.add('client.custom.category', (category) => {
    ...
    {
        callRemote("client_trycatch", "shop/custom/index", "client.custom.category", e.toString());
    }
});
...
gm.events.add('client.custom.buy', () => {
    try
    {
        ...
        if (selectCategory === "FrontWheels" && defaultComponents ["WheelsType"] == selectData.otherIndex && defaultComponents ["FrontWheels"] !== undefined && defaultComponents ["FrontWheels"] === selectData.index) return call('notify', 1, 9, translateText("У Вас уже установлена данная модификация"), 3000);
            ...
            defaultComponents [selectCategory] !== undefined && defaultComponents [selectCategory] === selectData.index) return call('notify', 1, 9, translateText("У Вас уже установлена данная модификация"), 3000);
        let text;
        ...
        call('openDialog', 'tuningbuy', text);
    }
    catch (e) 
    {
        callRemote("client_trycatch", "shop/custom/index", "client.custom.buy", e.toString());
    }
});
...
gm.events.add('client.custom.sbuy', (state) => {
    try
    {
        ...
        if (state)
            callRemote(eventList [eventId], selectCategory, selectData.index, selectData.otherIndex, selectData.red, selectData.green, selectData.blue);
    }
    catch (e) 
    {
        callRemote("client_trycatch", "shop/custom/index", "client.custom.sbuy", e.toString());
    }
});
...
const Exit = () => {
    callRemote('server.custom.exit', eventId);
...
const OpenComponentToMod = (category) => {
	...
	{
		callRemote("client_trycatch", "shop/custom/index", "OpenComponentToMod - " + OpenComponentToModFix, e.toString());
	}
}
...
global.setNeonLight = (vehicle, enabledValues) => {
	...
	{
		callRemote("client_trycatch", "shop/custom/index", "setNeonLight", e.toString());
	}
}
