...
global.updateCameraToBone = (bone, entity = null, isBone = false, speed = 500) => {
	...
	{
		callRemote("client_trycatch", "camera/other", "global.updateCameraToBone", e.toString());
	}
}
...
const calculateCameraParamsAndStartInterp = (entity, boneId, boneOffset, azimutMin, azimutMax, speed) => {
	...
	{
		callRemote("client_trycatch", "camera/other", "calculateCameraParamsAndStartInterp", e.toString());
	}
}
...
global.cameraManager.on('stopInterp', (camera) => {
	...
	{
		callRemote("client_trycatch", "camera/other", "global.cameraManager.on", e.toString());
	}
});
...
gm.events.add("render", () => {
	...
	if (cameraInfo.activeMovementCamera) {
		...
		if (mp.game.controls.isControlPressed(0, 237) || cameraInfo.init) {
			...
			//call('notify', 4, 9, "azimuthAngleDeg - " + cameraInfo.azimuthAngleDeg, 3000);
		}
		...
		//    call('notify', 4, 9, "nextCamLocation - 1", 3000);
		...
		//} else {
		//    call('notify', 4, 9, "nextCamLocation - 2", 3000);
...
const startInterpCameraPointing = (from, to, duration) => {
	...
	{
		callRemote("client_trycatch", "camera/other", "startInterpCameraPointing", e.toString());
	}
};
