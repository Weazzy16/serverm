...
rpc.register(rpcName + "getAdsList", () => {
	if (selectedID != null) {
		callRemote(serverName + "take", selectedID);
	}
	selectedID = null;
...
gm.events.add(clientName + "add", (json) => {
	call('notify', 0, 2, translateText("Пришло новое объявление!"), 3000);
...
gm.events.add(clientName + "delete", (index, a) => {
	try
	{
		...
		callRemote(serverName + "delete", index, a);
	}
	catch (e) 
    {
        callRemote("client_trycatch", "fractions/advert", "client.advert.delete", e.toString());
    }
})
...
gm.events.add(clientName + "send", (index, a) => {
	try
	{
		if(new Date().getTime() - global.lastCheck < 50) return; 
		global.lastCheck = new Date().getTime();
		callRemote(serverName + "send", index, a);
	}
	catch (e) 
    {
        callRemote("client_trycatch", "fractions/advert", "client.advert.send", e.toString());
    }
})
...
gm.events.add('client.advert.phone', (index) => {
	try
	{
		if(new Date().getTime() - global.lastCheck < 50) return; 
		global.lastCheck = new Date().getTime();
		callRemote('server.advert.phone', index);
	}
	catch (e) 
    {
        callRemote("client_trycatch", "fractions/advert", "client.advert.phone", e.toString());
    }
})