...
mp.events.addDataHandler("REDNAME", (entity, value, oldValue) => {
	...
	{
		callRemote("client_trycatch", "synchronization/state", "REDNAME", e.toString());
	}
});
...
mp.events.addDataHandler("fraction", (entity, value, oldValue) => {
	try
	{
		if (entity && mp.players.exists(entity) && entity.type === 'player' && entity.remoteId === global.localplayer.remoteId) {
			...
			call('client.charStore.FractionID', value);
			if (!Number (value)) {
				call('client.charStore.FractionLVL', 0);
			}
		}
	}
	catch (e) 
	{
		callRemote("client_trycatch", "synchronization/state", "fraction", e.toString());
	}
});
...
mp.events.addDataHandler("leader", (entity, value, oldValue) => {
	...
	{
		if (entity && mp.players.exists(entity) && entity.type === 'player' && entity.remoteId === global.localplayer.remoteId) {
			call('client.charStore.IsLeader', value);
			global.isLeader = value;
		}
	}
	catch (e) 
	{
		callRemote("client_trycatch", "synchronization/state", "leader", e.toString());
	}
});
...
mp.events.addDataHandler("vmuted", (entity, value, oldValue) => {
	...
	{
		callRemote("client_trycatch", "synchronization/state", "vmuted", e.toString());
	}
});
...
mp.events.addDataHandler("organization", (entity, value, oldValue) => {
	try
	{
		if (entity && mp.players.exists(entity) && entity.type === 'player' && entity.remoteId === global.localplayer.remoteId) {
			...
			call('client.charStore.OrganizationID', value);
			if (!Number (value)) {
				call('client.charStore.OrganizationLVL', 0);
			}
		}
	}
	catch (e) 
	{
		callRemote("client_trycatch", "synchronization/state", "organization", e.toString());
	}
});
...
mp.events.addDataHandler("InDeath", (entity, value, oldValue) => {
	...
	{
		callRemote("client_trycatch", "synchronization/state", "InDeath", e.toString());
	}
});
...
mp.events.addDataHandler("isDeaf", (entity, value, oldValue) => {
	...
	{
		callRemote("client_trycatch", "synchronization/state", "isDeaf", e.toString());
	}
});
...
global.SetWalkStyle = (entity, index) => {
	...
	{
		callRemote("client_trycatch", "synchronization/state", "SetWalkStyle", e.toString());
	}
}
...
global.SetFacialClipset = (entity, index) => {
	...
	{
		callRemote("client_trycatch", "synchronization/state", "SetFacialClipset", e.toString());
	}
}
...
mp.events.addDataHandler("AGM", (entity, value, oldValue) => {
	...
	{
		callRemote("client_trycatch", "synchronization/state", "AGM", e.toString());
	}
});