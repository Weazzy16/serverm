...
class SitClass {
  constructor() {
    ...
    this.objectsInfo = {
      ...
      [mp.game.joaat("hei_heist_stn_benchshort")]: {
        typeFlags: this.defaultData.yMinus | this.defaultData.yPlus,
        zOffset: 0.25,
      }, //73
      ...
      [mp.game.joaat("v_med_bed2")]: {
        typeFlags: this.defaultData.xMinus | this.defaultData.xPlus,
      }, //282 -
    ...
    gm.events.add("client.seat", () => {
      if (this.isCanSitDown()) return this.sitDown();
      else if (this.isCanStandUp()) return this.standUp();
    });
  ...
  sitDown() {
    try {
      ...
      const rotation = Natives.GET_ENTITY_ROTATION(
      ...
      mp.events.callRemote(
        "server.landing.sit",
        getNearSitInfo.position.x,
        getNearSitInfo.position.y,
        getNearSitInfo.position.z,
        rotation.z + getNearSitInfo.angleOffset
      );
      return true;
    } catch (e) {
      mp.events.callRemote(
  ...
  standUp() {
    try {
      if (this.isCanStandUp()) {
        ...
        if (this.currentObject) mp.events.call("hud.oEnter", "Seating");
      }
    } catch (e) {
      mp.events.callRemote(
        "client_trycatch",
  ...
  isNearSitPosition(entity) {
    try {
      ...
      if (!getNearSitInfo) return false;
      return (
        utils.getDistance(mp.players.local.position, getNearSitInfo.position) <
        1
      );
    } catch (e) {
      mp.events.callRemote(
  ...
  getNearSitInfo(entity) {
    try {
      ...
      const model = utils.fixHash(Natives.GET_ENTITY_MODEL(entity)),
        currentObject =
          null !== this.currentObject && undefined !== this.currentObject
            ? this.currentObject
            : this.objectsInfo[model];

      //mp.gui.chat.push(model + " = " + JSON.stringify (this.objectsInfo[model]));
      if (!currentObject) return null;
      const { x: xPos, y: yPos, z: zPos } = mp.players.local.position;
      const worldCoords = Natives.GET_OFFSET_FROM_ENTITY_GIVEN_WORLD_COORDS(
        entity,
        xPos,
        yPos,
        zPos
      ...
      worldCoords.x = global.clamp(
        worldCoords.x,
        dimensions.min.x + xMin,
        dimensions.max.x - xMin
      );
      worldCoords.y = global.clamp(
        worldCoords.y,
        dimensions.min.y + yMin,
        dimensions.max.y - yMin
      );

      const currentZOffset =
          null !==
            (zOffset =
              null == currentObject ? undefined : currentObject.zOffset) &&
          undefined !== zOffset
            ? zOffset
            : 0,
        getInteractionObjectTypeOffsets = this.getInteractionObjectTypeOffsets(
          currentObject,
          worldCoords,
          dimensions
        ),
        getNearestPositionAndOffset = this.getNearestPositionAndOffset(
          entity,
          currentZOffset,
      ...
      if (getNearestPositionAndOffset.offset.x !== worldCoords.x) {
        if (getNearestPositionAndOffset.offset.x <= 0) angleOffset = 90;
        else if (getNearestPositionAndOffset.offset.x > 0) angleOffset -= 90;
      }

      if (getNearestPositionAndOffset.offset.y !== worldCoords.y) {
        if (getNearestPositionAndOffset.offset.y < 0) angleOffset = 180;
      }
  ...
  getNearestPositionAndOffset(entity, zPos, positions) {
    try {
      ...
      positions.forEach((offset) => {
        const worldCoords = Natives.GET_OFFSET_FROM_ENTITY_IN_WORLD_COORDS(
            entity,
            offset.x,
            offset.y,
            offset.z
      ...
      return returnPosition
        ? { position: returnPosition, offset: returnOffset }
        : null;
    } catch (e) {
      mp.events.callRemote(
        "client_trycatch",
  ...
  getInteractionObjectTypeOffsets(currentObject, worldCoords, dimensions) {
    try {
      ...
      if (currentObject.typeFlags & this.defaultData.yMinus)
        returnPosition.push(
          new mp.Vector3(worldCoords.x, -0.2 + dimensions.min.y, diff / 2)
        );

      if (currentObject.typeFlags & this.defaultData.yPlus)
        returnPosition.push(
          new mp.Vector3(worldCoords.x, 0.2 + dimensions.max.y, diff / 2)
        );

      if (currentObject.typeFlags & this.defaultData.xMinus)
        returnPosition.push(
          new mp.Vector3(-0.2 + dimensions.min.x, worldCoords.y, diff / 2)
        );

      if (currentObject.typeFlags & this.defaultData.xPlus)
        returnPosition.push(
          new mp.Vector3(0.2 + dimensions.max.x, worldCoords.y, diff / 2)
        );
  ...
  async start() {
    try {
      ...
      const { x: xPos, y: yPos, z: zPos } = mp.players.local.position,
        ...
        testCapsule = mp.raycasting.testCapsule(
          front_vector,
          right_vector,
          0.5,
          mp.players.local,
      ...
      if (testCapsule) {
        if ("object" == typeof testCapsule.entity) {
          ...
          "number" == typeof testCapsule.entity &&
          testCapsule.entity !== 0 &&
          nativeInvoke("IS_ENTITY_AN_OBJECT", testCapsule.entity)
        ) {
          entity = testCapsule.entity;
          model = global.utils.fixHash(
            nativeInvoke("GET_ENTITY_MODEL", testCapsule.entity)
          );
        }
...
gm.events.add("client.editor.sit", () => {
  try {
    if (objdata && mp.objects.exists(objdata)) objdata.destroy();
...
mp.keys.bind(0x51, true, function () {
  // 0x51 is the Q key
  try {
    if (--selecttestfsd < 0) selecttestfsd = 0;
    ...
    console.error("Keybind error (Q key):", e);
  }
});

mp.keys.bind(0x45, true, function () {
  // 0x45 is the keycode for E
  try {
    if (++selecttestfsd >= testfsd.length) selecttestfsd = testfsd.length - 1;
    ...
    console.error("Keybind error (E key):", e);
  }
});

// LEFT Arrow Key (Decrease zOffset)
mp.keys.bind(0x25, true, function () {
  // 0x25 is the LEFT arrow key
  try {
    ...
    console.error("Keybind error (LEFT key):", e);
  }
});

// RIGHT Arrow Key (Increase zOffset)
mp.keys.bind(0x27, true, function () {
  // 0x27 is the RIGHT arrow key
  try {
    if (!sitData.objectsInfo[testfsd[selecttestfsd]].zOffset)
      sitData.objectsInfo[testfsd[selecttestfsd]].zOffset = 0;
    ...
    console.error("Keybind error (RIGHT key):", e);
  }
});

// UP Arrow Key (Change typeFlags)
let selectsda = 0;
mp.keys.bind(0x26, true, function () {
  // 0x26 is the UP arrow key
  try {
    selectsda--;

    switch (selectsda) {
      case 1:
        sitData.objectsInfo[testfsd[selecttestfsd]].typeFlags =
    ...
    console.error("Keybind error (UP key):", e);
  }
});

// DOWN Arrow Key (Change typeFlags)
mp.keys.bind(0x28, true, function () {
  // 0x28 is the DOWN arrow key
  try {
    selectsda++;
    ...
    console.error("Keybind error (DOWN key):", e);
  }
});
