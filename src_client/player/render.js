...
gm.events.add('CUFFED', function (argument) {
	...
	{
		callRemote("client_trycatch", "player/render", "CUFFED", e.toString());
	}
});
/*
...
gm.events.add('safeZone', function (argument) {
	...
	{
		callRemote("client_trycatch", "player/render", "safeZone", e.toString());
	}
});*/
...
global.binderFunctions.openCircleMenu = () => {// G key
	...
	{
		callRemote("client_trycatch", "player/render", "global.binderFunctions.openCircleMenu", e.toString());
	}
};
...
global.binderFunctions.dropObject = () => {// F2 key
	try 
	{
		...
		if (global.entity != null && global.entity.doesExist() && global.entity.type == 'object' && global.entity.isAnObject()) {				
			if (global.entity['furniture'])
				callRemote('oSelected', global.entity.remoteId);
	...
	{
		callRemote("client_trycatch", "player/render", "global.binderFunctions.dropObject", e.toString());
	}
};
...
const getLookingAtEntity = () => {
	...
	{
		callRemote("client_trycatch", "player/render", "getLookingAtEntity", e.toString());
	}
...
const getNearestLookingObject = () => {
	...
	{
		callRemote("client_trycatch", "player/render", "getNearestLookingObject", e.toString());
	}
}
...
const GetLooking = (isObj = false, isBtn = false) => {
	...
	catch (e) 
	{
		...
		callRemote("client_trycatch", "player/render", "time.5s", e.toString());
	}
	return null;
};
