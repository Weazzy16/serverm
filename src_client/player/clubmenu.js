...
gm.events.add("openAlco", (club, modief, isOwner, stock) => {
		...
		setTimeout(() => {
			...
			alcoUI.ItemSelect.on(item => {
				...
				if (item.Text == translateText("Купить")) {
					callRemote('menu_alco', 0, selectedAlco);
				}
				else if (item.Text == translateText("Взять")) {
					callRemote('menu_alco', 1, selectedAlco);
				}
				else if (item.Text == translateText("Скрафтить")) {
					callRemote('menu_alco', 2, selectedAlco);
				}
	...
	{
		callRemote("client_trycatch", "player/clubmenu", "openAlco", e.toString());
	}
});
...
gm.events.add("render", () => {
	...
	catch (e) 
	{
		...
		callRemote("client_trycatch", "player/clubmenu", "render", e.toString());
	}
});
