...
const selectStartQuests = (entity) => {
    if (global.isNewChar && questNameToPeds [startActorName] && questNameToPeds [startActorName].includes(entity)) {
        ...
        call('notify', 3, 9, translateText("Не забудь подойти к NPC Виталий Дебич, чтобы начать стартовую линейку квестов, которая познакомит тебя с нашим чудесным штатом!"), 3000);
        global.createCamera("peds", entity);
        call('createWaypoint', entity.position.x, entity.position.y);
...
const actorData = {
    ...
    npc_donateautoroom:translateText("Генералиссимусом Юки"),
...
//     if (conversationPed && mp.peds.exists (conversationPed)) {
//         call("sounds.trigger", "onEnd", "ped_" + conversationPed.remoteId);
//     }
// });
...
gm.events.add('client.quest.perform', (isClose) => {
    ...
    callRemote('server.quest.perform', isClose);

    if (isClose)
        call('client.quest.close', false);
});
...
gm.events.add('client.quest.action', (isClose) => {
    ...
    callRemote('server.quest.action', isClose);
...
gm.events.add('client.quest.take', (index = 0) => {
    ...
    callRemote('server.quest.take', index);
...
gm.events.add('client.quest.router', (ActorName) => {
    try {
        ...
        if (ped != null && mp.peds.exists (ped)) {
            call('notify', 2, 9, translateText("Вы поставили метку на карте!"), 3000);
            call('createWaypoint', ped.position.x, ped.position.y);
        }
    }
    catch (e) 
    {
        callRemote("client_trycatch", "quests", "client.quest.router", e.toString());
    }
});
...
gm.events.add('client.quest.selectQuest.Add', (ActorName, isEnd) => {
    ...
    {
        callRemote("client_trycatch", "quests", "client.quest.selectQuest.Add", e.toString());
    }
});
...
gm.events.add('client.quest.selectQuest', (actorName) => {
    callRemote('server.quest.selectQuest', actorName);
});
...
gm.events.add('client.take_quest_item', () => {
    ...
    if ((collecting_items.collected + 1) >= 5) {
        ...
        call('notify', 2, 9, translateText("Вы собрали всех Мишек, отдайте их Бабушке Granny!"), 3000);
        ...
        if (collecting_items.collected == 4) {
            ...
            call('notify', 2, 9, translateText("Вы собрали 4/5 Мишек. Остался последний Мишка. На карте отмечено следующее примерное расположение последнего Мишки!"), 3000);
        } else {
            mp.gui.chat.push(translateText("Вы собрали {0}/5 Мишек, на карте отмечено следующее примерное расположение Мишки!"));
            call('notify', 2, 9, translateText("Вы собрали {0}/5 Мишек, на карте отмечено следующее примерное расположение Мишки!"), 3000);
        }
...
gm.events.add('playerEnterColshape', (shape) => {
    ...
    {
        callRemote("client_trycatch", "events/halloween", "playerEnterColshape2", e.toString());
    }
});
...
gm.events.add('playerExitColshape', (shape) => {
    ...
    {
        callRemote("client_trycatch", "events/halloween", "playerExitColshape2", e.toString());
    }
});
...
gm.events.add('client.create.npc_dfday_mission', (mission_type) => {
    ...
    if (mission_type == 1) {
        ...
        call('notify', 1, 9, translateText("На карте отмечены документы, которые ты должен собрать."), 3000);
    }
});
    ...
    try {
        if (npc_dfday_mission.type == 1) {
            if (npc_dfday_mission.collected > 0) {
                call('notify', 0, 9, translateText("Вы уже начали чинить машину."), 3000);
                return;
            }
            ...
            setTimeout(() => {
                ...
                call('freeze', false);
                call('notify', 2, 9, translateText("Ты починил машину, поговори с Дядюшкой."), 3000);
                callRemote('server.update.npc_dfday_mission');
            }, 10000);
        }
        else if (npc_dfday_mission.type == 2) {
            if ((npc_dfday_mission.collected + 1) >= 5) {
                clearDFDayMissionInfo();
                call('notify', 2, 9, translateText("Вы развезли все документы, поговорите с Эдвардом снова."), 3000);
                call('createWaypoint', 2.710338, -708.8864);
                callRemote('server.update.npc_fd_edward');
                return;
            }
            ...
            call('notify', 2, 9, translateText("Вы привезли документы. Двигайтесь к следующему месту!"), 3000);
        }
    }
    catch (e) 
	{
		callRemote("client_trycatch", "player/npc_dialogs", "client.update.npc_dfday_mission", e.toString());
	}
});
...
function clearDFDayMissionInfo() {
    ...
    {
        callRemote("client_trycatch", "player/npc_dialogs", "clearDFDayMissionInfo", e.toString());
    }
}