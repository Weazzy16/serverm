...
mp._events.add('outgoingDamage', (sourceEntity, targetEntity, sourcePlayer, weapon, boneIndex, damage) => {
    ...
    let alcoholBoost = targetEntity.getVariable("alcohol.boost");
    if (alcoholBoost !== undefined && typeof(alcoholBoost) == 'number' && Number(alcoholBoost) > 0) {
        customDamage = customDamage * (1 - alcoholBoost / 100);
    }
    ...
    if (targetEntity.type === "player") {
        ...
        if (global.hitPoint)
            call("client.addHit", Math.round(customDamage), targetEntity.remoteId, rageBoneId, !!headBone.includes(boneIndex), isDead);

        if (damageData.isPet) {
            callRemote("server.damage.petToPlayer", targetEntity, sourceEntity, customDamage);
    ...
    if (sourceEntity !== global.localplayer && sourceEntity && sourceEntity.type === "ped") {
        if (targetEntity && targetEntity.type === "ped" && targetEntity.isPet) {
            ...
            callRemoteUnreliable("server.damage.petToPet", targetEntity, sourceEntity, customDamage);
            return true;
        }
    ...
    if (targetEntity && targetEntity.type === "ped" && targetEntity.isPet) {
        ...
        callRemoteUnreliable("server.damage.playerToPet", targetEntity, customDamage);
        return true;
    }
});
...
mp._events.add('incomingDamage', (sourceEntity, sourcePlayer, targetEntity, weapon, boneIndex, damage) => {
    ...
    if (targetEntity === mp.players.local) {
        ...
        if (mp.players.exists(sourcePlayer))
            call('client.pet.attack', sourcePlayer.remoteId);
        ...
        let customDamage = global.clamp(damageData.damage, damageData.damage / 2, damageData.damage) * boneDamageMultiplier / damageData.shotCount;
        let alcoholBoost = global.localplayer.getVariable("alcohol.boost");
        if (alcoholBoost !== undefined && typeof(alcoholBoost) == 'number' && Number(alcoholBoost) > 0) {
            customDamage = customDamage * (1 - alcoholBoost / 100);
        }
        customDamage = Math.round(customDamage);
    ...
    if (weaponHash != global.weaponData.weapon) {
        switch (global.weaponData.weapon) {
            ...
            case mp.game.joaat('weapon_snowball') >> 0://126349499:
                ...
                callRemote('server.weapon.remove');
                return;
        }
