...
gm.events.add(
	"client.capture.open",
	(squaresInControl, squaresInfo, myFractionName) => {
	  global.menuOpen();
	  mp.gui.emmit(
		  `window.router.setView("CaptureTablet", [${true}, ${squaresInControl}, '${squaresInfo}', '${myFractionName}']);`
		)
	}
  );
  gm.events.add(
	...
	(squaresInControl, squaresInfo, myFractionName) => {
	  mp.gui.emmit(
		  `window.listernEvent ('cef.capture.load', ${squaresInControl}, '${squaresInfo}', '${myFractionName}');`
		)
	}
  );
  gm.events.add("client.capture.close", () => {
	global.menuClose();
	mp.gui.emmit(`window.router.setHud();`);
  });
  
  gm.events.add("client.capture.getNotifications", (data) =>
	mp.gui.emmit(
	  `window.listernEvent ('cef.capture.getNotifications', '${data}');`
	)
  );
  
  gm.events.add("client.capture.getInfoCapture", (data) =>
	mp.gui.emmit(`window.listernEvent ('cef.capture.getInfoCapture', '${data}');`)
  );
  
  gm.events.add("client.capture.getMembers", (data) =>
	mp.gui.emmit(`window.listernEvent ('cef.capture.getMembers', '${data}');`)
  );
  
  gm.events.add("client.toserver.capture.getMembers", (squareId) =>
	callRemote("server.capture.getMembers", squareId)
  );
  ...
  gm.events.add("client.toserver.capture.getInfoCapture", (squareId) =>
	callRemote("server.capture.getInfoCapture", squareId)
  );
  gm.events.add("client.toserver.capture.cancel", (squareId) =>
	callRemote("server.capture.cancel", squareId)
  );
...
gm.events.add('loadCaptureBlips', function (json) {
	...
	{
		callRemote("client_trycatch", "player/gangzones", "loadCaptureBlips", e.toString());
	}
});
...
gm.events.add('quitcmd', function () {
	try
	{
		setTimeout(function() { callRemote('kickclient') }, 500);
	}
	catch (e) 
	{
		callRemote("client_trycatch", "player/gangzones", "quitcmd", e.toString());
	}
});
