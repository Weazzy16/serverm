...
gm.events.add('client.weapon.give', (weaponHash, ammo, isReload = false, ItemId = 0) => {
    ...
    {
        callRemote("client_trycatch", "player/weapon", "client.weapon.give", e.toString());
    }
});
...
global.binderFunctions.playerReload = () => {// R key
	try {
		...
		callRemote("server.weapon.reload", global.pInt (global.weaponData.ammo));
...
global.binderFunctions.changeweap_1 = () => {// 1 key
	...
	callRemote('changeweap', 1, global.pInt (global.weaponData.ammo));
    global.lastCheck = new Date().getTime();
};
...
gm.events.add('client.weapon.take', (sendback) => {
	try
	{
		if(sendback && global.pInt (global.weaponData.ammo) > 0)
			callRemote("server.weapon.ammoin", global.weaponData.weapon, global.pInt (global.weaponData.ammo));
    ...
    {
        callRemote("client_trycatch", "player/weapon", "client.weapon.take", e.toString());
    }
});
...
gm.events.add('playerWeaponShot', function (targetPosition, targetEntity) {
    ...
    {
        callRemote("client_trycatch", "player/weapon", "playerWeaponShot", e.toString());
    }
});
...
const updateWeaponShotAmmo = () => {
	...
	callRemote('server.weaponShot', countWeaponShotAmmo);
...
gm.events.add("removeAllWeapons", function () {
    ...
    {
        callRemote("client_trycatch", "player/weapon", "removeAllWeapons", e.toString());
    }
});
...
gm.events.add("playerDeath", function (player, reason, killer) {
    ...
    {
        callRemote("client_trycatch", "player/weapon", "playerDeath", e.toString());
    }
});
