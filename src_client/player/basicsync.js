...
gm.events.add("payday", (exp, level, cash) => {
	call('client.charStore.EXP', exp);
	call('client.charStore.LVL', level);
...
gm.events.add('disabledmg', function (player, toggle) {
	try {
		...
		callRemote("client_trycatch", "player/basicsync", "disabledmg", e.toString());
	}
});
	...
	if ([7, 9].includes(global.fractionId)) {
		...
		if (myvehicle && myvehicle.getPedInSeat(-1) == global.localplayer.handle && myvehicle.getClass() == 18) {
			...
			if (vehicleswanted[vehnum] !== undefined) {
				//TODO
				call('notify', 3, 9, translateText("Транспортное средство с номерами ") + vehnum + translateText(" было замечено неподалёку!"), 3000);
			}
		}
	}
});
