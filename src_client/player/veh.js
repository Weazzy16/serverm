...
class PutToPlayerInVehicle {
    ...
    clear (seat = false) {
		...
		{
			callRemote("client_trycatch", "player/veh", "clear", e.toString());
		}
    }
    ...
    delInterval () {
		...
		{
			callRemote("client_trycatch", "player/veh", "delInterval", e.toString());
		}
    }
    ...
    delTimer () {
		...
		{
			callRemote("client_trycatch", "player/veh", "delTimer", e.toString());
		}
    }
    ...
    stop (seat = false) {
		...
		{
			callRemote("client_trycatch", "player/veh", "stop", e.toString());
		}
    }
    ...
    getVehicle () {
		...
		{
			callRemote("client_trycatch", "player/veh", "getVehicle", e.toString());
		}
    }
    ...
    getVehicleDoor (vehicle, playerPos) {
		...
		{
			callRemote("client_trycatch", "player/veh", "getVehicleDoor", e.toString());
			return 0;
		}
    }
    ...
    getVehicleDoorPos (vehicle, playerPos) {
		...
		{
			callRemote("client_trycatch", "player/veh", "getVehicleDoorPos", e.toString());
    ...
    getUnBind() {
		...
		{
			callRemote("client_trycatch", "player/veh", "getUnBind", e.toString());
		}
    }
    ...
    up () {
		...
		{
			callRemote("client_trycatch", "player/veh", "up", e.toString());
		}
    }
    ...
    enter (inDrive) {
		try
		{
			...
			else if(this.inVehicle != null && mp.vehicles.exists(this.inVehicle) && this.dist < this.maxDistEnter && this.inVehicle.getSpeed() < 6.0) 
			{
				...
				{
					this.clear ();
					call('notify', 1, 9, translateText("Чтобы сесть в транспортное средство Вам нужно отменить анимацию."), 3000);
				}
				else if (2 == this.inVehicle.getDoorLockStatus()) {
					this.clear ();
					call('notify', 1, 9, translateText("Двери закрыты"), 1000);
				}
					...
					if(this.doorsId === -1) {
						this.stop ();
						call('notify', 1, 9, translateText("Мест нет"), 1000);
					} else if(this.doorsId !== -1) {
						global.localplayer.clearTasks();
						this.SitToggle = 0;
						this.status = PutToPlayerInVehicleStatus.passenger;
		...
		{
			callRemote("client_trycatch", "player/veh", "enter", e.toString());
		}
    }
    ...
    Interval () {
		...
		{
			callRemote("client_trycatch", "player/veh", "Interval", e.toString());
		}
    }
    ...
    bind (toggled) {
		...
		{
			callRemote("client_trycatch", "player/veh", "bind", e.toString());
		}
    }
}
...
mp.keys.bind(70, false, () => {
    if(global.loggedin === true && !global.menuCheck () && global.chatActive === false/* && global.showhud*/ && global.ANTIANIM != true && !global.attachedtotrunk && !global.localplayer.carryngData) {
        ...
        callRemote('server.vehicle.detachPlayer');
    }
});
...
gm.events.add("render", () => {
	...
	catch (e) 
	{
		...
		callRemote("client_trycatch", "player/veh", "render", e.toString());
	}
});