...
gm.events.add('client.animation.play', (item) => {
    try
    {
        ...
        callRemote("server.animation.play", item, true);
    ...
    {
        callRemote("client_trycatch", "player/animation", "client.animation.play", e.toString());
    }
});
...
gm.events.add('client.animation.stop', () => {
    try
    {
        ...
        callRemote("server.animation.play", -1, true);
    }
    catch (e) 
    {
        callRemote("client_trycatch", "player/animation", "client.animation.stop", e.toString());
    }
});

gm.events.add('client.animation.favorite', (json) => {
	callRemote("server.animation.favorite", json);
});

gm.events.add('client.animation.bind', (json) => {
	callRemote("server.animation.bind", json);
});
...
gm.events.add("playerEnterVehicle", (vehicle, seat) => {
    try 
	{
        ...
        if (isPlay == 1) {
            mp.gui.emmit(`window.UpdateButtonText('', '');`);
	        callRemote("server.animation.play", -1, false);
        }
	}
	catch (e) 
	{
		callRemote("client_trycatch", "player/animation", "playerStartEnterVehicle", e.toString());
	}
});