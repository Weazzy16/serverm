...
gm.events.add("lumberjackJob_setTreeInfo", (index, stage, position, hp) => {
    ...
    {
        callRemote("client_trycatch", "player/lumberjack", "lumberjackJob_setTreeInfo", e.toString());
    }
});
...
gm.events.add("lumberjackJob_updateTreeInfo", (index, stage, hp) => {
    ...
    {
        callRemote("client_trycatch", "player/lumberjack", "lumberjackJob_updateTreeInfo", e.toString());
    }
});
...
gm.events.add("lumberjackJob_startProcess", () => {
    try
    {
        ...
        callRemote('Lumberjack_StartAnimCutTree');
        ...
        lumberjackJob.interval = setInterval(() => {
            if (global.localplayer.vehicle || global.cuffed || global.isDeath === true || global.isDemorgan) {
                call('lumberjackJob_stopProcess');
                return;
            }
            ...
            if (lumberjackJob.process_count >= 4000) {
                ...
                callRemote('Lumberjack_CutTree', lumberjackJob.treeIndex);

                lumberjackJob.restartTimeout = setTimeout(() => {
                    call('lumberjackJob_startProcess');
                }, 15500);
            }
        }, 3300);
    }
    ...
    {
        callRemote("client_trycatch", "player/lumberjack", "lumberjackJob_startProcess", e.toString());
    }
});
...
mp.keys.bind(0x45, true, () => {
    ...
    if (!global.anti_flood || global.anti_flood && new Date().getTime() - global.anti_flood >= 1000) {
        if (lumberjackJob.treeStage == 0 && !global.startedMining) callRemote("Lumberjack_HitTree", lumberjackJob.treeIndex);
        else if (global.startedMining) {
            call("lumberjackJob_stopProcess");
            callRemote("Lumberjack_StopCutTree", lumberjackJob.treeIndex);
        }
        else if (lumberjackJob.treeStage == 1) callRemote("Lumberjack_TakeTimber", lumberjackJob.treeIndex);
    }
...
gm.events.add("render", () => {
    ...
    catch (e)
    {
        ...
        callRemote("client_trycatch", "player/lumberjack", "render", e.toString());
    }
});
