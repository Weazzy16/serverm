...
gm.events.add('client.donate.buyVehNumber', (number) => {
    callRemote('server.donate.buyVehNumber', number);
});

gm.events.add('client.donate.buySim', (number) => {
    callRemote('server.donate.buySim', number);
});
...
gm.events.add('client.donate.close', () => {
    ...
    {
        callRemote("client_trycatch", "player/donatemenu", "client.donate.close", e.toString());
    }
});

gm.events.add('client.donate.buy.set', (id) => {
    callRemote("server.donate.buy.set", id);
});

gm.events.add('client.donate.buy.clothes', (id) => {
    callRemote("server.donate.buy.clothes", id);
});


gm.events.add('client.donate.buy.char', (id, name) => {
    callRemote("server.donate.buy.char", id, name);
});

gm.events.add('client.donate.reward', () => {
    callRemote("server.donate.reward");
});

gm.events.add('client.donate.buy.premium', (id) => {
    callRemote("server.donate.buy.premium", id);
});

gm.events.add('client.donate.change', (amount) => {
    callRemote("server.donate.change", amount);
});
...
gm.events.add('donbuy', (id, data, surname) => {
    try
    {
        if(id === 1)
        {
            ...
            callRemote("donate", id, newname);
        }
        else if(id === 999) {
            if(data.length <= 0 || data.length > 50) {
                call('notify', 1, 9, translateText("Неправильный формат логина"), 3000);
                return;
            }
            if(surname < 25 || surname > 9999) {
                call('notify', 1, 9, translateText("За раз можно отправить от 25 до 9.999 RedBucks"), 3000);
                return;
            }
            callRemote("giftdonate", data, surname);
        } else callRemote("donate", id, data);
    ...
    {
        callRemote("client_trycatch", "player/donatemenu", "donbuy", e.toString());
    }
});
...
gm.events.add('client.sapper.bet', (value) => {
    callRemote('server.sapper.bet', value);
});
...
gm.events.add('client.sapper.end', (value) => {
    callRemote('server.sapper.end', value);
});
...
gm.events.add('client.opendonatesite', (url) => {
    try
    {
        ...
        donateBrowsers = browsers.new(url);
        mp.gui.cursor.show(true, true);
    }
    catch (e) 
    {
        callRemote("client_trycatch", "player/donatemenu", "client.opendonatesite", e.toString());
    }
});
...
gm.events.add('client.closedonatesite', () => {
    ...
    {
        callRemote("client_trycatch", "player/donatemenu", "client.closedonatesite", e.toString());
    }
});
...
gm.events.add('client.donatepack.open', (id) => {
    callRemote('server.donatepack.open', id);
});
...
gm.events.add('client.donatepack.rb', (id) => {
    callRemote('server.donatepack.rb', id);
});

gm.events.add('client.donatepack.donate', (id) => {
    callRemote('server.donatepack.donate', id);
});
...
gm.events.add('client.donate.roulette.load', () => {
    callRemote('server.donate.roulette.load');
});
...
const getCasesData = (item) => {
    let returnItem = {
        ...
        name: item[3],
        desc: item[4],
        startCase: item[5] ? new Date(item[5]).getTime() : 0,
        endCase:   item[6] ? new Date(item[6]).getTime() : Infinity,
        items: [],
    }

    let items = [];
    item[7].forEach((itemData) => {
        items.push({
            title: itemData[0],
            image: itemData[1],
...
gm.events.add('client.donate.roulette.loadCase', (caseId) => {
    callRemote('server.donate.roulette.loadCase', caseId);
});
