...
gm.events.add("client.create.setSlot", (slot) => {
    currentSlot = slot;
});
...
gm.events.add('client.characters.customization.updateIndex', (index) => {
    try {
        ...
        callRemote("client_trycatch", "player/character", "updateIndex", e.toString());
    }
});
...
gm.events.add('client.characters.customization.UpdateGender', (gender) => {
    try {
        ...
        callRemote("client_trycatch", "player/character", "clUpdateGender", e.toString());
    }
});
...
gm.events.add('client.characters.customization.UpdateParents', (gender, motherId, fatherId) => {
    try {
		...
		callRemote("client_trycatch", "player/character", "clUpdateParents", e.toString());
	}
});
...
gm.events.add('client.characters.customization.UpdateShapeMix', (gender, shapemix) => {
    try {
        ...
        callRemote("client_trycatch", "player/character", "clUpdateShapeMix", e.toString());
    }
});
...
gm.events.add('client.characters.customization.UpdateSkinTone', (gender, skintone) => {
    try {
        ...
        callRemote("client_trycatch", "player/character", "clUpdateSkinTone", e.toString());
    }
});
...
gm.events.add('client.characters.customization.UpdateAppearance', (gender, key, id, color, opacity) => {
    try {
        ...
        callRemote("client_trycatch", "player/character", "clUpdateAppearance", e.toString());
    }
});
...
gm.events.add('client.characters.customization.UpdateCharacteristic', (gender, xindex, x, yindex, y) => {
    try {
        ...
        callRemote("client_trycatch", "player/character", "clUpdateCharacteristic", e.toString());
    }
});
...
gm.events.add('client.characters.customization.UpdateClothes', (gender, key, drawable, texture) => {
    try {
        ...
        callRemote("client_trycatch", "player/character", "clUpdateClothes", e.toString());
    }
});
...
gm.events.add('initCustomizationCharsData', (customizationsData, clothes, accessory, charsSlot) => {
    try {
        ...
        callRemote("client_trycatch", "player/character", "CreatorCamera", e.toString());
    }
});
    ...
    try {
        ...
        if (!isCreate) {
            ...
            for (let i = 0; i < maxSlots; i++) {
                const scenarioData = currentScenario.characters[i + 1];
                if (customizationCharsData.charsSlot [i] > 0) {
                    const ped = mp.peds.new(mp.game.joaat('mp_m_freemode_01'), scenarioData.position, scenarioData.heading, playerDimension);
                    await global.IsLoadEntity (ped);

                    pedData.push (ped);

                    await global.requestAnimDict(scenarioData.animation.animDict);
                    ped.taskPlayAnim(scenarioData.animation.animDict, scenarioData.animation.animName, 1, 1, -1, 1, 0.0, false, false, false);
                } 
                else {
                    pedData.push (false);
                }
            ...
            const goFrom = customizationData.goFrom;
            global.localplayer.position = new mp.Vector3(goFrom.x, goFrom.y, goFrom.z);
            
            crushStartCharacterCustomization = 7;

            const characterPosition = customizationData.startPosition;
            const ped = mp.peds.new(mp.game.joaat('mp_m_freemode_01'), new mp.Vector3(characterPosition.x, characterPosition.y, characterPosition.z), characterPosition.r, playerDimension);             
            await global.IsLoadEntity (ped);
        ...
        callRemote("client_trycatch", "player/character", "StartCharacterCustomization - " + crushStartCharacterCustomization, e.toString());
    }

    if (!isCreate) {
        call("client.characters.initChars2");

        await global.wait(500);
        call("e-dev.intro.switch_scene", "characters", 400);
        majesticCef.execute(`characterSelector.open()`);
...
const updatePedGender = async (index, gender) => {    
    ...
    if (!isCreator) {
        const scenario = currentScenario.characters[Number(index) + 1];
        pedData[index] = mp.peds.new(gender ? mp.game.joaat('mp_m_freemode_01') : mp.game.joaat('mp_f_freemode_01'), scenario.position, scenario.heading, playerDimension);
        pedData[index].freezePosition(scenario.freezePosition);
        
        await global.requestAnimDict(scenario.animation.animDict);
        pedData[index].taskPlayAnim(scenario.animation.animDict, scenario.animation.animName, 1, 1, -1, 1, 0.0, false, false, false);
    }
    else {
        const characterPosition = customizationData.startPosition;
        creatorPed = mp.peds.new(gender ? mp.game.joaat('mp_m_freemode_01') : mp.game.joaat('mp_f_freemode_01'), new mp.Vector3(characterPosition.x, characterPosition.y, characterPosition.z), characterPosition.r, playerDimension);
    }
...
function clearClothes(ped, gender) {
    try {
        ...
        callRemote("client_trycatch", "player/character", "clearClothes", e.toString());
    }
}

gm.events.add('client.characters.initChars2', async () => {
    try {
        InitPlayerCharDeb = 0;
        ...
        for (let key in customizationCharsData.face) {
            if (customizationCharsData.face [key]) {
                ...
                await updatePedGender(key, Number (customData.Gender), false);
            }
        }
        ...
        callRemote("client_trycatch", "player/character", "InitPlayerChar - " + InitPlayerCharDeb, `${e}`);
    }
});
...
gm.events.add('client.characters.initChars', async () => {
    try {
        ...
        callRemote("client_trycatch", "player/character", "InitPlayerChar - " + InitPlayerCharDeb, e.toString());
    }
});
...
const UpdateGender = async (gender) => {
    try {
        ...
        callRemote("client_trycatch", "player/character", "UpdateGender", e.toString());
    }
}
...
const UpdateAll = (gender) => {
    try {
        ...
        callRemote("client_trycatch", "player/character", "UpdateAll", e.toString());
    } 
}
...
const updateHeadBlendData = (gender) => {
    try {
        ...
        callRemote("client_trycatch", "player/character", "updateHeadBlendData", e.toString());
    }
}
const SetAppearanceChange = (gender, key, id, color, opacity) => {   
    try {
        ...
        callRemote("client_trycatch", "player/character", "SetAppearanceChange", e.toString());
    }
}
...
const UpdateAppearance = (gender, appearance) => {
    try {
        ...
        callRemote("client_trycatch", "player/character", "UpdateAppearance", e.toString());
    }
}
...
const SetCharacteristicChange = (gender, xindex, x, yindex, y) => {
    try {
        ...
        callRemote("client_trycatch", "player/character", "SetCharacteristicChange", e.toString());
    }
}
...
const SetClothesChange = (gender, key, drawable, texture) => {
    try {
        ...
        callRemote("client_trycatch", "player/character", "SetClothesChange", e.toString());
    }
}
...
const UpdateClothesChange = (gender, key) => {
    try {
        ...
        callRemote("client_trycatch", "player/character", "UpdateClothesChange", e.toString());
    }
}
...
gm.events.add('client.characters.customization.create', async (slot, firstName, lastName, gender) => {
    try {    
		...
		callRemote("client_trycatch", "player/character", "create", e.toString());
	}
});
