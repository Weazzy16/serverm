...
gm.events.add('createCheckpoint', function (uid, type, position, scale, dimension, r, g, b, dir) {
	...
	{
		callRemote("client_trycatch", "utils/checkpoints", "createCheckpoint", e.toString());
	}
});
...
gm.events.add('deleteCheckpoint', function (uid) {
	...
	{
		callRemote("client_trycatch", "utils/checkpoints", "deleteCheckpoint", e.toString());
	}
});
...
gm.events.add('createWaypoint', async (x, y) => {
	...
	{
		callRemote("client_trycatch", "utils/checkpoints", "createWaypoint", e.toString());
	}
});
...
gm.events.add('createWorkBlip', function (position, blipColor = 49) {
	...
	{
		callRemote("client_trycatch", "utils/checkpoints", "createWorkBlip", e.toString());
	}
}); 
gm.events.add('deleteWorkBlip', function () {
	...
	{
		callRemote("client_trycatch", "utils/checkpoints", "deleteWorkBlip", e.toString());
	}
});
...
gm.events.add('createGarageBlip', function (position) {
	...
	{
		callRemote("client_trycatch", "utils/checkpoints", "createGarageBlip", e.toString());
	}
});
...
gm.events.add('deleteGarageBlip', function () {
	...
	{
		callRemote("client_trycatch", "utils/checkpoints", "deleteGarageBlip", e.toString());
	}
});
...
gm.events.add('changeBlipColor', function (blip, color) {
	...
	{
		callRemote("client_trycatch", "utils/checkpoints", "changeBlipColor", e.toString());
	}
});
...
gm.events.add('changeBlipAlpha', function (blip, alpha) {
	...
	{
		callRemote("client_trycatch", "utils/checkpoints", "changeBlipAlpha", e.toString());
	}
});
...
var blip = null;
gm.events.add('BlipsHijacking', function (state, pos) {
	if (state) {
	   blip = mp.game.ui.addBlipForRadius(pos.x, pos.y, pos.z, 500);
	   mp.game.invoke("0xDF735600A4696DAF", blip, 4);
	   mp.game.invoke("0x45FF974EEE1C8734", blip, 255);
	   mp.game.invoke("0x03D7FB09E75D6B7E", blip, 47);
	}
	else {
