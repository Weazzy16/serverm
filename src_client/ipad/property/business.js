...
const
    clientName = "client.ipad.business.",
    rpcName = "rpc.ipad.business.",
    serverName = "server.ipad.business.";

gm.events.add(clientName + "load", (bizId) => {
    callRemote(serverName + "load", bizId);
...
gm.events.add(clientName + "init", (statsJson, stocksJson, ordersJson) => {
    ...
    mp.gui.emmit(`window.listernEvent ('ipadBusinessInit');`);
    mp.gui.emmit(`window.listernEvent ('ipadBusinessUpdate');`);
});
...
gm.events.add(clientName + "extraCharge", (name, value) => {
    callRemote(serverName + "extraCharge", name, value);
});
...
gm.events.add(clientName + "maxProducts", () => {
    callRemote(serverName + "maxProducts");
});
...
gm.events.add(clientName + "addOrder", (name, value) => {
    callRemote(serverName + "addOrder", name, value);
});
...
gm.events.add(clientName + "cancelOrder", (uid) => {
    ...
    if (orders [orderIndex])
        callRemote(serverName + "cancelOrder", uid);
});
...
gm.events.add(clientName + "loadStats", () => {
    ...
    if (!jsonStats || jsonStats.length <= 1)
        callRemote(serverName + "loadStats");
    else
        initStats ();
});
...
const initStats = () => {
    ...
    mp.gui.emmit(`window.listernEvent ('ipadBusinessStatsInit', '${jsonStats}', '${jsonProducts}', '${jsonUsers}');`);
}

gm.events.add(clientName + "sell", () => {
    call('client.ipad.close');
    callRemote(serverName + "sell");
});