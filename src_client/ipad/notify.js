...
const UpdateNotification = () => {
    if (typeof notifications[0] === "object") {
        ...
        mp.gui.emmit(`window.listernEvent ('ipad.notify', '${JSON.stringify(notifyData)}');`);
    } else
        mp.gui.emmit(`window.listernEvent ('ipad.notify', false);`);
}

const Tick = () => {
    if ((!global.menuOpened || global.isipadOpen) && notifications.length > 0) {
        if (typeof notifications[0] === "object") {
...
gm.events.add('ipad.notify', (number, msg, time) => {
