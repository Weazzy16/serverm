...
const
    clientName = "client.ipad.mech.",
    rpcName = "rpc.ipad.mech.",
    serverName = "server.ipad.mech.";
...
const onCancelOrder = () => {
    ...
    call('deleteBlip', "mech");
    call('deleteBlip', "mechCircle");
}
...
const onUpdateOrder = () => {
    ...
    mp.gui.emmit(`window.listernEvent ('ipad.mech.load');`);

    if (orderMech.pos) {

        call("createBlip", "mech", translateText("Такси"), 636, orderMech.pos, 1.25, 5);

        if (typeof orderMech.shape === "undefined") {
            const playerPos = mp.players.local.position;

            call("createBlip", "mechCircle", "", 9, playerPos, false, 5, 155, undefined, 15);
...
gm.events.add("render", () => {
    ...
    if (Natives.IS_WAYPOINT_ACTIVE ()) {
        ...
        if(returnPosition !== null)
            mp.gui.emmit(`window.listernEvent ('ipadmechPos', '${JSON.stringify(returnPosition)}');`);
    }
