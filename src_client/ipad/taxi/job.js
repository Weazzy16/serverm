...
const
    clientName = "client.ipad.taxijob.",
    rpcName = "rpc.ipad.taxijob.",
    serverName = "server.ipad.taxijob.";
...
gm.events.add(clientName + "add", (id, name, posX, posY, posZ) => {
    ...
    mp.gui.emmit(`window.listernEvent ('ipad.taxijob.update');`);
    ...
    call('ipad.notify', 228, translateText("Появился новый заказ! :)"), 4);
});
...
gm.events.add(clientName + "cancel", () => {
    callRemote("server.ipad.taxi.cancel");
});

gm.events.add(clientName + "successCancel", () => {
    selectedOrders = {};
    global.isTaxiOrder = false;
    mp.gui.emmit(`window.listernEvent ('ipad.taxijob.load');`);
});
