...
global.binderFunctions.acceptPressed = () => {
    ...
    callRemote('acceptPressed');
    global.lastCheck = new Date().getTime();
};

global.binderFunctions.cancelPressed = () => {
    if (!global.loggedin || global.chatActive || global.editing || new Date().getTime() - global.lastCheck < 1000 || global.menuCheck() || global.isDemorgan) return;
    callRemote('cancelPressed');
    global.lastCheck = new Date().getTime();
};
...
gm.events.add('restart.add', (message) => {
	...
	callRemote('restart.add', message);
});
...
global.binderFunctions.interactionPressed = () => {// E key
	...
	if (global.selectFestive != undefined && global.selectFestive != -1 && global.selectFestive.fId != undefined && !global.localplayer.vehicle) 
	{
		...
		{
            call('notify', 4, 9, translateText("Немного подождите... на вас действует минутное ограничение, вы очень быстрый."), 10000);
            return;
        }
        callRemote('server.events.collect', global.selectFestive.fId);
		return;
	} else if (global.selectFightId != undefined && global.selectFightId != -1 && !global.localplayer.vehicle && !global.canHackAirdrop) {
		if (global.ANTIANIM) return;
		callRemote('server.fight.open', global.selectFightId);
		return;	
		...
		if (global.ANTIANIM) return;
		callRemote('server.matwar.fight.open', global.selectMatwarFightId);
		return;	
	}/* else if (global.selectPed != undefined && global.selectPed != -1 && global.selectPed.questName && !global.localplayer.vehicle) {
		if (global.ANTIANIM) return; 
		callRemote('server.quest.open.' + global.selectPed.questName);
		return;
	} */else if (global.selectBear != undefined && global.selectBear != -1) {
		if (global.ANTIANIM) return; 
		callRemote('server.take_quest_item', global.selectBear);
		return;
	} else if (global.dfdayMissionCanPress === true) {
		if (global.ANTIANIM) return; 
		call('client.update.npc_dfday_mission');
		return;
	} else if (global.isSeat) {
		if (global.localplayer.vehicle) return; 
		call('client.seat');
	} else {
		if (global.isEnter === "buyMetro" || global.isEnter === "exitMetro") call('metroEnter');
		else callRemote('server.useEvent');
	}
	global.lastCheck = new Date().getTime();
};
...
global.binderFunctions.playerPressFollowBut = () => {// Z key
	...
	callRemote('playerPressFollowBut');
	global.lastCheck = new Date().getTime();
};
    ...
    try {
        ...
        if (global.localplayer.vehicle.getPedInSeat(-1) == global.localplayer.handle) {
            if (global.VehicleSeatFix > new Date().getTime()) return;
                ...
                // Список разрешённых моделей автомобилей для автопилота
                const allowedModels = [
                    mp.game.joaat("models"),
                    mp.game.joaat("modelx"),
                    mp.game.joaat("cybertruck")
                ];

                const vehicleModel = global.localplayer.vehicle.getModel();
                if (!allowedModels.includes(vehicleModel)) {
                    call('notify', 1, 9, translateText("Автопилот недоступен для этого автомобиля."), 1000);
                    return;
                }
                ...
                switch (global.localplayer.vehicle.getClass()) {
                    ...
                    case 18: // Emergency
                    case 21: // Trains
                        call('notify', 1, 9, translateText("Автопилот недоступен."), 1000);
                        return;
                }
                ...
                if (coord !== null) {
                    ...
                    call('notify', 2, 9, translateText("Вы включили автопилот. Обратите внимание, что данная функция не освобождает Вас от RP ситуаций."), 5000);
                        ...
                        apinterval = setInterval(function () {
                            ...
                            if (!Natives.IS_WAYPOINT_ACTIVE() || !global.localplayer.vehicle || (petrol1 !== undefined && petrol1 <= 0) || (engine1 != null && !engine1)) {
                                if (global.localplayer.vehicle) {
                                    ...
                                    call('notify', 2, 9, translateText("Автопилот отключён."), 1500);
                                }
        ...
        callRemote("client_trycatch", "main", "onAutoPilot", e.toString());
    }
};
...
global.binderFunctions.onSendWaypoint = () => {
	...
	if (global.localplayer.vehicle.getPedInSeat(-1) != global.localplayer.handle) {
        let coord = GetWaypointCoords();
        if (coord !== null) callRemote('syncWaypoint', coord.x, coord.y);
    }
...
gm.events.add('syncWP', function (bX, bY, type) {
    if(!Natives.IS_WAYPOINT_ACTIVE ()) {
		...
		if(type == 0) call('notify', 2, 9, translateText("Пассажир передал Вам информацию о своём маршруте!"), 3000);
		else if(type == 1) call('notify', 2, 9, translateText("Человек из списка контактов Вашего телефона передал Вам метку его местоположения!"), 3000);
	} else {
		if(type == 0) call('notify', 4, 9, translateText("Пассажир попытался передать Вам информацию о маршруте, но у Вас уже установлен другой маршрут."), 5000);
		else if(type == 1) call('notify', 4, 9, translateText("Человек из списка контактов Вашего телефона попытался передать Вам метку его местоположения, но у Вас уже установлена другая метка."), 5000);
	}
});

mp.keys.bind(global.Keys.VK_OEM_3, false, function () { // ` key
	if ((mp.game.ui.isPauseMenuActive() && !mp.gui.cursor.visible) || global.isEditor || global.chatActive || f1pressed || (global.menuCheck () && !chipTuning.opened && !global.dropEditor && global.blackjack.selectTable == null && global.rouletteplay == false && !global.horsesplaying && mp.gui.cursor.visible)) return;
...
gm.events.add("playerRuleTriggered", (rule, counter) => 
{
    if (rule === 'ping' && counter > 5) {
        call('notify', 4, 2, translateText("Ваш ping слишком большой. Зайдите позже"), 5000);
        callRemote("kickclient");
    }
    /*if (rule === 'packetLoss' && counter => 10) {
        call('notify', 4, 2, translateText("У Вас большая потеря пакетов. Зайдите позже"), 5000);
        callRemote("kickclient");
    }*/
});
