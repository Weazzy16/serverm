...
const SetFollowPet = (entity) => {
	...
	{
		callRemote("client_trycatch", "synchronization/clothes", "SetFollowPet", e.toString());
	}
}
...
gm.events.add('client.initPet', (ped, health, petName) => {
    ...
    {
        callRemote("client_trycatch", "world/petSystem", "initPet", e.toString());
    }
});
...
gm.events.add("render", () => {
	...
	/*if (isOpenUsePet && petData.entity && mp.peds.exists(petData.entity) && mp.keys.isDown(global.Keys.VK_RBUTTON)) {
		...
		global.OnPetEditor (petData.entity.model, data, (pos, rot, index) => {
			mp.gui.chat.push("asd - " + pos.x, pos.y, pos.z, rot);
			//callRemote('server.object.finish', data [index], pos.x, pos.y, pos.z, rot, 1800000);
		})
	}*/
...
const OnTimer = async () => {
	try {
		...
		{	
			...
			if (!Natives.DOES_ENTITY_EXIST (petData.ballHandle)) {
				...
				callRemote('server.pet.dellball', petData.entity, petData.ballPosition.x, petData.ballPosition.y, petData.ballPosition.z);
    ...
    {
        callRemote("client_trycatch", "world/petSystem", "OnTimer", e.toString());
    }
}
...
const onToFollow = (player, type) => {
	try {
		...
		if (distance <= 2.0 && pedDimension === playerDimension) {
			...
			if (type == "me" && petData.ballThrown) {
				petData.ballThrown = false; 
				callRemote('server.pet.getBall', petData.entity);
				...
				petData.toSniffTimer = setTimeout (() => {
					...
					petData.entity.freeze = false;
					ClearAttackInterval ();
					callRemote('server.pet.sniff', player);
				}, 2500);
			}
		} else {
    ...
    {
        callRemote("client_trycatch", "world/petSystem", "onToFollow", e.toString());
    }
}
...
function freezePet(pet) {
	...
	{
		callRemote("client_trycatch", "world/petSystem", "freezePet", e.toString());
	}
}
	...
	try {
		if (pet) {
			if (status) {
				...
				if (msg) 
					call('notify', 4, 9, translateText("Питомец сел."), 3000);
			} else {
				global.requestAnimDict(Config.Freeze.dict).then(async () => {
					pet.taskPlayAnim (Config.Freeze.dict, Config.Freeze.anim, 1, 1.0, -1, 0, 1.0, false, false, false);
				});
				...
				if (msg) 
					call('notify', 4, 9, translateText("Питомец встал."), 3000);
			}
		}
	}
	catch (e)
	{
		callRemote("client_trycatch", "world/petSystem", "sitPet", e.toString());
	}
}
...
function sleepPet(pet, status, msg = true) {
		...
		if (pet) {
			if (status) {
				global.requestAnimDict(Config.Sleep.dict).then(async () => {
				   ...
				   if(msg) 
					   call('notify', 4, 9, translateText("Питомец лёг."), 3000);			
			} else {
				global.requestAnimDict(Config.Freeze.dict).then(async () => {
					pet.taskPlayAnim (Config.Freeze.dict, Config.Freeze.anim, 1, 1.0, -1, 0, 1.0, false, false, false);
				});
	...
	{
		callRemote("client_trycatch", "world/petSystem", "sleepPet", e.toString());
	}
}
...
function findBall(pet) {
	...
	{
		callRemote("client_trycatch", "world/petSystem", "findBall", e.toString());
	}
}
...
gm.events.add('client.pet.dellball', (xPos, yPos, zPos) => {
	...
	{
		callRemote("client_trycatch", "world/petSystem", "dellball", e.toString());
	}
});
...
gm.events.add('client.pet.follow', () => {
	try {
		...
		if (distance >= 7.0) 
		{
			call('notify', 4, 9, translateText("Подойдите ближе к питомцу, чтобы отдавать ему команды."), 3000);
			return;
		}
		...
		if (!petData.entity.freeze) call('notify', 4, 9, translateText("Питомец будет следовать за Вами."), 3000);
		else 
		{
			freezePet(petData.entity);
			call('notify', 4, 9, translateText("Питомец будет ждать тут."), 3000);
		}
	...
	{
		callRemote("client_trycatch", "world/petSystem", "follow", e.toString());
	}
});
...
gm.events.add('client.pet.sit', async () => {
	try {
		...
		if (distance >= 10.0) {
			...
			call('notify', 4, 9, translateText("Чтобы выполнить это действие, питомец не должен лежать."), 3000);
			return;
		}
...
gm.events.add('client.pet.getBall', async () => {
	...
	{
		callRemote("client_trycatch", "world/petSystem", "getBall", e.toString());
	}
});
...
gm.events.add('client.pet.sleep', async () => {
	...
	{
		callRemote("client_trycatch", "world/petSystem", "sleep", e.toString());
	}
});
...
const ClearAttackInterval = (isAttack = false) => {
	...
	{
		callRemote("client_trycatch", "world/petSystem", "ClearAttackInterval", e.toString());
	}
}
...
gm.events.add('client.pet.attack', (playerId) => {
	...
	{
		callRemote("client_trycatch", "world/petSystem", "attack", e.toString());
	}
});
...
gm.events.add('client.pet.sniff', (playerId) => {
	...
	{
		callRemote("client_trycatch", "world/petSystem", "sniff", e.toString());
	}
});
...
gm.events.add('client.pet.toFollow', (playerId) => {
	...
	{
		callRemote("client_trycatch", "world/petSystem", "toFollow", e.toString());
	}
});
...
const updateMenu = () => {
	...
	{
		callRemote("client_trycatch", "world/petSystem", "updateMenu", e.toString());
	}
}
...
const getPlayers = (events) => {	
	...
	{
		callRemote("client_trycatch", "world/petSystem", "getPlayers", e.toString());
	}
}
...
gm.events.add('client.pet.nameChange', (newPetName) => {
	...
	{
		callRemote("client_trycatch", "world/petSystem", "nameChange", e.toString());
	}
});
...
const getPeds = (events) => {	
	...
	{
		callRemote("client_trycatch", "world/petSystem", "getPeds", e.toString());
	}
}
...
gm.events.add('client.pet.setInVehicle', () => {
	try {
		...
		call('notify', 4, 9, translateText("Питомец не смог найти свободное место и теперь ждет новую команду!"), 7000);
		petData.entity.freeze = true;
		freezePet(petData.entity);
	...
	{
		callRemote("client_trycatch", "world/petSystem", "setInVehicle", e.toString());
	}
});
...
const Open = async (json) => {
    ...
    {
        callRemote("client_trycatch", "shop/newshop/index", "client.shop.open", e.toString());
    }
}
...
const OnPetshopBuy = (index) => {
    callRemote('server.petshop.buy', index);
}
