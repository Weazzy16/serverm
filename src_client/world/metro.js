...
const CreateShape = () => {
    ...
    {
        callRemote("client_trycatch", "world/other", "CreateShape", e.toString());
    }
}
...
gm.events.add("metroEnter", () => {
    ...
    {
        callRemote("client_trycatch", "world/other", "metroEnter", e.toString());
    }
});
...
gm.events.add("playerEnterColshape", (shape) => {
    ...
    {
        callRemote("client_trycatch", "world/other", "playerEnterColshape", e.toString());
    }
});
gm.events.add("playerExitColshape", (shape) => {
    ...
    {
        callRemote("client_trycatch", "world/other", "playerExitColshape", e.toString());
    }
});
...
gm.events.add("client.metro.buyTicket", async (station, Increase) => {
    if (!metroData.metroColshape)
        ...
        call('notify', 4, 9, translateText("Вы и так находитесь на данной станции!"), 3000);
        return;
    }
    callRemote("metro.server.buyTicket", station, Increase);
});
...
gm.events.add("metro.server.buyTicketSuccess", async (station) => {
    ...
    {
        callRemote("client_trycatch", "world/other", "buyTicketSuccess", e.toString());
    }
});
...
const StartMetro = async (stationData) => {
    ...
    {
        callRemote("client_trycatch", "world/other", "StartMetro", e.toString());
    }
}
const StopMetro = async (isEnd) => {
    try {
        ...
        if (isEnd || LastStation === selectMetro.name)
            ...
            call('notify', 2, 9, translateText("Вы на станции {0}. Это еще не Ваша станция, но можете выйти здесь.", selectMetro.name), 9000);
            call('hud.oEnter', "exitMetro");
    ...
    {
        callRemote("client_trycatch", "world/other", "StopMetro", e.toString());
    }
};
...
gm.events.add("metro.clear", async () => {
    try {
        ...
        callRemote("metro.server.exit");
        selectMetro = null;
        LastStation = null;
        call("setTraffic", 0);
        call("cleartraffic");
        call('hud.cEnter');
        //callRemote("metro.server.stop");
    }
    catch (e) 
    {
        callRemote("client_trycatch", "world/other", "clear", e.toString());
    }
});
