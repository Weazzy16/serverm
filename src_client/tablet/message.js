...
rpc.register(rpcName + "getMessages", () => {
    ...
    messagesData.list.forEach(message => {
        ...
        const contactData = tabletData.contacts [newMessage.Number];
...
gm.events.add(clientName + "updMsgStatus", (number, key, date, status) => {
    ...
    if (typeof message === "object") {
        ...
        if (status === messageStatus.received)
            global.tabletSound ("msgReceived", "messagesent.ogg");
...
gm.events.add(clientName + "msgAdd", (number, text, date, type) => {
    ...
    if (isStatus)
        ...
        mp.events.call('tablet.notify', number, notifyText, 5);
    }

    updateListData (number, JSON.parse(date), text, type, false, isStatus);


    //global.tabletSound ("msgReceived", "newmessage.ogg");
});
...
const setChatStatus = (number) => {
    if (messagesData.ChatStatus [number]) {
        ...
        if (chatStatus.IsWrite)
            mp.gui.emmit(`window.listernEvent ('tabletChatUpdStatus', 3);`);
        else
            mp.gui.emmit(`window.listernEvent ('tabletChatUpdStatus', ${chatStatus.status});`);
    } else
        mp.gui.emmit(`window.listernEvent ('tabletChatUpdStatus', 0);`);
}


let statusAntiFlood = 0;

gm.events.add(clientName + "getTabletChatStatus", (number) => {
    const contact = global.getContact(number);

    if (contact && contact.IsSystem) {
        mp.gui.emmit(`window.listernEvent ('tabletChatUpdStatus', 1);`);
        return;
    }
...
gm.events.add(clientName + "setTabletChatStatus", (number, status) => {
...
gm.events.add(clientName + "setTabletChatWrite", (number, toggled) => {
    if (typeof messagesData.ChatStatus [number] !== "object")
        messagesData.ChatStatus [number] = {};
...
gm.events.add(clientName + "startWrite", () => {
    if (typeof selectNumber === "number") {
        if (tabletData.blackList.includes (selectNumber))
            return;
