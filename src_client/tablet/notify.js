...
const UpdateNotification = () => {
    if (typeof notifications[0] === "object") {
        ...
        mp.gui.emmit(`window.listernEvent ('tablet.notify', '${JSON.stringify(notifyData)}');`);
    } else
        mp.gui.emmit(`window.listernEvent ('tablet.notify', false);`);
}

const Tick = () => {
    if ((!global.menuOpened || global.isTabletOpen) && notifications.length > 0) {
        if (typeof notifications[0] === "object") {
...
gm.events.add('tablet.notify', (number, msg, time) => {
