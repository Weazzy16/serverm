const
    clientName = "client.tablet.mechjob.",
    rpcName = "rpc.tablet.mechjob.",
    serverName = "server.tablet.mechjob.";
...
gm.events.add(clientName + "init", (json) => {
    ...
    mp.gui.emmit(`window.listernEvent ('tablet.mech.getMenu');`);
})
...
gm.events.add(clientName + "add", (id, name, posX, posY, posZ) => {
    ...
    mp.gui.emmit(`window.listernEvent ('tablet.mechjob.update');`);
    ...
    mp.events.call('tablet.notify', 333, translateText("Появился новый заказ! :)"), 4);
});
...
gm.events.add(clientName + "cancel", () => {
    mp.events.callRemote("server.tablet.mech.cancel");
});
...
gm.events.add(clientName + "successCancel", () => {
    ...
    mp.gui.emmit(`window.listernEvent ('tablet.mechjob.load');`);
});
