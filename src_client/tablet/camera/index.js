...
const updateInterfaceState = () => {
    ...
    let popupData = {
        ...
        isTabletThisFrame: isTabletThisFrame,
...
global.tabletCameraOpen = false;

gm.events.add('camera.open', (_isDofEnabled = true) => {
    isTabletThisFrame = false;
    isFreeMode = false;
    isMode = false;
...
const onDestroy = (_isScreen) => {
    ...
    mp.events.callRemote("server.tablet.anim", 0);
...
const onScreen = async () => {
    ...
    mp.game.audio.playSoundFrontend(-1, "Camera_Shoot", "Tablet_SoundSet_Michael", true);
    return;
}
...
const CreateMobile = () => {
    ...
    nativeInvoke ("_CELL_CAM_DISABLE_THIS_FRAME", isTabletThisFrame);
    nativeInvoke ("0xA2CCBE62CD4C91A4", nativeInvoke ("_SET_MOBILE_PHONE_UNK", dofEnabled));
    mp.events.callRemote("server.tablet.anim", 1);
}
...
const changeView = async () => {
    //if (mp.keys.isUp(40)/* && global.actionAntiFlood("__smarttabletScaleVisibleChange", 500)*/) {
    if (!isMode && mp.game.controls.isControlJustPressed(3, 172)) {
        ...
        //Natives.CellFrontCamActivate (isTabletThisFrame)
        nativeInvoke ("_CELL_CAM_DISABLE_THIS_FRAME", isTabletThisFrame);
