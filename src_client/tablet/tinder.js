const
    clientName = "client.tablet.tinder.",
    rpcName = "rpc.tablet.tinder.",
    serverName = "server.tablet.tinder.";
...
gm.events.add(clientName + "init", (_profile, _likes, _list) => {
    ...
    mp.gui.emmit(`window.listernEvent ('tablet.tinder.load', true);`);
});

gm.events.add(clientName + "create", () => {

    mp.gui.emmit(`window.listernEvent ('tablet.tinder.load', false);`);
});
...
gm.events.add(clientName + "addList", (_list, isClear) => {
    ...
    addList (_list);
    mp.gui.emmit(`window.listernEvent ('tablet.tinder.getList');`);
});
...
gm.events.add(clientName + "addLikes", (name, avatar, number) => {
    ...
    mp.gui.emmit(`window.listernEvent ('tablet.tinder.getLikes');`);
});
...
gm.events.add(clientName + "save", (avatar, text, type, isVisible) => {
    ...
    mp.gui.emmit(`window.listernEvent ('tablet.tinder.getProfile');`);
