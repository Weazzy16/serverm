const
    clientName = "client.tablet.taxijob.",
    rpcName = "rpc.tablet.taxijob.",
    serverName = "server.tablet.taxijob.";
...
gm.events.add(clientName + "init", (json) => {
    ...
    mp.gui.emmit(`window.listernEvent ('tablet.taxi.getMenu');`);
})
...
gm.events.add(clientName + "add", (id, name, posX, posY, posZ) => {
    ...
    mp.gui.emmit(`window.listernEvent ('tablet.taxijob.update');`);
    ...
    mp.events.call('tablet.notify', 228, translateText("Появился новый заказ! :)"), 4);
});
...
gm.events.add(clientName + "cancel", () => {
    mp.events.callRemote("server.tablet.taxi.cancel");
});

gm.events.add(clientName + "successCancel", () => {
    selectedOrders = {};
    global.isTaxiOrder = false;
    mp.gui.emmit(`window.listernEvent ('tablet.taxijob.load');`);
});
